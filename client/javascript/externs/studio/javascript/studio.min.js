...
nsVFB.ElementForm.prototype.elementFormController=function($scope,$rootScope,$timeout,$q){oVFB.log("nsVFB.ElementForm.elementFormController");var elementFormController=this;this["applicationName"]=oVFB.getApplication();$rootScope["Utils"]={"keys":Object.keys};$scope["model"]={};$scope["collapsed"]=oVFB.Collapse_[4];$scope["aWebServiceAttributs"]=[];$scope["aBusinessObjects"]=[];$scope["aBoAttributs"]=[];$scope["aFormAttributs"]=[];$scope["datasourcesArray"]=[];$scope.tmp={};var selectedElement={};
var selectedElementInTreeview={};$scope.$on("labelizeForm",function(){console.log(elementFormController);var oJson=oVFB.getJsonOutput();var oForm=oJson[$rootScope["selected_form_type"]];var inputType=["select","list","double_select","date","datetime","color_picker","text","password","url","email","integer","float","number"];var mapType=["map_osm","map_bing","map_vmap"];var disabledType=["checkbox","radio","textarea","slider"];for(var i=0;i<oForm["rows"].length;i++){var oRow=oForm["rows"][i];for(var j=
0;j<oRow["fields"].length;j++){var oField=oRow["fields"][j];if(inputType.indexOf(oField["type"])>-1){elementFormController.convert(oField,"label");oField["type"]="label"}else if(mapType.indexOf(oField["type"])>-1){oField["map_options"]["interactions"]={"multi_geometry":false,"full_screen":true,"RA":false,"RO":false,"ED":false,"DP":false,"DL":false,"DPol":false,"SE":false};$rootScope.$broadcast("updateMap","interact")}else if(disabledType.indexOf(oField["type"])>-1)oField["disabled"]=true;else if(oField["type"]===
"upload"){elementFormController.convert(oField,"link");oField["type"]="link"}else if(oField["type"]==="codemirror")oField["cm_options"]["readOnly"]=true}}oJson[$rootScope["selected_form_type"]]=oForm;oVFB.setJsonOutput(oJson);$rootScope.$broadcast("updateEvent",oVFB.getJsonOutput());$rootScope.$broadcast("updateJsonModel",oVFB.getJsonOutput());$rootScope.$broadcast("updateForm",oVFB.getJsonOutput())});$scope.$watch("text",function(){oVFB.log("text updated");if(typeof $scope["text"]["Element"]!=="undefined"){elementFormController["text"]=
$scope["text"]["Element"];$scope["Component_list"]=[];var aAcceptedTypes=oVFB["config"]["avaliable_elems"];for(var i=0;i<aAcceptedTypes.length;i++)$scope["Component_list"].push({"alias":elementFormController["text"]["Select_Type"][aAcceptedTypes[i]],"value":aAcceptedTypes[i]})}});$scope.$watch("model.id",function(){oVFB.log("model.id changed");$scope["jsonOutput"]=oVFB.getJsonOutput();if(!goog.isDef($scope["jsonOutput"]["datasources"])||!goog.isObject($scope["jsonOutput"]["datasources"])||goog.isArray($scope["jsonOutput"]["datasources"]))$scope["jsonOutput"]["datasources"]=
{};$scope["datasources"]=$scope["jsonOutput"]["datasources"];$scope["datasourcesArray"]=[];for(var key in $scope["datasources"]){$scope["datasources"][key]["id"]=key;$scope["datasourcesArray"].push($scope["datasources"][key])}elementFormController["updateDatasourceInfos"]();if($scope["model"]["type"]==="select"||$scope["model"]["type"]==="list"||$scope["model"]["type"]==="double_select")if(goog.isDefAndNotNull($scope["model"]["datasource"])&&goog.isDefAndNotNull($scope["model"]["datasource"]["parents"])){$scope["cascade"]=
true;$scope["aParents"]=[];for(var i=0;i<$scope["model"]["datasource"]["parents"].length;i++)$scope["aParents"].push($scope["model"]["datasource"]["parents"][i]["name"])}else{$scope["cascade"]=false;$scope["aParents"]=[]}oVFB.setJsonOutput($scope["jsonOutput"])});$scope.$watch("model.datasource.datasource_id",function(){oVFB.log("model.datasource.datasource_id changed");if(!goog.isDefAndNotNull($scope["model"]["datasource"]))return 0;if(!goog.isDefAndNotNull($scope["model"]["datasource"]["datasource_id"]))return 0;
if(!goog.isDefAndNotNull($scope["datasources"][$scope["model"]["datasource"]["datasource_id"]]))return 0;var datasource=$scope["datasources"][$scope["model"]["datasource"]["datasource_id"]];if(datasource["type"]==="web_service")goog.array.clear($scope["aWebServiceAttributs"]);elementFormController["updateDatasourceInfos"]()});$scope.$watch("model.default_value",function(){$rootScope.$broadcast("updateDefautValue",$scope["model"])});$scope.$watch("model.name",function(current,old){$scope["jsonOutput"]=
oVFB.getJsonOutput();var aElems=oVFB.utils_.getElementsByName($scope["jsonOutput"],$rootScope["selected_form_type"],current);if(aElems.length>1){$scope["model"]["name"]=$scope["model"]["name"]+"_2";$.notify($scope["text"]["Element"]["SameNameWarning"],"warning")}if(goog.isString(old))if(old.length>0)if(!goog.isDefAndNotNull($scope["model"]["name"])){$scope["model"]["name"]=old;$.notify($scope["text"]["Element"]["EmptyNameWarning"],"warning")}});$scope.$watch("model",function(current,old){$rootScope.$broadcast("update model",
null);if(goog.isDefAndNotNull(current["type"])&&goog.isDefAndNotNull(current["id"]))if(current["type"]!==old["type"]&&current["id"]===old["id"]){oVFB.Update=true;if(selectedElement.length>0){if(goog.isDefAndNotNull(selectedElement[0]))selectedElement[0]["icon"]=elementFormController.convert($scope["model"],current["type"]);if(goog.isDefAndNotNull(selectedElementInTreeview[0]))selectedElementInTreeview[0]["icon"]=elementFormController.convert($scope["model"],current["type"])}if(current["type"]==="select")if(elementFormController["optionsSelect"]===
"Text"){if(goog.isDef(current["web_service"]))goog.object.remove(current,"web_service");$scope["selectedRessource"]="";$scope["selectedValue"]="";$scope["selectedLabel"]=""}else if(elementFormController["optionsSelect"]==="TableValues"){if(goog.isDef(current["options"]))goog.object.remove(current,"options");$scope["selectedRessource"]="";$scope["selectedValue"]="";$scope["selectedLabel"]=""}else if(elementFormController["optionsSelect"]==="WebService"){if(goog.isDef(current["options"]))goog.object.remove(current,
"options");if(goog.isDef(current["web_service"]["parameters"]))goog.object.remove(current["web_service"],"parameters")}}else if(current["id"]===old["id"]&&current["name"]!==old["name"]){oVFB.Update=true;if(goog.isDef(selectedElement[0])&&goog.isDef(selectedElementInTreeview[0])){selectedElement[0]["text"]=current["name"];selectedElementInTreeview[0]["text"]=current["name"]}oVFB.updateElemNameInTabs(current,old);oVFB.updateElemNameInWab(current,old)}else if(current["id"]===old["id"]){if(current["type"]==
"ui_grid"){var oGridOptions=angular.element("#"+$scope["model"]["id"]+"_grid .ui-grid-contents-wrapper").scope()["grid"]["options"];if($scope["bListenUiGridEvent"]!==true){oGridOptions["columnDefs"]=$scope["model"]["options"]["columnDefs"];oGridOptions["appActions"]=$scope["model"]["options"]["appActions"];$scope["bListenUiGridEvent"]=true;var oGridApi=angular.element("#"+$scope["model"]["id"]+"_grid .ui-grid-contents-wrapper").scope()["grid"]["api"];oGridApi["colMovable"].on["columnPositionChanged"]($scope,
function(colDef,originalPosition,newPosition){var oColumns=angular.element("#"+$scope["model"]["id"]+"_grid .ui-grid-header-cell-row").scope().grid.columns;var aColumnsDef=[];for(var i=0;i<oColumns.length;i++)if(oColumns[i]["field"]!="selectionRowHeaderCol")aColumnsDef.push(oColumns[i]["colDef"]);$scope["model"]["options"]["columnDefs"].length=0;$scope["model"]["options"]["columnDefs"]=angular.copy(aColumnsDef);aColumnsDef.length=0});var uiGridConstants=angular.element(vitisApp.appWorkspaceListDrtv).injector().get(["uiGridConstants"]);
oGridApi["core"]["on"]["sortChanged"]($scope,function(grid,sortColumns){var sSortOrder=sortColumns[0]["sort"]["direction"].toUpperCase();for(var i=0;i<$scope["model"]["options"]["columnDefs"].length;i++){delete $scope["model"]["options"]["columnDefs"][i]["sort"];if($scope["model"]["options"]["columnDefs"][i]["field"]==sortColumns[0]["field"])$scope["model"]["options"]["columnDefs"][i]["sort"]={"direction":uiGridConstants[sSortOrder],"ignoreSort":true,"priority":0}}if(typeof $scope["model"]["datasource"]!=
"undefined"){$scope["model"]["datasource"]["order_by"]=sortColumns[0]["field"];$scope["model"]["datasource"]["sort_order"]=sSortOrder}})}document.querySelector("#studio_form_reader .ui-grid-form-field-header-title > span").textContent=current["options"]["appGridTitle"];if($scope["model"]["options"]["appHeader"])document.getElementById($scope["model"]["id"]+"_grid_header").style.display="";else document.getElementById($scope["model"]["id"]+"_grid_header").style.display="none";if($scope["model"]["label"]==
"")document.getElementById($scope["model"]["id"]+"_label").style.display="none";else document.getElementById($scope["model"]["id"]+"_label").style.display="";if($scope["bEditColumn"]===true){$scope["bShowColumnParameters"]=true;for(var i=0;i<$scope["model"]["options"]["columnDefs"].length;i++)if($scope["model"]["options"]["columnDefs"][i]["field"]==$scope["sSelectedColumn"]){$scope["columnDef"]=angular.copy($scope["model"]["options"]["columnDefs"][i]);break}$scope["bEditColumn"]=false}else if($scope["bAddColumn"]===
true){$scope["columnDef"]={"displayName":"","field":"","width":""};$scope["bAddColumn"]=false}if($scope["bSaveColumn"]===true){$scope["bSaveColumn"]=false;var oColumnDef=Object.assign({"name":$scope["columnDef"]["field"],"type":"string","enableSorting":true,"enableColumnMoving":true,"enableColumnResizing":true},$scope["columnDef"]);if(!isNaN(oColumnDef["width"]))oColumnDef["width"]=parseInt(oColumnDef["width"]);if(oColumnDef["displayName"]!=""&&oColumnDef["field"]!=""&&oColumnDef["width"]!="")if(typeof $scope["sSelectedColumn"]==
"undefined"||$scope["sSelectedColumn"]==""){$scope["model"]["options"]["columnDefs"].push(angular.copy(oColumnDef));$scope["sSelectedColumn"]=oColumnDef["field"]}else for(var i=0;i<$scope["model"]["options"]["columnDefs"].length;i++)if($scope["model"]["options"]["columnDefs"][i]["field"]==$scope["sSelectedColumn"]){$scope["model"]["options"]["columnDefs"][i]=angular.copy($scope["columnDef"]);break}}if($scope["bDeleteColumn"]===true){$scope["bDeleteColumn"]=false;for(var i=0;i<$scope["model"]["options"]["columnDefs"].length;i++)if($scope["model"]["options"]["columnDefs"][i]["field"]==
$scope["sSelectedColumn"]){$scope["model"]["options"]["columnDefs"].splice(i,1);$scope["sSelectedColumn"]="";break}}if($scope["bEditButton"]===true){$scope["bShowButtonParameters"]=true;for(var i=0;i<$scope["model"]["options"]["appActions"].length;i++)if($scope["model"]["options"]["appActions"][i]["name"]==$scope["sSelectedButton"]){$scope["appAction"]=angular.copy($scope["model"]["options"]["appActions"][i]);break}$scope["bEditButton"]=false}else if($scope["bAddButton"]===true){$scope["appAction"]=
{"name":"","label":"","event":""};$scope["bAddButton"]=false}if($scope["bSaveButton"]===true){$scope["bSaveButton"]=false;if($scope["appAction"]["label"]!=""&&$scope["appAction"]["name"]!="")if(typeof $scope["sSelectedButton"]=="undefined"||$scope["sSelectedButton"]==""){$scope["model"]["options"]["appActions"].push(angular.copy($scope["appAction"]));$scope["sSelectedButton"]=$scope["appAction"]["name"]}else for(var i=0;i<$scope["model"]["options"]["appActions"].length;i++)if($scope["model"]["options"]["appActions"][i]["name"]==
$scope["sSelectedButton"]){$scope["model"]["options"]["appActions"][i]=angular.copy($scope["appAction"]);break}}if($scope["bDeleteButton"]===true){$scope["bDeleteButton"]=false;for(var i=0;i<$scope["model"]["options"]["appActions"].length;i++)if($scope["model"]["options"]["appActions"][i]["name"]==$scope["sSelectedButton"]){$scope["model"]["options"]["appActions"].splice(i,1);$scope["sSelectedButton"]="";break}}}else if(current["type"]=="treeview"){var this_=this;var oTreeviewDefaultOptions={"showBorder":false,
"levels":0,"showCheckbox":true,"expandIcon":"glyphicon glyphicon-folder-close","collapseIcon":"glyphicon glyphicon-folder-open","highlightSelected":false,"multiSelect":true};if(typeof this["getTreeviewData"]=="undefined"){this["getTreeviewData"]=function(sTreeviewElementId,bCleanNode){var aReturnData=[];var oTreeviewElem=document.getElementById(sTreeviewElementId);if(oTreeviewElem!==null){var oTreeview=$("#"+sTreeviewElementId)["treeview"](true);var aTreeviewData=oTreeview["getEnabled"]().concat(oTreeview["getDisabled"]());
var aNode;for(var i=0;i<aTreeviewData.length;i++)if(aTreeviewData[i]["parentId"]===undefined){aNode=angular.copy(aTreeviewData[i]);aReturnData.push(aNode)}}return aReturnData};this["cleanNode"]=function(aNode){for(var i=0;i<aNode.length;i++){delete aNode[i]["parentId"];delete aNode[i]["nodeId"];if(typeof aNode[i]["nodes"]!="undefined")if(aNode[i]["nodes"].length>0)aNode[i]["nodes"]=this_["cleanNode"](aNode[i]["nodes"]);else delete aNode[i]["nodes"]}return aNode};this["addTreeviewData"]=function(aTreeviewData,
aNode,iNodeId){for(var i=0;i<aTreeviewData.length;i++)if(aTreeviewData[i]["nodeId"]==iNodeId){if(!goog.isArray(aTreeviewData[i]["nodes"]))aTreeviewData[i]["nodes"]=[];aTreeviewData[i]["nodes"].push(aNode)}else if(goog.isArray(aTreeviewData[i]["nodes"]))this_["addTreeviewData"](aTreeviewData[i]["nodes"],aNode,iNodeId);return aTreeviewData};this["deleteNode"]=function(aTreeviewData,iNodeId){for(var i=0;i<aTreeviewData.length;i++)if(aTreeviewData[i]["nodeId"]==iNodeId)aTreeviewData.splice(i,1);else if(goog.isArray(aTreeviewData[i]["nodes"]))this_["deleteNode"](aTreeviewData[i]["nodes"],
iNodeId);return aTreeviewData}}var aData;var oTreeview=$("#"+$scope["model"]["id"]+"_treeview")["treeview"](true);if($scope["bListenTreeviewEvent"]!==true){$("#"+$scope["model"]["id"]+"_treeview").on("nodeSelected",function(event,data){$scope["node"]={"text":data["text"],"value":data["value"],"selectable":data["selectable"],"state":angular.copy(data["state"])};$scope["iSelectedNode"]=data["nodeId"];if(data["parentId"]!=undefined){var oNodeParent=oTreeview["getNode"](data["parentId"]);$scope["sSelectedNodeParent"]=
oNodeParent["text"]}else $scope["sSelectedNodeParent"]="";$scope["bShowNodeParameters"]=true;$scope["bEditNode"]=true;$scope.$applyAsync()});$scope["bListenTreeviewEvent"]=true}if($scope["bAddNode"]===true&&($scope["bSaveNode"]!==true||typeof $scope["bSaveNode"]=="undefined")){$scope["node"]={"text":"","value":"","selectable":true,"state":{"checked":false,"disabled":false,"expanded":true,"selected":false}};if(typeof $scope["iSelectedNode"]!="undefined"&&String($scope["iSelectedNode"])!=""){var oSelectedNode=
oTreeview["getNode"]($scope["iSelectedNode"]);$scope["sSelectedNodeParent"]=oSelectedNode["text"]}$scope.$applyAsync()}if($scope["bSaveNode"]===true){$scope["bSaveNode"]=false;if($scope["node"]["text"]!=""&&$scope["node"]["value"]!="")if($scope["bAddNode"]===true){if(typeof $scope["iSelectedNode"]=="undefined"||String($scope["iSelectedNode"])==""){$scope["model"]["options"]["data"].push(angular.copy($scope["node"]));oTreeview["init"](Object.assign(oTreeviewDefaultOptions,{"data":$scope["model"]["options"]["data"]}));
$scope["bListenTreeviewEvent"]=false;$scope["bShowNodeParameters"]=false}else{aData=this["addTreeviewData"](this_["getTreeviewData"]($scope["model"]["id"]+"_treeview"),$scope["node"],$scope["iSelectedNode"]);$scope["iSelectedNode"]="";$scope["sSelectedNodeParent"]="";$scope["bShowNodeParameters"]=false;$scope["model"]["options"]["data"].length=0;$scope["model"]["options"]["data"]=angular.copy(this_["cleanNode"](aData));oTreeview["init"](Object.assign(oTreeviewDefaultOptions,{"data":$scope["model"]["options"]["data"]}));
$scope["bListenTreeviewEvent"]=false}$scope["bAddNode"]=false;$scope["bShowNodeParameters"]=false}else if($scope["bEditNode"]===true){$scope["bEditNode"]=false;var oSelectedNode=oTreeview["getNode"]($scope["iSelectedNode"]);oSelectedNode["text"]=$scope["node"]["text"];oSelectedNode["value"]=$scope["node"]["value"];aData=this_["getTreeviewData"]($scope["model"]["id"]+"_treeview");$scope["model"]["options"]["data"].length=0;$scope["model"]["options"]["data"]=angular.copy(this_["cleanNode"](aData));
oTreeview["init"](Object.assign(oTreeviewDefaultOptions,{"data":$scope["model"]["options"]["data"]}));$scope["bListenTreeviewEvent"]=false}$scope.$applyAsync()}if($scope["bDeleteNode"]===true){$scope["bDeleteNode"]=false;$scope["bEditNode"]=false;aData=this_["deleteNode"](this_["getTreeviewData"]($scope["model"]["id"]+"_treeview"),$scope["iSelectedNode"]);$scope["iSelectedNode"]="";$scope["sSelectedNodeParent"]="";$scope["bShowNodeParameters"]=false;$scope["model"]["options"]["data"].length=0;$scope["model"]["options"]["data"]=
angular.copy(this_["cleanNode"](aData));oTreeview["init"](Object.assign(oTreeviewDefaultOptions,{"data":$scope["model"]["options"]["data"]}));$scope["bListenTreeviewEvent"]=false;$scope.$applyAsync()}}delete $scope["model"]["bCallWatch"]}$scope["model"]["selected"]=true;$timeout(function(){if(goog.isDef(selectedElement[0])&&goog.isDef(selectedElementInTreeview[0])){if(goog.isDefAndNotNull(selectedElement[0]))$("#treeWithLines")["treeview"]("selectNode",[selectedElement[0]["nodeId"],{silent:false}]);
if(goog.isDefAndNotNull(selectedElementInTreeview[0]))$("#treeView")["treeview"]("selectNode",[selectedElementInTreeview[0]["nodeId"],{silent:false}])}},10);$rootScope.$broadcast("updateJsonModel",oVFB.getJsonOutput());$rootScope.$broadcast("updateForm",oVFB.getJsonOutput())},true);$scope.$on("update element",function(event,data){oVFB.log("update element");$scope["model"]=data;selectedElement=$("#treeWithLines")["treeview"]("getSelected");selectedElementInTreeview=$("#treeView")["treeview"]("getSelected");
if(goog.isDef($scope.model))$("#select_element_to_add").val($scope.model.type);$scope["AllSelects"]=[];if(goog.isDefAndNotNull(oVFB.getJsonOutput()[$rootScope["selected_form_type"]]))if(!goog.object.isEmpty(oVFB.getJsonOutput()[$rootScope["selected_form_type"]]["rows"])){var obj=oVFB.getJsonOutput()[$rootScope["selected_form_type"]]["rows"];for(var i=0;i<obj.length;i++)for(var j=0;j<obj[i]["fields"].length;j++)if((obj[i]["fields"][j]["type"]==="select"||obj[i]["fields"][j]["type"]==="list")&&obj[i]["fields"][j]["name"]!==
$scope["model"]["name"])$scope["AllSelects"].push(obj[i]["fields"][j]["name"])}if($("#load_Perso_button").hasClass("active")&&oVFB.valid&&data["type"]!=="undefined")$scope["locker"]=false;else if(oVFB.valid){$scope["locker"]=true;if($scope["model"]["type"]==="map_vmap")$("#Vmap_Downloader_input").fileinput("disable")}if(!$scope.$$phase)$scope.$digest()});elementFormController.updateWSInfos=function(value){oVFB.log("elementFormController.updateWSInfos");if(typeof value["selectedValue"]!=="undefined")$scope["model"]["web_service"]["id_key"]=
value["selectedValue"].replace(/\s/g,"");if(typeof value["selectedLabel"]!=="undefined")$scope["model"]["web_service"]["label_key"]=value["selectedLabel"].replace(/\s/g,"");$scope["model"]["web_service"]["optionsSelect"]=elementFormController["optionsSelect"]};elementFormController["updateDatasourceInfos"]=function(){oVFB.log("elementFormController.updateDatasourceInfos");if(!goog.isDefAndNotNull($scope["model"]["datasource"]))return 0;$scope["model"]["datasource"]["sort_order"]=goog.isDefAndNotNull($scope["model"]["datasource"]["sort_order"])?
$scope["model"]["datasource"]["sort_order"]:"ASC";$scope["model"]["datasource"]["distinct"]=goog.isDefAndNotNull($scope["model"]["datasource"]["distinct"])?$scope["model"]["datasource"]["distinct"]:"true";if(goog.isDefAndNotNull($scope["model"]["datasource"]["id_key"]))$scope["model"]["datasource"]["id_key"]=$scope["model"]["datasource"]["id_key"].replace(/\s/g,"");if(goog.isDefAndNotNull($scope["model"]["datasource"]["label_key"]))$scope["model"]["datasource"]["label_key"]=$scope["model"]["datasource"]["label_key"].replace(/\s/g,
"");if(goog.isDefAndNotNull($scope["model"]["datasource"]["id_key"])&&goog.isDefAndNotNull($scope["model"]["datasource"]["label_key"]))$scope["model"]["datasource"]["attributs"]=$scope["model"]["datasource"]["id_key"]+"|"+$scope["model"]["datasource"]["label_key"];if(goog.isDefAndNotNull($scope["model"]["datasource"]["label_key"]))$scope["model"]["datasource"]["order_by"]=$scope["model"]["datasource"]["label_key"]};elementFormController["updateWebServiceAttributs"]=function(datasource){oVFB.log("updateWebServiceAttributs");
var params={"limit":1};if(goog.isObject(datasource["parameters"]))goog.object.extend(params,datasource["parameters"]);console.log(datasource["parameters"]);var canceler=$q.defer();ajaxRequest({"method":"GET","url":oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/"+datasource["ressource_id"],"headers":{"Accept":"application/x-vm-json"},"params":params,"scope":$scope,"timeout":5E3,"abord":canceler.promise,"success":function(response){if(!goog.isDefAndNotNull(response))return 0;
if(!goog.isDefAndNotNull(response["data"]))return 0;if(!goog.isDefAndNotNull(response["data"]["data"]))return 0;if(!goog.isDefAndNotNull(response["data"]["data"][0]))return 0;var data;if(datasource["dataType"]==="businessObject"&&goog.isDefAndNotNull(datasource["businessObjectRequest"])){data=[];for(var i=0;i<response["data"]["data"].length;i++)data.push(response["data"]["data"][i]["bo_"+datasource["businessObjectRequest"]])}else data=response["data"]["data"];$scope["aWebServiceAttributs"]=Object.keys(data[0])}})};
elementFormController["updateCascadeField"]=function(){oVFB.log("elementFormController.updateCascadeField");if(!goog.isDefAndNotNull($scope["aParents"])){console.error("aParents not defined");return 0}if(!goog.isDefAndNotNull($scope["model"]["datasource"])){console.error("datasource not defined");return 0}if($scope["cascade"]===false)$scope["aParents"]=[];if(goog.isArray($scope["aParents"])){if($scope["aParents"].length===0){if(goog.isDef($scope["model"]["datasource"]["parents"]))delete $scope["model"]["datasource"]["parents"];
return 0}if(!goog.isDefAndNotNull($scope["model"]["datasource"]["parents"]))$scope["model"]["datasource"]["parents"]=[];if(goog.isArray($scope["model"]["datasource"]["parents"]))goog.array.clear($scope["model"]["datasource"]["parents"]);for(var i=0;i<$scope["aParents"].length;i++)$scope["model"]["datasource"]["parents"].push({"name":$scope["aParents"][i],"filter_attr":$scope["aParents"][i],"filter_equality":"=","wait_for_parent":true})}};elementFormController.updateVmapData=function(tree){oVFB.log("elementFormController.updateVmapData");
if(tree["children"]){for(var i=0;i<tree["children"].length;i++)if(tree["children"][i]["view"]){$scope["model"]["map_options"]["center"]["coord"]=tree["children"][i]["view"]["center"]?tree["children"][i]["view"]["center"]:[0,0];$scope["model"]["map_options"]["center"]["zoom"]=tree["children"][i]["view"]["zoom"]?tree["children"][i]["view"]["zoom"]:5;$scope["model"]["map_options"]["center"]["extent"]=tree["children"][i]["view"]["extent"]?tree["children"][i]["view"]["extent"]:[-2.002637639E7,-2.00489661E7,
2.002637639E7,2.00489661E7];$scope["model"]["map_options"]["proj"]=tree["children"][i]["view"]["projection"]?tree["children"][i]["view"]["projection"]:"EPSG:3857";$rootScope.$broadcast("updateMap","view")}$scope["model"]["map_options"]["tree"]=tree;$rootScope.$broadcast("updateMap","tree")}};elementFormController.updateWS=function(){oVFB.log("elementFormController.updateWS");$rootScope.$broadcast("updateOptions",{})};elementFormController["updateMap"]=function(carte,option){oVFB.log("elementFormController.updateMap");
switch(option){case "center":var coord=ol.proj.transform($scope["model"]["map_options"]["center"]["coord"],$scope["model"]["map_options"]["proj"],"EPSG:3857");var boundary=[-2.002637639E7,-2.00489661E7,2.002637639E7,2.00489661E7];if(!ol.extent.containsCoordinate(boundary,coord))$["notify"](elementFormController["text"]["Component"]["Map"]["Notify"]["OutRange"],{"className":"error","autoHideDelay":oVFB.TimeNotify_});else $rootScope.$broadcast("updateMap",option);break;case "zoom":if($scope["model"]["map_options"]["center"]["scale"]>
0&&$scope["model"]["map_options"]["center"]["scale"]<=2E8)$rootScope.$broadcast("updateMap",option);else $["notify"](elementFormController["text"]["Component"]["Map"]["Notify"]["OutRange"],{"className":"error","autoHideDelay":oVFB.TimeNotify_});break;case "extent":var extent=$scope.model.map_options.center.extent;if(extent[0]<=extent[2]&&extent[0]>=-2.002637639E7&&extent[0]<=2.002637639E7)if(extent[0]<=extent[2]&&extent[2]>=-2.002637639E7&&extent[2]<=2.002637639E7)if(extent[1]<=extent[3]&&extent[1]>=
-2.00489661E7&&extent[1]<=2.00489661E7)if(extent[1]<=extent[3]&&extent[3]>=-2.00489661E7&&extent[3]<=2.00489661E7){$rootScope.$broadcast("updateMap",option);break}$["notify"](elementFormController["text"]["Component"]["Map"]["Notify"]["OutRange"],{"className":"error","autoHideDelay":oVFB.TimeNotify_});break;case "multi_geom":break;default:$rootScope.$broadcast("updateMap",option);break}};elementFormController.convert=function(src,dst){oVFB.log("elementFormController.convert");var fields;var icon=
"icon-point";if(src["type"]!=="undefined"){switch(dst){case "button":fields=["type","id","class","name","nb_cols","buttons","default_value"];break;case "title":case "label":fields=["type","id","label","name","default_value","nb_cols"];dst==="label"?icon="icon-eye":icon;break;case "hr":fields=["type","id","name","default_value"];dst==="hr"?icon="icon-minus":icon;break;case "hidden":fields=["type","id","name","default_value","nb_cols"];dst==="hidden"?icon="icon-user-secret":icon;break;case "subtitle":fields=
["type","id","label","name","class","default_value","nb_cols"];break;case "checkbox":fields=["type","id","label","name","checked","default_value","nb_cols"];icon="icon-check-square";break;case "radio":fields=["type","id","label","name","checked","options","nb_cols","default_value"];icon="icon-dot-circle-o";break;case "textarea":fields=["type","id","label","name","required","nb_cols","default_value"];icon="icon-paragraph";break;case "select":fields=["type","id","label","name","required","nb_cols",
"multiple","options","web_service","child_select","default_value"];icon="icon-format_list_bulleted";break;case "list":fields=["type","id","label","name","required","nb_cols","multiple","options","web_service","child_select","default_value","size"];icon="icon-format_list_bulleted";break;case "double_select":fields=["type","id","label","name","required","nb_cols","options","web_service","size","label_from","label_to","name_from","name_to","default_value"];icon="icon-format_list_bulleted";break;case "date":case "datetime":case "label_display":case "color_picker":fields=
["type","id","label","name","nb_cols","default_value"];dst==="date"||dst==="datetime"?icon="icon-calendar-check-o":icon;dst==="label_display"?icon="icon-eye":icon;dst==="color_picker"?icon="icon-paint":icon;break;case "upload":fields=["type","id","label","name","nb_cols","default_value","ext"];icon="icon-file";break;case "tinymce":fields=["type","id","label","name","nb_rows","options","default_value","nb_cols"];icon="icon-font";break;case "slider":fields=["type","id","label","name","options","default_value",
"nb_cols"];icon="icon-sliders";break;case "text":case "password":case "url":case "email":fields=["type","id","label","name","pattern","required","nb_cols","default_value"];dst==="text"?icon="icon-font":icon;dst==="password"?icon="icon-key":icon;dst==="url"?icon="icon-chrome":icon;dst==="email"?icon="icon-at":icon;break;case "integer":case "float":case "number":fields=["type","id","label","name","min","max","nb_cols","default_value"];icon="icon-calculator";break;case "link":fields=["type","id","label",
"name","alias","folder","nb_cols","target"];icon="glyphicon glyphicon-link";break;case "image":fields=["type","id","label","name","alias","folder","nb_cols","width"];icon="glyphicon glyphicon-picture";break;case "map_osm":case "map_bing":case "map_vmap":fields=["type","id","label","name","required","map_options","nb_cols","style","default_value"];icon="icon-map";break;case "codemirror":fields=["type","id","label","ui","name","cm_options","nb_cols"];icon="icon-code";break}for(var srckey in src)if(goog.isDefAndNotNull(fields))if(fields.indexOf(srckey)===
-1)delete src[srckey];switch(dst){case "list":case "double_select":var typic={"size":5};break;case "button":goog.isDef(src["class"])?src["class"]:src["class"]="btn-ungroup btn-group-sm";goog.isDef(src["buttons"])?src["buttons"]:src["buttons"]=[];var typic={"type":"submit","name":"form_submit","label":"FORM_UPDATE","class":"btn-primary"};goog.isDef(src["buttons"][0])?src["buttons"][0]:src["buttons"][0]=typic;break;case "radio":var typic={"choices":[{label:"Oui",value:true},{label:"Non",value:false}]};
goog.isDef(src["options"])?src["options"]:src["options"]=typic;goog.isDef(src["nb_cols"])?src["nb_cols"]:src["nb_cols"]=12;break;case "slider":var typic={"min":0,"max":100,"precision":1,"step":1};goog.isDef(src["options"])?src["options"]:src["options"]=typic;goog.isDef(src["nb_cols"])?src["nb_cols"]:src["nb_cols"]=12;break;case "map_osm":goog.isDef(src["style"])?src["style"]:src["style"]={"height":"250px"};var typic={"proj":"EPSG:3857","type":"OSM","output_format":"ewkt","center":{"coord":[3E5,6E6],
"zoom":5,"scale":25E6,"extent":[-2.002637639E7,-2.00489661E7,2.002637639E7,2.00489661E7]},"controls":{"MP":true,"ZO":true,"SL":true,"CP":true},"layers":[],"interactions":{"multi_geometry":false,"full_screen":true,"RA":false,"RO":true,"ED":true,"DP":true,"DL":true,"DPol":true,"SE":true},"draw_color":"rgba(54,184,255,0.6)","contour_color":"rgba(0,0,0,0.4)","contour_size":2,"circle_radius":6,"features":[],"coord_accuracy":8};goog.isDef(src["map_options"])&&src["map_options"]["type"]==="OSM"?src["map_options"]:
src["map_options"]=typic;break;case "map_bing":goog.isDef(src["style"])?src["style"]:src["style"]={"height":"250px"};var typic={"proj":"EPSG:3857","type":"bing","output_format":"ewkt","center":{"coord":[3E5,6E6],"zoom":5,"scale":25E6,"extent":[-2.002637639E7,-2.00489661E7,2.002637639E7,2.00489661E7]},"controls":{"MP":true,"ZO":true,"SL":true,"CP":true},"layers":[],"interactions":{"multi_geometry":false,"full_screen":true,"RA":false,"RO":true,"ED":true,"DP":true,"DL":true,"DPol":true,"SE":true},"draw_color":"rgba(54,184,255,0.6)",
"contour_color":"rgba(0,0,0,0.4)","contour_size":2,"circle_radius":6,"features":[],"coord_accuracy":8,"source":{"culture":"fr-FR","key":"","style":"Aerial"}};goog.isDef(src["map_options"])&&src["map_options"]["type"]==="bing"?src["map_options"]:src["map_options"]=typic;break;case "map_vmap":goog.isDef(src["style"])?src["style"]:src["style"]={"height":"250px"};var typic={"proj":"EPSG:3857","type":"vmap","output_format":"ewkt","center":{"coord":[3E5,6E6],"zoom":5,"scale":25E6,"extent":[-2.002637639E7,
-2.00489661E7,2.002637639E7,2.00489661E7]},"controls":{"MP":true,"ZO":true,"SL":true,"CP":true},"layers":[],"interactions":{"multi_geometry":false,"full_screen":true,"RA":false,"RO":true,"ED":true,"DP":true,"DL":true,"DPol":true,"SE":true},"draw_color":"rgba(54,184,255,0.6)","contour_color":"rgba(0,0,0,0.4)","contour_size":2,"circle_radius":6,"features":[],"coord_accuracy":8};goog.isDef(src["map_options"])&&src["map_options"]["type"]==="vmap"?src["map_options"]:src["map_options"]=typic;break;case "codemirror":var typic=
{"mode":"javascript","readOnly":false,"styleActiveLine":true,"lineNumbers":true,"lineSeparator":"\r\n","indentWithTabs":true,"tabSize":2,"viewportMargin":50,"theme":"Veremes"};goog.isDef(src["cm_options"])?src["cm_options"]:src["cm_options"]=typic;goog.isDef(src["nb_cols"])?src["nb_cols"]:src["nb_cols"]=12;goog.isDef(src["ui"])?src["ui"]:src["ui"]=true;break;case "bo_grid":var typic={"enableRowSelection":true,"enableSelectAll":true,"enablePagination":false,"enablePaginationControls":false,"enableColumnMenus":false,
"enableColumnResizing":false,"appHeader":true,"appHeaderTitleBar":true,"columnDefs":[],"data":[]};goog.isDef(src["options"])?src["options"]:src["options"]=typic;goog.isDef(src["nb_cols"])?src["nb_cols"]:src["nb_cols"]=12;setTimeout(function(){oVFB.reloadForm()});break;case "section_grid":var typic={"enableRowSelection":true,"enableSelectAll":true,"enablePagination":false,"enablePaginationControls":false,"enableColumnMenus":false,"enableColumnResizing":false,"appHeader":true,"appHeaderTitleBar":true,
"columnDefs":[],"data":[]};goog.isDef(src["options"])?src["options"]:src["options"]=typic;goog.isDef(src["nb_cols"])?src["nb_cols"]:src["nb_cols"]=12;setTimeout(function(){oVFB.reloadForm()});break;case "ui_grid":var typic={"enableRowSelection":true,"enableSelectAll":true,"enablePagination":false,"enablePaginationControls":false,"enableColumnMenus":false,"enableColumnResizing":false,"appHeader":true,"appHeaderTitleBar":true,"appGridTitle":"Titre","appEnableDragAndDrop":false,"columnDefs":[],"data":[],
"appActions":[],"datasource":{}};goog.isDef(src["options"])?src["options"]:src["options"]=typic;goog.isDef(src["nb_cols"])?src["nb_cols"]:src["nb_cols"]=12;angular.element("#studio_form_reader > form").scope()["gridOptions"]=src["options"];setTimeout(function(){oVFB.reloadForm()});break;case "treeview":var typic={"showBorder":false,"levels":0,"showCheckbox":true,"expandIcon":"glyphicon glyphicon-folder-close","collapseIcon":"glyphicon glyphicon-folder-open","highlightSelected":false,"multiSelect":true,
"data":[],"dataLoadingEvent":""};goog.isDef(src["options"])?src["options"]:src["options"]=typic;goog.isDef(src["nb_cols"])?src["nb_cols"]:src["nb_cols"]=12;setTimeout(function(){oVFB.reloadForm()});break;default:if(!src["nb_cols"])src["nb_cols"]=12;break}return icon}};elementFormController["resize"]=function(){$scope["collapsed"]=!$scope["collapsed"];oVFB.Collapse_[4]=$scope["collapsed"];oVFB.resize()};elementFormController["updateBoList"]=function(){$scope["aBusinessObjects"]=[];var canceler=$q.defer();
ajaxRequest({"method":"GET","url":oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/vmap/businessobjects","headers":{"Accept":"application/x-vm-json"},"scope":$scope,"timeout":5E3,"abord":canceler.promise,"success":function(response){if(!goog.isDefAndNotNull(response))return 0;if(!goog.isDefAndNotNull(response["data"]))return 0;if(!goog.isDefAndNotNull(response["data"]["data"]))return 0;if(!goog.isDefAndNotNull(response["data"]["data"][0]))return 0;$scope["aBusinessObjects"]=
response["data"]["data"]}})};elementFormController["updateBoLinkLists"]=function(sBoId){if(!goog.isDefAndNotNull(sBoId))return 0;var aBoFields=[];var aFormFields=[];var canceler=$q.defer();ajaxRequest({"method":"GET","url":oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/vmap/querys/"+sBoId,"headers":{"Accept":"application/x-vm-json"},"scope":$scope,"timeout":5E3,"abord":canceler.promise,"success":function(response){if(!goog.isDefAndNotNull(response))return 0;if(!goog.isDefAndNotNull(response["data"]))return 0;
if(!goog.isDefAndNotNull(response["data"]["data"]))return 0;if(!goog.isDefAndNotNull(response["data"]["data"][0]))return 0;if(!goog.isDefAndNotNull(response["data"]["data"][0]))return 0;var aBoFields=Object.keys(response["data"]["data"][0]);$scope["aBoAttributs"]=aBoFields}});if(typeof oVFB.getJsonOutput()[$rootScope["selected_form_type"]]!=="undefined"){var obj=oVFB.getJsonOutput()[$rootScope["selected_form_type"]]["rows"];for(var i=0;i<obj.length;i++)for(var j=0;j<obj[i]["fields"].length;j++)aFormFields.push(obj[i]["fields"][j]["name"])}$scope["aFormAttributs"]=
aFormFields};elementFormController["updateSectionList"]=function(){$scope["aBusinessObjects"]=[];var canceler=$q.defer();ajaxRequest({"method":"GET","url":oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/vitis/vitissections","headers":{"Accept":"application/x-vm-json"},"params":{"filter":{"relation":"AND","operators":[{"column":"template","compare_operator":"=","value":"workspaceListTpl.html"},{"column":"lang","compare_operator":"=","value":oVFB.AppProperties.language}]}},
"scope":$scope,"timeout":5E3,"abord":canceler.promise,"success":function(response){if(!goog.isDefAndNotNull(response))return 0;if(!goog.isDefAndNotNull(response["data"]))return 0;if(!goog.isDefAndNotNull(response["data"]["data"]))return 0;if(!goog.isDefAndNotNull(response["data"]["data"][0]))return 0;$scope["aSections"]=response["data"]["data"]}})};elementFormController["updateSectionLinkLists"]=function(sSectionId){$scope["aBusinessObjects"]=[];var canceler=$q.defer();ajaxRequest({"method":"GET",
"url":oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/vitis/vitissections/"+sSectionId,"headers":{"Accept":"application/x-vm-json"},"scope":$scope,"timeout":5E3,"abord":canceler.promise,"success":function(response){if(!goog.isDefAndNotNull(response))return 0;if(!goog.isDefAndNotNull(response["data"]))return 0;if(!goog.isDefAndNotNull(response["data"]["data"]))return 0;if(!goog.isDefAndNotNull(response["data"]["data"][0]))return 0;var oSection=response["data"]["data"][0];
if(goog.isDefAndNotNull(oSection["ressource_id"]))ajaxRequest({"method":"GET","url":oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/"+oSection["ressource_id"],"headers":{"Accept":"application/x-vm-json"},"scope":$scope,"timeout":5E3,"abord":canceler.promise,"success":function(response){if(!goog.isDefAndNotNull(response))return 0;if(!goog.isDefAndNotNull(response["data"]))return 0;if(!goog.isDefAndNotNull(response["data"]["data"]))return 0;if(!goog.isDefAndNotNull(response["data"]["data"][0]))return 0;
var aSectionAttributes=Object.keys(response["data"]["data"][0]);$scope["aSectionAttributs"]=aSectionAttributes}})}});var aFormFields=[];if(typeof oVFB.getJsonOutput()[$rootScope["selected_form_type"]]!=="undefined"){var obj=oVFB.getJsonOutput()[$rootScope["selected_form_type"]]["rows"];for(var i=0;i<obj.length;i++)for(var j=0;j<obj[i]["fields"].length;j++)aFormFields.push(obj[i]["fields"][j]["name"])}$scope["aFormAttributs"]=aFormFields}};goog.provide("oVFB");goog.require("nsUtils");goog.require("nsVFB.properties");goog.require("nsVFB.JsonLoader");goog.require("nsVFB.FormTools");goog.require("nsVFB.Visualizer");goog.require("nsVFB.ElementForm");goog.require("nsVitisComponent.Map");var oVFB={};oVFB["config"]={};oVFB.init=function(config){console.log("oVFB.init: ",config);oVFB["config"]=config};oVFB.resize=function(bFullScreen){};oVFB.getUpdate=function(){return oVFB.Update};oVFB.setUpdate=function(Update){oVFB.Update=Update};
oVFB.getDevTools=function(){return oVFB.oDevTools_};oVFB.getTreeView=function(){return oVFB.oTreeView_};oVFB.getFormTools=function(){return oVFB.oFormTools_};oVFB.getJsonLoader=function(){return oVFB.oJsonLoader_};oVFB.getElementForm=function(){return oVFB.oElementForm_};oVFB.getVisualizer=function(){return oVFB.oVisualizer_};oVFB.getJson=function(){return oVFB.oJson_};oVFB.setJson=function(oJson){oVFB.oJson_=oJson};oVFB.getJs=function(){return oVFB.sJs_};oVFB.setJs=function(sJs){oVFB.sJs_=sJs};
oVFB.getCss=function(){return oVFB.sCSS_};oVFB.setCss=function(sCss){oVFB.sCSS_=sCss};oVFB.getJsonOutput=function(){if(!goog.isDef(oVFB.oJsonOutput_))console.error("oVFB.oJsonOutput_ undefined");if(goog.object.isEmpty(oVFB.oJsonOutput_))console.error("oVFB.oJsonOutput_ is empty");return oVFB.oJsonOutput_};oVFB.setJsonOutput=function(oJson){oVFB.oJsonOutput_=oJson};
...
oVFB.cleanFormToSave=function(oJson){if(oVFB.getApplication()!=="wab")return oJson;var oForm;for(var sFormName in oJson){oForm=oJson[sFormName];if(goog.isDefAndNotNull(oForm["rows"]))for(var i=0;i<oForm["rows"].length;i++)if(goog.isDefAndNotNull(oForm["rows"][i]["fields"]))for(var ii=0;ii<oForm["rows"][i]["fields"].length;ii++){if(goog.isDefAndNotNull(oForm["rows"][i]["fields"][ii]["selected"]))delete oForm["rows"][i]["fields"][ii]["selected"];if(goog.isDefAndNotNull(oForm["rows"][i]["fields"][ii]["disabled"]))delete oForm["rows"][i]["fields"][ii]["disabled"];
if(goog.isDefAndNotNull(oForm["rows"][i]["fields"][ii]["visible"]))delete oForm["rows"][i]["fields"][ii]["visible"]}}return oJson};
...
oVFB.checkElemsOnWab=function(oWab,aElems,aStates,aGroups){for(var i=0;i<aStates.length;i++)if(!goog.isDefAndNotNull(oWab[aStates[i]]))oWab[aStates[i]]={};for(var state in oWab)if(aStates.indexOf(state)===-1)delete oWab[state];else{for(var i=0;i<aGroups.length;i++)if(!goog.isDefAndNotNull(oWab[state][aGroups[i]]))oWab[state][aGroups[i]]={};for(var group in oWab[state])if(aGroups.indexOf(group)===-1)delete oWab[state][group];else{for(var sWabElem in oWab[state][group])if(aElems.indexOf(sWabElem)===
-1)delete oWab[state][group][sWabElem];for(var i=0;i<aElems.length;i++)if(!goog.isDefAndNotNull(oWab[state][group][aElems[i]]))oWab[state][group][aElems[i]]=""}}return oWab};oVFB.cleanTabs=function(oTabs){var oTab;if(goog.isDefAndNotNull(oTabs["list"]))for(var i=0;i<oTabs["list"].length;i++){oTab=oTabs["list"][i];if(goog.isDefAndNotNull(oTab["$$hashKey"]))delete oTab["$$hashKey"];if(goog.isDefAndNotNull(oTab["studio_mode"]))delete oTab["studio_mode"]}return oTabs};oVFB.getTreeDnD=function(){return oVFB.tree_};
oVFB.setTreeDnD=function(str){oVFB.tree_=str};oVFB.getToken=function(){return oVFB.token};oVFB["setToken"]=function(token){oVFB.token=token};oVFB.getId=function(){return oVFB.id};oVFB.getApplication=function(){return oVFB.AppName};oVFB["setId"]=function(id){oVFB.id=id};oVFB["setContainer"]=function(str){oVFB.el=str};oVFB["setAppProperties"]=function(obj){oVFB.AppProperties=obj};oVFB["setApplication"]=function(sApp){oVFB.AppName=sApp};oVFB["reset"]=function(){};
oVFB.module=angular.module("vfb",["ui.codemirror","ui.grid","ui.grid.autoResize","formReader"]);oVFB.studioMainDirective=function(){oVFB.log("nsVFB.MainDirective");if(!goog.isDefAndNotNull(oVFB["config"]))console.error("oVFB['config'] not defined, please use oVFB.init(config)");return{restrict:"A",replace:true,controller:"AppStudioMainController",controllerAs:"ctrl",scope:true,bindToController:true,templateUrl:"javascript/externs/studio/templates/MainTemplate.html"}};
oVFB.module.directive("appStudioMainDirective",oVFB.studioMainDirective);
oVFB.studioMainController=function($scope,$q,$timeout,$rootScope){oVFB.log("nsVFB.MainController");var this_=this;this.$scope_=$scope;this.$rootScope_=$rootScope;this.$q_=$q;this.$timeout_=$timeout;this.iLocalDatabaseChangeCounter_=0;this["text"]={};$scope["aCollapsed"]=oVFB.Collapse_;$scope["text"]={};$scope["jsonOutput"]={};$scope["datasources"]={};$scope["selectedDatasource"]={};$scope["webservices"]=[];$scope["businessObjects"]=[];$scope["ressources"]=[];$scope["aExternalDatabaseTables"]=[];$scope["schemas"]=
[];$scope["databases"]=[];$scope["tables"]=[];$scope["selectedDatasource"]={};$scope["editedDatasource"]={};$scope["newSource"]={"options":["Value one|1","Value two|2","Value three|3"],"result_options":[{"label":"Value one","value":"1"},{"label":"Value two","value":"2"},{"label":"Value three","value":"3"}]};$scope["wabVisualizer"]=false;$scope["editorOptions"]={"mode":"schema","lineNumbers":false,"lineSeparator":"\n","theme":"Veremes","onLoad":function(cm_){}};$scope["studioAddModalTestGridOptions"]=
{"data":[],"enableColumnMenus":false,"enableColumnResizing":true,"onRegisterApi":function(studioAddModalTestGridApi){$scope.studioAddModalTestGridApi=studioAddModalTestGridApi}};$scope["studioEditModalTestGridOptions"]={"data":[],"enableColumnMenus":false,"enableColumnResizing":true,"onRegisterApi":function(studioEditModalTestGridApi){$scope.studioEditModalTestGridApi=studioEditModalTestGridApi}};$scope["studioDatasourcesGridOptions"]={"data":[],"columnDefs":[{"field":"name","displayName":"Nom"}],
"enableColumnMenus":false,"enableColumnResizing":true,"enableRowSelection":true,"multiSelect":false,"enableFullRowSelection":true,"onRegisterApi":function(datasourcesGridApi){$scope["datasourcesGridApi"]=datasourcesGridApi;datasourcesGridApi["selection"]["on"]["rowSelectionChanged"]($scope,function(row){$scope["selectedDatasource"]=angular.copy($scope["datasources"][row["entity"]["id"]]);$scope["selectedDatasource"]["id"]=row["entity"]["id"];this_.setDatasourceContent($scope["selectedDatasource"])})}};
$scope["tabs"]={};$scope["elems"]=[];$scope["sTestTabFormDefinitionName"]="";$scope["oTestTabFormValues"]={};$scope["oTestTabFormDefinition"]={};$scope["oTestTabFormScope"]={};$scope["sTestWabFormDefinitionName"]="";$scope["oTestWabFormValues"]={};$scope["oTestWabFormDefinition"]={};$scope["oTestWabFormScope"]={};$scope["oProperties"]=oVFB.AppProperties;$scope["sToken"]=sessionStorage["session_token"];$scope["wab"]={};$scope["wabGroups"]=[];$scope["wabStates"]=[];$scope["module"]="";$scope["object"]=
"";$scope["wabSelectedGroup"]="";$scope["wabSelectedState"]="";$scope["applicationName"]=oVFB.getApplication();ajaxRequest({"method":"GET","url":"javascript/externs/studio/lang/lang-"+oVFB.AppProperties["language"]+".json","scope":this.$scope_,"success":function(response){$scope["text"]=response["data"];this_["text"]=response["data"]},"error":function(response){console["error"]("ERROR : (AJAX request : "+status+" ) On loading Language, contact Veremes please")}});ajaxRequest({"method":"GET","url":oVFB.AppProperties["web_server_name"]+
"/"+oVFB.AppProperties["services_alias"]+"/vitis/webservices","headers":{"Accept":"application/x-vm-json"},"params":{"attributs":"name"},"scope":this.$scope_,"success":function(response){if(!goog.isDefAndNotNull(response.data["data"])){console.error("response.data['data'] not defined");return}$scope["webservices"]=response.data["data"]}});if($scope["applicationName"]==="vmap")ajaxRequest({"method":"GET","url":oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/vmap/businessobjects",
"headers":{"Accept":"application/x-vm-json"},"scope":this.$scope_,"params":{"order_by":"title"},"success":function(response){if(!goog.isDefAndNotNull(response.data["data"])){console.error("response.data['data'] not defined");return}var aBusinessObjects=response.data["data"];for(var i=0;i<aBusinessObjects.length;i++)aBusinessObjects[i]["sql_form"]="SELECT * FROM "+aBusinessObjects[i]["schema"]+"."+aBusinessObjects[i]["table"];$scope["businessObjects"]=[];for(var i=0;i<aBusinessObjects.length;i++)$scope["businessObjects"].push({"title":aBusinessObjects[i]["title"],
"business_object_id":aBusinessObjects[i]["business_object_id"],"sql_form":aBusinessObjects[i]["sql_form"],"sql_summary":aBusinessObjects[i]["sql_summary"],"sql_list":aBusinessObjects[i]["sql_list"]})}});$("#studio-datasource-modal").on("hide.bs.modal",function(e){oVFB.unselectAll();this_.clearDatasourcesGrid();oVFB.reloadForm()});$("#studio-datasource-add-modal").on("hide.bs.modal",function(e){$scope["aExternalDatabaseTables"].length=0;$scope["schemas"].length=0;$scope["tables"].length=0;this_.clearTestGrids()});
$("#studio-datasource-edit-modal").on("hide.bs.modal",function(e){$scope["aExternalDatabaseTables"].length=0;$scope["schemas"].length=0;$scope["tables"].length=0;this_.clearTestGrids()});$scope.$on("updateForm",function(event,data){$scope.$applyAsync(function(){$scope["jsonOutput"]=data})});$scope.$on("updateTestTabForm",function(event,data){if(data[$scope["selected_form_type"]]){var oJson=data[$scope["selected_form_type"]];for(var i=0;i<oJson["rows"].length;i++)for(var j=0;j<oJson["rows"][i]["fields"].length;j++)if(!oJson["rows"][i]["fields"][j]["id"])oJson["rows"][i]["fields"][j]["id"]=
oJson["rows"][i]["fields"][j]["name"]+"_"+(i+1)+"_"+(j+1);$scope["oTestTabFormDefinition"]=data;$scope["sTestTabFormDefinitionName"]=$scope["selected_form_type"];$scope["title"]=oJson["title"];if(!goog.isDef($scope["oTestTabFormValues"]))$scope["oTestTabFormValues"]={};if(!goog.isDef($scope["oTestTabFormValues"][$scope["sTestTabFormDefinitionName"]]))$scope["oTestTabFormValues"][$scope["sTestTabFormDefinitionName"]]={};angular.element($("#studio_test_tabs_form_reader").children()).scope()["ctrl"].refreshForm()}else oVFB.log("visualizerFormModeController. Load JSON form fail: "+
$scope["selected_form_type"]+" undefined in ",data)});$scope.$on("loadWABGroups",function(event,data){$scope["module"]=data["module"];$scope["object"]=data["object"];this_.loadWabGoups($scope["module"])});$scope.$on("updateTestWabForm",function(event,data){if(data[$scope["selected_form_type"]]){var oJson=data[$scope["selected_form_type"]];for(var i=0;i<oJson["rows"].length;i++)for(var j=0;j<oJson["rows"][i]["fields"].length;j++)if(!oJson["rows"][i]["fields"][j]["id"])oJson["rows"][i]["fields"][j]["id"]=
oJson["rows"][i]["fields"][j]["name"]+"_"+(i+1)+"_"+(j+1);$scope["oTestWabFormDefinition"]=data;$scope["sTestWabFormDefinitionName"]=$scope["selected_form_type"];$scope["title"]=oJson["title"];if(!goog.isDef($scope["oTestWabFormValues"]))$scope["oTestWabFormValues"]={};if(!goog.isDef($scope["oTestWabFormValues"][$scope["sTestWabFormDefinitionName"]]))$scope["oTestWabFormValues"][$scope["sTestWabFormDefinitionName"]]={};var oFormReaderScope=angular.element($("#studio_test_wab_form_reader").children()).scope();
if(goog.isDefAndNotNull(oFormReaderScope))oFormReaderScope.$applyAsync(function(){oFormReaderScope["wabState"]=$scope["wabSelectedState"];oFormReaderScope["wabGroup"]=$scope["wabSelectedGroup"];oFormReaderScope["ctrl"].refreshForm()})}else oVFB.log("visualizerFormModeController. Load JSON form fail: "+$scope["selected_form_type"]+" undefined in ",data)});$scope.$watch("tabs",function(){this_.updateTestTabForm()},true);$scope.$watch("wabSelectedGroup",function(){if(goog.isString($scope["wabSelectedState"])&&
goog.isString($scope["wabSelectedGroup"])&&$scope["wabSelectedGroup"]!==""&&$scope["wabSelectedState"]!=="")this_.updateTestWabForm()});$scope.$watch("wabSelectedState",function(){if(goog.isString($scope["wabSelectedState"])&&goog.isString($scope["wabSelectedGroup"])&&$scope["wabSelectedGroup"]!==""&&$scope["wabSelectedState"]!=="")this_.updateTestWabForm()})};oVFB.module.controller("AppStudioMainController",["$scope","$q",oVFB.studioMainController]);
oVFB.module.filter("sortStates",function(){return function(input,sortBy){var sortedOutput=[];for(var key in sortBy){input[sortBy[key]]["name"]=sortBy[key];sortedOutput.push(input[sortBy[key]])}return sortedOutput}});
oVFB.studioMainController.prototype.showTabsManagerModal=function(){oVFB.log("studioMainController.showTabsManagerModal");oVFB.Update=true;var $scope=this.$scope_;var sFormDefinitionName=$scope["selected_form_type"];var oElems=oVFB.getFormElems($scope["jsonOutput"][sFormDefinitionName]);oVFB.unselectAll();$scope["jsonOutput"]=oVFB.getJsonOutput();$scope["tabs"]=angular.copy($scope["jsonOutput"][sFormDefinitionName]["tabs"]);$scope["elems"]=oElems["elems"];$scope["buttons"]=oElems["buttons"];$scope["tabs"]=
oVFB.checkElemsOnTabs($scope["tabs"],$scope["elems"]);this.updateTestTabForm();$("#studio-tabsmanager-modal").modal("show")};oVFB.studioMainController.prototype.toggleElemOnTab=function(sElem,oTab){if(oTab.elements.indexOf(sElem)!==-1)oTab.elements.splice(oTab.elements.indexOf(sElem),1);else oTab.elements.push(sElem)};
oVFB.studioMainController.prototype.generateWabMethod=function(){var aGrp={};var oJson=oVFB.getJsonOutput();var sLabelKey=oJson["update"]["title"].replace("_TITLE","_FORM_");var oLangAddons={};for(var i=0;i<this.$scope_["aBoMethods"].length;i++)if(goog.isDefAndNotNull(this.$scope_["aBoMethods"][i]["selected"]))if(this.$scope_["aBoMethods"][i]["selected"]){if(!goog.isDefAndNotNull(aGrp[this.$scope_["aBoMethods"][i]["group"]]))aGrp[this.$scope_["aBoMethods"][i]["group"]]={"type":"button","nb_cols":12,
"class":"btn-ungroup btn-group-sm","name":this.$scope_["aBoMethods"][i]["group"],"id":this.$scope_["aBoMethods"][i]["group"],"buttons":[]};var sLabelKeyMethod=sLabelKey+this.$scope_["aBoMethods"][i]["name"].toUpperCase();aGrp[this.$scope_["aBoMethods"][i]["group"]]["buttons"].push({"type":"button","name":"form_submit","label":sLabelKeyMethod,"class":"btn-primary","event":"callBoMethod('"+this.$scope_["aBoMethods"][i]["name"]+"')"});oLangAddons[sLabelKeyMethod]=this.$scope_["aBoMethods"][i]["label"]}console.log(oLangAddons);
for(var sKey in aGrp)oJson["update"]["rows"].push({"fields":[aGrp[sKey]]});oVFB.setJsonOutput(oJson);this.$scope_.$broadcast("updateForm",oJson);this.saveWabModuleLangJsonAdd(oLangAddons);$("#studio-wabmanager-method-modal").modal("hide")};
oVFB.studioMainController.prototype.saveWabModuleLangJsonAdd=function(oLangAddon){var $scope=this.$scope_;ajaxRequest({"method":"PUT","url":oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/wab/modules/"+$scope["module"],"headers":{"Accept":"application/x-vm-json"},"data":{"sLangAddon":oLangAddon},"scope":$scope,"success":function(response){}})};
oVFB.studioMainController.prototype.loadWabGoups=function(sModuleName){var $scope=this.$scope_;var $this=this;ajaxRequest({"method":"POST","url":oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/wab/groups","headers":{"Accept":"application/x-vm-json","X-HTTP-Method-Override":"GET"},"scope":$scope,"data":{"order_by":"name","distinct":true,"filter":{"column":"module_id","compare_operator":"=","value":sModuleName}},"success":function(response){if(!goog.isDefAndNotNull(response.data["data"])){console.error("response.data['data'] not defined");
return}$scope["wabGroups"]=[];for(var i=0;i<response.data["data"].length;i++)$scope["wabGroups"].push(response.data["data"][i]["name"]);$this.loadWabStates($scope["module"],$scope["object"])}})};
oVFB.studioMainController.prototype.loadWabStates=function(sModuleName,sObjectName){var $scope=this.$scope_;var $this=this;ajaxRequest({"method":"POST","url":oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/wab/states","headers":{"Accept":"application/x-vm-json","X-HTTP-Method-Override":"GET"},"scope":$scope,"data":{"order_by":"progress_lvl","sort_order":"ASC","distinct":true,"filter":{"relation":"AND","operators":[{"column":"module_id","compare_operator":"=","value":sModuleName},
{"column":"bo_name","compare_operator":"=","value":sObjectName}]}},"success":function(response){if(!goog.isDefAndNotNull(response.data["data"])){console.error("response.data['data'] not defined");return}$scope["wabStates"]=[];for(var i=0;i<response.data["data"].length;i++)$scope["wabStates"].push(response.data["data"][i]["name"]);$this.createEmptyWabObject()}})};
oVFB.studioMainController.prototype.createEmptyWabObject=function(){var $scope=this.$scope_;for(var i=0;i<$scope["wabStates"].length;i++){$scope["wab"][$scope["wabStates"][i]]={};for(var j=0;j<$scope["wabGroups"].length;j++)$scope["wab"][$scope["wabStates"][i]][$scope["wabGroups"][j]]={}}};oVFB.studioMainController.prototype.addTab=function(oTab){var $scope=this.$scope_;if(!goog.isDefAndNotNull(oTab))oTab={"label":$scope["text"]["Loader"]["Tab"],"elements":[]};$scope.$applyAsync(function(){$scope["tabs"]["list"].push(oTab)})};
...
oVFB.studioMainController.prototype.showWabManagerModal=function(){oVFB.log("studioMainController.showWabManagerModal");var $scope=this.$scope_;var sFormDefinitionName=$scope["selected_form_type"];var oElems=oVFB.getFormElems($scope["jsonOutput"][sFormDefinitionName]);oVFB.unselectAll();$scope["jsonOutput"]=oVFB.getJsonOutput();if(goog.isDefAndNotNull($scope["jsonOutput"][sFormDefinitionName]["wab"]))$scope["wab"]=angular.copy($scope["jsonOutput"][sFormDefinitionName]["wab"]);$scope["elems"]=oElems["elems"];
$scope["buttons"]=oElems["buttons"];$scope["wab"]=oVFB.checkElemsOnWab($scope["wab"],$scope["elems"],$scope["wabStates"],$scope["wabGroups"]);$("#studio-wabmanager-modal").modal("show")};
...
"rw";else sNextValue="r";oState[sWabSelectedGroup][sElem]=sNextValue}else oState[sWabSelectedGroup][sElem]=sAction;this.updateTestWabForm()}};
oVFB.studioMainController.prototype.saveWab=function(oWab){var $scope=this.$scope_;var sFormDefinitionName=$scope["selected_form_type"];var oFinalWab={};$("#studio-wabmanager-modal").modal("hide");for(var sState in oWab){oFinalWab[sState]={};for(var sGrp in oWab[sState])if(sGrp!=="name")oFinalWab[sState][sGrp]=oWab[sState][sGrp]}console.log(oFinalWab);$scope["jsonOutput"][sFormDefinitionName]["wab"]=oFinalWab;oVFB.setJsonOutput($scope["jsonOutput"]);$scope.$broadcast("updateForm",$scope["jsonOutput"])};
oVFB.studioMainController.prototype.updateTestWabForm=function(){var $scope=this.$scope_;var oForm=oVFB.getUnselectedForm($scope["jsonOutput"]);var sFormDefinitionName=$scope["selected_form_type"];if(goog.isDefAndNotNull(oForm))if(goog.isDefAndNotNull(oForm[sFormDefinitionName])){oForm[sFormDefinitionName]["wab"]=angular.copy($scope["wab"]);$scope.$broadcast("updateTestWabForm",oForm)}};
oVFB.studioMainController.prototype.showDatasourcesModal=function(){oVFB.Update=true;oVFB.log("studioMainController.showDatasourcesModal");var $scope=this.$scope_;$scope["jsonOutput"]=oVFB.getJsonOutput();if(!goog.isDef($scope["jsonOutput"]["datasources"])||!goog.isObject($scope["jsonOutput"]["datasources"])||goog.isArray($scope["jsonOutput"]["datasources"]))$scope["jsonOutput"]["datasources"]={};$scope["datasources"]=$scope["jsonOutput"]["datasources"];oVFB.unselectAll();$("#studio-datasource-modal").modal("show");
$scope["selectedDatasource"]={};this.setStudioGridDatasources();var $timeout=angular.element(vitisApp.appMainDrtv).injector().get(["$timeout"]);$timeout(function(){if($scope["datasourcesGridApi"]["grid"]["rows"].length>0&&$scope["datasourcesGridApi"]["selection"]["getSelectedRows"]().length===0)$scope["datasourcesGridApi"]["selection"]["selectRow"]($scope["studioDatasourcesGridOptions"]["data"][0])},1)};
...
oVFB.studioMainController.prototype.setLocalDatabases=function(){oVFB.log("studioMainController.setLocalDatabases");var $scope=this.$scope_;var this_=this;ajaxRequest({"method":"GET","url":oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/vitis/genericquerys/databases","headers":{"Accept":"application/x-vm-json"},"scope":this.$scope_,"success":function(response){if(!goog.isDefAndNotNull(response.data["data"])){$.notify(this_["text"]["Element"]["Component"]["Select"]["Database_List_Error"],
"error");$scope["databases"]=[];return}$scope["databases"]=response.data["data"]}})};
oVFB.studioMainController.prototype.setLocalDatabaseSchemas=function(database){oVFB.log("studioMainController.setLocalDatabaseSchemas");var $scope=this.$scope_;var this_=this;if(!goog.isDefAndNotNull(database))database=oVFB.AppProperties["database"];ajaxRequest({"method":"GET","url":oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/vitis/genericquerys/"+database+"/schemas","headers":{"Accept":"application/x-vm-json"},"scope":this.$scope_,"success":function(response){if(!goog.isDefAndNotNull(response.data["data"])){$.notify(this_["text"]["Element"]["Component"]["Select"]["Schema_List_error"],
"error");$scope["schemas"]=[];return}$scope["schemas"]=response.data["data"]}})};
oVFB.studioMainController.prototype.reloadTablesList=function(sSchema,sDatabase){oVFB.log("studioMainController.reloadTablesList");var $scope=this.$scope_;var this_=this;ajaxRequest({"method":"GET","url":oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/vitis/genericquerys","headers":{"Accept":"application/x-vm-json"},"params":{"database":sDatabase,"schema":sSchema},"scope":this.$scope_,"success":function(response){if(!goog.isDefAndNotNull(response.data["data"])){$.notify(this_["text"]["Element"]["Component"]["Select"]["Table_List_error"],
"error");$scope["tables"]=[];return}$scope["tables"]=response.data["data"]}})};
...
"value":"2"},{"label":"Value three","value":"3"}]};$scope["newSource"]=this.loadDatasourceDefaultValues($scope["newSource"]);$scope["aExternalDatabaseTables"].length=0;$scope["schemas"].length=0;$scope["tables"].length=0;$("#studio-datasource-add-modal").modal("show");$scope["aExternalDatabaseTablesSelected"]={};$scope["databaseSelected"]={};$scope["schemaSelected"]={};$scope["tableSelected"]={};$("#typeSourceTooltip")["popover"]({"trigger":"hover","container":"body","content":function(){var sContent=
$scope["text"]["Element"]["Component"]["Select"]["TypeSourceContent"];return sContent},"placement":function(oPopoverNode,oElementNode){return $scope.$root["workspaceTooltipPlacement"](oElementNode)}})};
oVFB.studioMainController.prototype.showEditSourceModal=function(){oVFB.log("studioMainController.showEditSourceModal");var $scope=this.$scope_;if($scope["datasourcesGridApi"]["selection"]["getSelectedRows"]().length>0){if(Object.keys($scope["selectedDatasource"]).length===0)return 0;$scope["editedDatasource"]=angular.copy($scope["selectedDatasource"]);$scope["editedDatasource"]=this.loadDatasourceDefaultValues($scope["editedDatasource"]);$scope["schemas"].length=0;$scope["tables"].length=0;if($scope["editedDatasource"]["type"]===
"web_service")if(goog.isDefAndNotNull($scope["editedDatasource"]["webservice"]))if(goog.isDefAndNotNull($scope["editedDatasource"]["webservice"]["name"]))this.updateRessources($scope["editedDatasource"]["webservice"]);$("#studio-datasource-edit-modal").modal("show");var $timeout=angular.element(vitisApp.appMainDrtv).injector().get(["$timeout"]);$timeout(function(){$scope.refresh=true;$timeout(function(){$scope.refresh=false},100)},500);if($scope["databases"].length==0){$scope["databaseSelected"]=
$scope["editedDatasource"]["parameters"]["database"];this["setLocalDatabases"]($scope["editedDatasource"]["parameters"]["database"])}for(var i=0;i<$scope["databases"].length;i++)if($scope["databases"][i]["database"]===$scope["editedDatasource"]["parameters"]["database"])$scope["databaseSelected"]=$scope["databases"][i];for(var i=0;i<$scope["schemas"].length;i++)if($scope["schemas"][i]["schema_name"]===$scope["editedDatasource"]["parameters"]["schema"]){$scope["schemaSelected"]=$scope["schemas"][i];
this.reloadTablesList($scope["editedDatasource"]["parameters"]["schema"])}$timeout(function(){for(var i=0;i<$scope["tables"].length;i++)if($scope["tables"][i]["table_name"]===$scope["editedDatasource"]["parameters"]["table"])$scope["tableSelected"]=$scope["tables"][i]},500)}else bootbox.alert(this["text"]["Element"]["Component"]["Select"]["Datasource_NothingSelected"])};
oVFB.studioMainController.prototype.isEmptyFilter=function(filter){oVFB.log("studioMainController.isEmptyFilter");if(!goog.isDefAndNotNull(filter))return true;if(goog.isString(filter)){if(filter.length===0)return true}else if(goog.isObject(filter))if(goog.isArray(filter["operators"])){if(filter["operators"].length>1)return false;if(filter["operators"].length===0)return true;if(filter["operators"][0]["column"]===""&&filter["operators"][0]["value"]==="")return true}return false};
oVFB.studioMainController.prototype.loadDatasourceDefaultValues=function(oDatasource){oVFB.log("studioMainController.loadDatasourceDefaultValues");if(!goog.isDefAndNotNull(oDatasource["parameters"]))oDatasource["parameters"]={};if(this.isEmptyFilter(oDatasource["parameters"]["filter"])||!goog.isObject(oDatasource["parameters"]["filter"]))oDatasource["parameters"].filter={"relation":"AND","operators":[]};this.clearTestGrids();return oDatasource};
...
{"id":newSource["id"],"type":"web_service","dataType":newSource["dataType"],"name":newSource["name"],"description":newSource["description"],"parameters":newSource["parameters"],"ressource_id":"vitis/genericquerys/"+newSource["parameters"]["table"]};$("#studio-datasource-add-modal").modal("hide");this.saveDatasource(source);this.setStudioGridDatasources()}else if(newSource["dataType"]==="tableValue"){if(goog.isDefAndNotNull(newSource["parameters"]))var oParams=angular.copy(newSource["parameters"]);
if(!goog.isDef(newSource["id"])){console.error("newSource.id not defined");return 0}if(!goog.isDef(newSource["name"])){console.error("newSource.name not defined");return 0}if(!goog.isDef(newSource["parameters"]["schema"])){console.error("newSource.parameters.schema not defined");return 0}if(!goog.isDef(newSource["parameters"]["table"])){console.error("newSource.parameters.table not defined");return 0}if(goog.isDef($scope["datasources"][newSource["id"]])){bootbox.alert(this["text"]["Element"]["Component"]["Select"]["Datasource_Exists"]);
return 0}if(goog.isDef(oParams["filter"]["operators"]))for(var i=0;i<oParams["filter"]["operators"].length;i++)if(oParams["filter"]["operators"][i].compare_operator==""||oParams["filter"]["operators"][i].column==""||oParams["filter"]["operators"][i].value=="")oParams["filter"]["operators"].splice(i,1);var source={"id":newSource["id"],"type":"web_service","dataType":newSource["dataType"],"name":newSource["name"],"description":newSource["description"],"parameters":oParams,"ressource_id":"vitis/genericquerys"};
$("#studio-datasource-add-modal").modal("hide");this.saveDatasource(source);this.setStudioGridDatasources()}else if(newSource["dataType"]==="webService"){if(!goog.isDef(newSource["id"])){console.error("newSource.id not defined");return 0}if(!goog.isDef(newSource["name"])){console.error("newSource.name not defined");return 0}if(!goog.isDef(newSource["webservice"])){bootbox.alert(this["text"]["Element"]["Component"]["Select"]["WebService_undefined"]);console.error("newSource.webservice not defined");
return 0}if(!goog.isDef(newSource["ressource"])){bootbox.alert(this["text"]["Element"]["Component"]["Select"]["Ressource_undefined"]);console.error("newSource.ressource not defined");return 0}if(goog.isDef($scope["datasources"][newSource["id"]])){bootbox.alert(this["text"]["Element"]["Component"]["Select"]["Datasource_Exists"]);return 0}var source={"id":newSource["id"],"type":"web_service","dataType":newSource["dataType"],"name":newSource["name"],"description":newSource["description"],"webservice":newSource["webservice"],
"ressource":newSource["ressource"],"ressource_id":newSource["webservice"]["name"]+"/"+newSource["ressource"]["name"]};$("#studio-datasource-add-modal").modal("hide");this.saveDatasource(source);this.setStudioGridDatasources()}else if(newSource["dataType"]==="businessObject"){if(!goog.isDef(newSource["id"])){console.error("newSource.id not defined");return 0}if(!goog.isDef(newSource["name"])){console.error("newSource.name not defined");return 0}if(!goog.isDef(newSource["businessObject"]["business_object_id"])){bootbox.alert(this["text"]["Element"]["Component"]["Select"]["BusinessObject_undefined"]);
console.error("newSource.businessObject not defined");return 0}if(goog.isDef($scope["datasources"][newSource["id"]])){bootbox.alert(this["text"]["Element"]["Component"]["Select"]["Datasource_Exists"]);return 0}var source={"id":newSource["id"],"type":"web_service","dataType":newSource["dataType"],"name":newSource["name"],"description":newSource["description"],"businessObject":newSource["businessObject"],"businessObjectRequest":newSource["businessObjectRequest"],"subObject":"bo_"+newSource["businessObjectRequest"],
"ressource_id":"vmap/querys/"+newSource["businessObject"]["business_object_id"]+"/"+newSource["businessObjectRequest"]};$("#studio-datasource-add-modal").modal("hide");this.saveDatasource(source);this.setStudioGridDatasources()}};
...
oVFB.studioMainController.prototype.updateRessources=function(webService){oVFB.log("studioMainController.updateRessources");var $scope=this.$scope_;ajaxRequest({"method":"GET","url":oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/vitis/webservices/"+webService["name"]+"/ressources","headers":{"Accept":"application/x-vm-json"},"params":{"attributs":"name"},"scope":this.$scope_,"success":function(response){if(!goog.isDefAndNotNull(response.data["data"])){console.error("response.data['data'] not defined");
return}$scope["ressources"]=response.data["data"]}})};
oVFB.studioMainController.prototype.testTableValues=function(datasource,modalName){oVFB.log("studioMainController.testTableValues");var $scope=this.$scope_;if(goog.isDefAndNotNull(datasource["parameters"]))var oParams=angular.copy(datasource["parameters"]);if(goog.isDefAndNotNull(oParams["filter"]["operators"]))for(var i=0;i<oParams["filter"]["operators"].length;i++)if(oParams["filter"]["operators"][i].compare_operator==""||oParams["filter"]["operators"][i].column==""||oParams["filter"]["operators"][i].value==
"")oParams["filter"]["operators"].splice(i,1);if(goog.isDefAndNotNull(oParams["filter"]["operators"]))if(oParams["filter"]["operators"].length===0){delete oParams["filter"].operators;delete oParams["filter"].relation;delete oParams.filter}var url=oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/vitis/genericquerys";var params={"limit":10};if(goog.isDefAndNotNull(oParams))goog.object.extend(params,oParams);if(modalName==="studio-datasource-add-modal")this.testRequestAPI(url,
params,$scope["studioAddModalTestGridOptions"],$scope.studioAddModalTestGridApi);else if(modalName==="studio-datasource-edit-modal")this.testRequestAPI(url,params,$scope["studioEditModalTestGridOptions"],$scope.studioEditModalTestGridApi)};
...
"/vmap/querys/"+datasource["businessObject"]["business_object_id"]+"/"+datasource["businessObjectRequest"];var params={"limit":10};if(goog.isDefAndNotNull(datasource["parameters"]))goog.object.extend(params,datasource["parameters"]);if(modalName==="studio-datasource-add-modal")this.testRequestAPI(url,params,$scope["studioAddModalTestGridOptions"],$scope.studioAddModalTestGridApi,"bo_"+datasource["businessObjectRequest"]);else if(modalName==="studio-datasource-edit-modal")this.testRequestAPI(url,params,
$scope["studioEditModalTestGridOptions"],$scope.studioEditModalTestGridApi,"bo_"+datasource["businessObjectRequest"])};
oVFB.studioMainController.prototype.testRequestAPI=function(url,params,gridOptions,gridApi,subObject){oVFB.log("studioMainController.testRequestAPI");var this_=this;var $q=this.$q_;if(!goog.isDef(url)){console.error("url not defined");return 0}if(!goog.isDef(params)){console.error("params not defined");return 0}ajaxRequest({"method":"GET","url":url,"headers":{"Accept":"application/x-vm-json"},"params":params,"scope":this.$scope_,"success":function(response){if(!goog.isDef(response["data"]["data"])){bootbox.alert(this_["text"]["Element"]["Component"]["Select"]["NoDataReturned"]);
return 0}var data;if(goog.isDefAndNotNull(subObject)){data=[];for(var i=0;i<response["data"]["data"].length;i++)data.push(response["data"]["data"][i][subObject])}else data=response["data"]["data"];var keys=Object.keys(data[0]);var columnDefs=[];for(var i=0;i<keys.length;i++)columnDefs.push({"name":keys[i],"displayName":keys[i],"width":100});gridOptions["data"]=data;gridOptions["columnDefs"]=columnDefs;gridApi["grid"]["refresh"]()}})};
oVFB.studioMainController.prototype.clearDatasourcesGrid=function(){oVFB.log("studioMainController.clearDatasourcesGrid");var $scope=this.$scope_;$scope["studioDatasourcesGridOptions"]["data"]=[];$scope["datasourcesGridApi"]["grid"]["refresh"]()};
...
this["text"]["Element"]["Component"]["Select"]["Undefined"]);return 0}if(!goog.isDefAndNotNull(datasource["parameters"]["login"])){bootbox.alert("Login "+this["text"]["Element"]["Component"]["Select"]["Undefined"]);return 0}if(!goog.isDefAndNotNull(datasource["parameters"]["password"])){bootbox.alert("Password "+this["text"]["Element"]["Component"]["Select"]["Undefined"]);return 0}if(!goog.isDefAndNotNull(datasource["parameters"]["database"])){bootbox.alert("Database "+this["text"]["Element"]["Component"]["Select"]["Undefined"]);
return 0}if(!goog.isDefAndNotNull(datasource["parameters"]["schema"])){bootbox.alert("Schema "+this["text"]["Element"]["Component"]["Select"]["Undefined"]);return 0}var url=oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/vitis/genericquerys";var params={"login":datasource["parameters"]["login"],"password":datasource["parameters"]["password"],"server":datasource["parameters"]["server"],"port":datasource["parameters"]["port"],"sgbd":datasource["parameters"]["sgbd"],
"database":datasource["parameters"]["database"],"schema":datasource["parameters"]["schema"],"order_by":"table_name"};if(!goog.isDefAndNotNull(url)){console.error("url undefined");return 0}if(!goog.isDefAndNotNull(params)){console.error("params undefined");return 0}ajaxRequest({"method":"GET","url":url,"headers":{"Accept":"application/x-vm-json"},"params":params,"scope":this.$scope_,"timeout":5E3,"success":function(response){if(!goog.isDef(response["data"]["data"])){bootbox.alert(this_["text"]["Element"]["Component"]["Select"]["NoDataReturned"]);
return 0}if(!goog.isArray($scope["aExternalDatabaseTables"]))$scope["aExternalDatabaseTables"]=[];else $scope["aExternalDatabaseTables"].length=0;for(var i=0;i<response["data"]["data"].length;i++)$scope["aExternalDatabaseTables"].push(response["data"]["data"][i]["table_name"])}})};
oVFB.studioMainController.prototype.addNewFilterOperator=function(oDatasource){oVFB.log("studioMainController.addNewFilterOperator");if(!goog.isDef(oDatasource["parameters"]["filter"]))oDatasource["parameters"]["filter"]={"relation":"AND","operators":[{"column":"","compare_operator":"=","value":""}]};else if(!goog.isDef(oDatasource["parameters"]["filter"]["operators"]))oDatasource["parameters"]["filter"]["operators"]=[{"column":"","compare_operator":"=","value":""}];else if(oDatasource["parameters"]["filter"]["operators"].length>
0)oDatasource["parameters"]["filter"]["operators"].push({"column":"","compare_operator":"=","value":""});else if(oDatasource["parameters"]["filter"]["operators"].length===0){delete oDatasource["parameters"]["filter"].operators;oDatasource["parameters"]["filter"]["operators"]=[{"column":"","compare_operator":"=","value":""}]}};
oVFB.studioMainController.prototype.removeFilterOperator=function(oDatasource,oOperator){oVFB.log("studioMainController.removeFilterOperator");if(!goog.isDefAndNotNull(oOperator))return false;if(!goog.isDefAndNotNull(oDatasource))return false;if(!goog.isDefAndNotNull(oDatasource["parameters"]))return false;if(!goog.isDefAndNotNull(oDatasource["parameters"]["filter"]))return false;if(!goog.isArray(oDatasource["parameters"]["filter"]["operators"]))return false;var iOperatorIndex=oDatasource["parameters"]["filter"]["operators"].indexOf(oOperator);
if(iOperatorIndex!==-1)oDatasource["parameters"]["filter"]["operators"].splice(iOperatorIndex,1)};oVFB.utils_=nsUtils;if(oProperties["mode"]==="dev")oVFB.utils_.bDebugMode=true;oVFB.log=oVFB.utils_.log;oVFB.unselectAll=oVFB.utils_.unselectAll;oVFB.reloadForm=oVFB.utils_.reloadForm;oVFB.removeWebServiceFromFields=oVFB.utils_.removeWebServiceFromFields;oVFB.JsonStringifier=oVFB.utils_.JsonStringifier;oVFB.JsonOptimizer=oVFB.utils_.JsonOptimizer;oVFB.clone=oVFB.utils_.clone;oVFB.Update=false;
oVFB.oDevTools_={};oVFB.oJsonLoader_={};oVFB.oTreeView_={};oVFB.oFormTools_={};oVFB.oElementForm_={};oVFB.oCompoList_={};oVFB.oVisualizer_={};oVFB.oJson_={"empty":true};oVFB.oJsonOutput_={};oVFB.sJs_="";oVFB.sCSS_="";oVFB.TimeNotify_=oProperties["timeNotify"];oVFB.Collapse_=[false,false,false,false,false];oVFB.valid=true;oVFB.aNotValid=[];oVFB.token="";oVFB.AppName="";oVFB.id=0;oVFB.el=null;oVFB.AppProperties=null;oVFB.tree_="no tree";oVFB.oJsonLoader_=new nsVFB.JsonLoader;oVFB.oFormTools_=new nsVFB.FormTools;
oVFB.oElementForm_=new nsVFB.ElementForm;oVFB.oVisualizer_=new nsVFB.Visualizer;goog.provide("nsVFB.FormTools");goog.require("goog.object");nsVFB.FormTools=function(){oVFB.log("nsVFB.FormTools");oVFB.module.directive("appFormTools",this.formToolsDirective);oVFB.module.controller("AppFormToolsController",["$scope","$rootScope","$timeout",this.formToolsController])};
nsVFB.FormTools.prototype.formToolsDirective=function(){oVFB.log("nsVFB.FormTools.FormToolsDirective");return{restrict:"A",replace:true,controller:"AppFormToolsController",controllerAs:"ctrl",scope:true,bindToController:true,templateUrl:"javascript/externs/studio/templates/FormTools.html"}};
nsVFB.FormTools.prototype.formToolsController=function($scope,$rootScope,$timeout){oVFB.log("nsVFB.FormTools.FormToolsController");var FormToolsController=this;this.$rootScope_=$rootScope;this.$scope_=$scope;this.elemAdded_=0;this["applicationName"]=oVFB.getApplication();this.bTreeWithLines_=false;$scope["copyPasteContainer"]={};$scope.$watch("text",function(){oVFB.log("formtools text");if(goog.isDef($scope["text"]["FormTools"]))FormToolsController["text"]=$scope["text"]["FormTools"]});$scope["colapse_tools"]=
oVFB.Collapse_[2];FormToolsController.convertJson=function(displayLines){oVFB.log("nsVFB.FormTools.convertJson");var oJson=oVFB.getJsonOutput();var aTree=[];if(!goog.isDefAndNotNull(oJson)){console.error("No JSON founded");return aTree}displayLines=goog.isDef(displayLines)?displayLines:true;if(!goog.object.isEmpty(oJson[$rootScope["selected_form_type"]]))for(var i=0;i<oJson[$rootScope["selected_form_type"]]["rows"].length;i++){if(displayLines===true)if(goog.isDefAndNotNull(this["text"]))aTree.push({"text":this["text"]["Row"]+
" "+i,"state":{"expanded":true},"tags":[0],"nodes":[],"row":i,"draggable":true});for(var j=0;j<oJson[$rootScope["selected_form_type"]]["rows"][i]["fields"].length;j++){var glyphicon;switch(oJson[$rootScope["selected_form_type"]]["rows"][i]["fields"][j]["type"]){case "codemirror":glyphicon="icon-code";break;case "select":case "list":case "double_select":glyphicon="icon-format_list_bulleted";break;case "text":case "tinymce":glyphicon="icon-font";break;case "upload":glyphicon="icon-file";break;case "checkbox":glyphicon=
"icon-check-square";break;case "radio":glyphicon="icon-dot-circle-o";break;case "date":case "datetime":glyphicon="icon-calendar-check-o";break;case "password":glyphicon="icon-key";break;case "email":glyphicon="icon-at";break;case "url":glyphicon="icon-chrome";break;case "integer":case "float":case "number":glyphicon="icon-calculator";break;case "color_picker":glyphicon="icon-paint";break;case "slider":glyphicon="icon-sliders";break;case "textarea":glyphicon="icon-paragraph";break;case "hr":glyphicon=
"icon-minus";break;case "label":glyphicon="icon-eye";break;case "email":glyphicon="icon-at";break;case "hidden":glyphicon="icon-user-secret";break;case "link":glyphicon="glyphicon glyphicon-link";break;case "image":glyphicon="glyphicon glyphicon-picture";break;case "map_bing":case "map_osm":case "map_vmap":glyphicon="icon-map";break;default:glyphicon="icon-point"}if(displayLines===true){if(goog.isDefAndNotNull(aTree[i])){aTree[i]["tags"][0]+=1;aTree[i]["nodes"].push({"text":"  "+oJson[$rootScope["selected_form_type"]]["rows"][i]["fields"][j]["name"],
...
nsVFB.FormTools.prototype.formToolsController.prototype.up=function(){oVFB.Update=true;oVFB.log("nsVFB.FormTools.prototype.formToolsController.prototype.up");var this_=this;var aSelectedNode=$("#treeWithLines")["treeview"]("getSelected");var selectedNodeId=aSelectedNode[0]["nodeId"];var data=oVFB.getJsonOutput();if(aSelectedNode.length===1)if(!goog.isDef(aSelectedNode[0]["field"])){if(aSelectedNode[0]["row"]-1>=0){var row=aSelectedNode[0]["row"];var treeLine=aSelectedNode[0];var treePreviousLine=
this.getPreviousLineBeforeIndex(selectedNodeId);var aCollapsedElements=$("#treeWithLines")["treeview"]("getCollapsed");treeLine["collapsed"]=goog.object.contains(aCollapsedElements,treeLine);treePreviousLine["collapsed"]=goog.object.contains(aCollapsedElements,treePreviousLine);if(treePreviousLine["collapsed"]===true)$("#treeWithLines")["treeview"]("collapseNode",treeLine["nodeId"]);else $("#treeWithLines")["treeview"]("expandNode",treeLine["nodeId"]);if(treeLine["collapsed"]===true)$("#treeWithLines")["treeview"]("collapseNode",
treePreviousLine["nodeId"]);else $("#treeWithLines")["treeview"]("expandNode",treePreviousLine["nodeId"]);var tmp=treeLine["collapsed"];treeLine["collapsed"]=treePreviousLine["collapsed"];treePreviousLine["collapsed"]=treeLine["collapsed"];var tmp=data[this.$rootScope_["selected_form_type"]]["rows"][row-1];data[this.$rootScope_["selected_form_type"]]["rows"][row-1]=data[this.$rootScope_["selected_form_type"]]["rows"][row];data[this.$rootScope_["selected_form_type"]]["rows"][row]=tmp}var previousLine=
this.getPreviousLineBeforeIndex(selectedNodeId);selectedNodeId=previousLine["nodeId"];if(!this.bTreeWithLines_)selectedNodeId=previousLine["nodes"][0]["nodeId"]}else if(aSelectedNode[0]["field"]-1>=0){var tmp=data[this.$rootScope_["selected_form_type"]]["rows"][aSelectedNode[0]["row"]]["fields"][aSelectedNode[0]["field"]-1];data[this.$rootScope_["selected_form_type"]]["rows"][aSelectedNode[0]["row"]]["fields"][aSelectedNode[0]["field"]-1]=data[this.$rootScope_["selected_form_type"]]["rows"][aSelectedNode[0]["row"]]["fields"][aSelectedNode[0]["field"]];
...
nsVFB.FormTools.prototype.formToolsController.prototype.copyToClipboard=function(){var oJson=oVFB.getJsonOutput();var textArea=document.createElement("textarea");textArea.style.position="fixed";textArea.style.top=0;textArea.style.left=0;textArea.style.width="2em";textArea.style.height="2em";textArea.style.padding=0;textArea.style.border="none";textArea.style.outline="none";textArea.style.boxShadow="none";textArea.style.background="transparent";textArea.value=JSON.stringify(oJson);document.body.appendChild(textArea);
textArea.focus();textArea.select();try{var successful=document.execCommand("copy");var msg=successful?"successful":"unsuccessful";console.log("Copying text command was "+msg)}catch(err){console.log("error during copy"+err)}document.body.removeChild(textArea)};nsVFB.FormTools.prototype.formToolsController.prototype.copy=function(){var oJson=oVFB.getJsonOutput();oVFB.Update=true;this.$scope_["copyPasteContainer"]=angular.copy(oJson[this.$rootScope_["selected_form_type"]])};
nsVFB.FormTools.prototype.formToolsController.prototype.paste=function(){var oJson=oVFB.getJsonOutput();oVFB.Update=true;oJson[this.$rootScope_["selected_form_type"]]=angular.copy(this.$scope_["copyPasteContainer"]);oVFB.setJsonOutput(oJson);this.$rootScope_.$broadcast("updateEvent",oVFB.getJsonOutput())};nsVFB.FormTools.prototype.formToolsController.prototype.labelizeForm=function(){oVFB.Update=true;this.$rootScope_.$broadcast("labelizeForm",null)};
nsVFB.FormTools.prototype.formToolsController.prototype.openModalMethodcreator=function(){var $scope=this.$scope_;var $this=this;var sModuleName=$scope["module"];var sObjectName=$scope["object"];ajaxRequest({"method":"POST","url":oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/wab/methods","headers":{"Accept":"application/x-vm-json","X-HTTP-Method-Override":"GET"},"scope":$scope,"data":{"sort_order":"ASC","distinct":true,"filter":{"relation":"AND","operators":[{"column":"module_id",
"compare_operator":"=","value":sModuleName},{"column":"bo_name","compare_operator":"=","value":sObjectName}]}},"success":function(response){console.log(response);$this.$rootScope_["aBoMethods"]=response["data"]["data"];for(var i=0;i<$this.$rootScope_["aBoMethods"].length;i++){$this.$rootScope_["aBoMethods"][i]["group"]=$this.$rootScope_["aBoMethods"][i]["name"]+"_"+i;var sFinalLabel="";var iLastIndex=0;var iStartIndex=1;if($this.$rootScope_["aBoMethods"][i]["name"].indexOf("tr")===0){iLastIndex=2;
iStartIndex=3}for(var j=iStartIndex;j<$this.$rootScope_["aBoMethods"][i]["name"].length;j++)if($this.$rootScope_["aBoMethods"][i]["name"][j]===$this.$rootScope_["aBoMethods"][i]["name"][j].toUpperCase()&&$this.$rootScope_["aBoMethods"][i]["name"][j-1]===$this.$rootScope_["aBoMethods"][i]["name"][j-1].toLowerCase()){sFinalLabel+=$this.$rootScope_["aBoMethods"][i]["name"].substring(iLastIndex,j)+" ";iLastIndex=j}sFinalLabel+=$this.$rootScope_["aBoMethods"][i]["name"].substring(iLastIndex);$this.$rootScope_["aBoMethods"][i]["label"]=
sFinalLabel}$("#studio-wabmanager-method-modal").modal("show")}})};
nsVFB.FormTools.prototype.formToolsController.prototype.idName=function(){oVFB.Update=true;var oJson=oVFB.getJsonOutput();var oForm;for(var sFormName in oJson){oForm=oJson[sFormName];if(goog.isDefAndNotNull(oForm["rows"]))for(var i=0;i<oForm["rows"].length;i++)if(goog.isDefAndNotNull(oForm["rows"][i]["fields"]))for(var ii=0;ii<oForm["rows"][i]["fields"].length;ii++)oForm["rows"][i]["fields"][ii]["id"]=oForm["rows"][i]["fields"][ii]["name"]}oVFB.setJsonOutput(oJson);this.$rootScope_.$broadcast("updateEvent",
oVFB.getJsonOutput())};goog.provide("nsVFB.JsonLoader");goog.require("goog.array");nsVFB.JsonLoader=function(){oVFB.log("nsVFB.JsonLoader");this.slider=null;oVFB.module.directive("appJsonLoader",this.jsonLoaderDirective);oVFB.module.controller("AppJsonLoaderController",["$scope","$rootScope","$timeout",this.jsonLoaderController])};
...
nsVFB.JsonLoader.prototype.jsonLoaderController=function($scope,$rootScope,$timeout){oVFB.log("nsVFB.JsonLoader.jsonLoaderController");var this_=this;this.$scope_=$scope;this.$rootScope_=$rootScope;this.tmpForm_={};this["buttonHide"]=false;this["aSuggestedTableFields"]=[];this["applicationName"]=oVFB.getApplication();$scope["collapsed"]=oVFB.Collapse_[0];$scope["iWorkspaceId"]=oVFB.getId();$scope["iWorkSpace"]=oVFB.getId();$scope["default"]="Default";$scope["FormSelected"]="Perso";$scope["token"]=
oVFB.getToken();$rootScope["selected_form_type"]=oVFB["config"]["default_form_type"];$rootScope.$watch("selected_form_type",function(value){oVFB.log("jsonLoaderController selected_form_type");var oJSONform=oVFB.getJsonOutput();if(!goog.isDefAndNotNull(oJSONform[value])){oJSONform[value]={"name":"custom-form","title":"","input_size":"xxs","nb_cols":12,"javascript":false,"rows":[]};oVFB.setJsonOutput(oJSONform)}$rootScope.$broadcast("updateEvent",oVFB.getJsonOutput());$rootScope.$broadcast("update element",
{type:"undefined"})});oVFB.getJsonLoader().slider=new Slider("#slider_Form",{"formatter":function(value){return Math.round(value*100/12)+"%"}});oVFB.getJsonLoader().slider.on("change",function(){var tmp=oVFB.getJsonOutput();tmp[$rootScope["selected_form_type"]]["nb_cols"]=oVFB.getJsonLoader().slider["getValue"]();oVFB.setJsonOutput(tmp);$timeout(function(){$rootScope.$broadcast("updateJsonModel",oVFB.getJsonOutput());$rootScope.$broadcast("updateForm",oVFB.getJsonOutput());$scope.$apply()},1)});$scope.$watch("title",
function(value){oVFB.log("jsonLoaderController title");var tmp=oVFB.getJsonOutput();if(tmp[$rootScope["selected_form_type"]])tmp[$rootScope["selected_form_type"]]["title"]=value;oVFB.setJsonOutput(tmp);$rootScope.$broadcast("updateJsonModel",oVFB.getJsonOutput());$rootScope.$broadcast("updateForm",oVFB.getJsonOutput())});$scope.$watch("event",function(value){oVFB.log("jsonLoaderController event");var tmp=oVFB.getJsonOutput();if(tmp[$rootScope["selected_form_type"]])tmp[$rootScope["selected_form_type"]]["event"]=
value;oVFB.setJsonOutput(tmp);$rootScope.$broadcast("updateJsonModel",oVFB.getJsonOutput());$rootScope.$broadcast("updateForm",oVFB.getJsonOutput())});$scope.$watch("token",function(value){oVFB.log("jsonLoaderController token");oVFB["setToken"](value)});$scope.$watch("iWorkspaceId",function(value){oVFB.log("jsonLoaderController id");oVFB["setId"](value)});$scope.$watch("text",function(){oVFB.log("jsonLoaderController text");if(typeof $scope["text"]["Loader"]!=="undefined"){this_["text"]=$scope["text"]["Loader"];
var loadApp=function(){if(goog.isDefAndNotNull(angular.element($("#FormModal")).scope())){angular.element($("#load_Perso_button")).click();if(oVFB.getApplication()=="wab")this_["buttonHide"]=true}else setTimeout(function(){loadApp()},1E3)};$timeout(function(){loadApp()})}});$scope.state=false};
nsVFB.JsonLoader.prototype.jsonLoaderController.prototype.requestToJson=function(successCallback,errorCallback){oVFB.log("nsVFB.JsonLoader.prototype.jsonLoaderController.requestToJson");var this_=this;var headers={};var params={};angular.element($("#FormModal")).scope()["ctrl"].emptyFormValues();this.$scope_["iWorkSpace"]=this.$scope_["iWorkspaceId"];this.$scope_["default"]=this.$scope_["FormSelected"];this.$scope_["collapsed"]=oVFB.Collapse_[0];if(this["applicationName"]==="gtf"){var url=oVFB.AppProperties["web_server_name"]+
"/"+oVFB.AppProperties["services_alias"]+"/gtf/workspaces/"+oVFB.getId();headers={"Accept":"application/json"};params={"form":this.$scope_["FormSelected"]}}else if(this["applicationName"]==="vmap"){var form=this.$scope_["FormSelected"]==="Perso"?"custom":angular.lowercase(this.$scope_["FormSelected"]);var url=oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/"+oVFB["config"]["get"]["api"]+"/"+oVFB["config"]["get"]["ressource"]+"/"+oVFB.getId()+"/forms/"+form+"/"}else if(this["applicationName"]===
"wab")url=oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/"+oVFB["config"]["get"]["api"]+"/"+oVFB["config"]["get"]["ressource"]+"/"+oVFB.getId()+"/form";showAjaxLoader();ajaxRequest({"method":"GET","url":url,"headers":headers,"params":params,"scope":this.$scope_,"success":function(data){hideAjaxLoader();if(!goog.isDefAndNotNull(data[0]))if(goog.isDefAndNotNull(data["data"]))data=data["data"];if(this_["applicationName"]==="gtf"){if(!goog.isDefAndNotNull(data["workspaces"])&&
!goog.isDefAndNotNull(data["data"])){var cmd;switch(this_.$scope_["FormSelected"]){case "Default":cmd="Default_Reset";break;case "Published":cmd="Default_Published";break;case "Perso":cmd="Perso_Reset";break}this_.putRequest(cmd,undefined,function(data){if(goog.isDefAndNotNull(data["errorMessage"])){bootbox.alert("<h4>"+data["errorMessage"]+"</h4>",function(){});return 0}this_.requestToJson()},function(data,status){$["notify"](this_.$scope_["text"]["Loader"]["Notify"]["GetJson_Error"]+status,{"className":"error",
"autoHideDelay":oVFB.TimeNotify_})})}}else if(!goog.isDefAndNotNull(data[0])){if(goog.isDefAndNotNull(oVFB["config"]["get"]["no_result_function"]))this_[oVFB["config"]["get"]["no_result_function"]](data,successCallback);return 0}this_.useJson(data,successCallback)},"error":function(response){hideAjaxLoader();if(goog.isFunction(errorCallback))this_.$scope_.$eval(errorCallback);$["notify"](this_.$scope_["text"]["Loader"]["Notify"]["GetJson_Error"]+status,{"className":"error","autoHideDelay":oVFB.TimeNotify_})}})};
nsVFB.JsonLoader.prototype.jsonLoaderController.prototype.useJson=function(data,successCallback){oVFB.log("nsVFB.JsonLoader.prototype.jsonLoaderController.prototype.useJson");if(this["applicationName"]==="gtf")if(goog.isDefAndNotNull(data["workspaces"]))data=data["workspaces"][0]["json_form"];else data=data["data"][0]["json_form"];if(!goog.isDefAndNotNull(data[0])){console.error("data[0] not defined, data: ",data);return 0}oVFB.setJson(angular.copy(data[0]));oVFB.setJsonOutput(angular.copy(data[0]));
this.tmpForm_=angular.copy(data[0]);if(goog.isDefAndNotNull(oVFB["config"]["avaliable_form_types"]))this.$scope_["json_forms"]=oVFB["config"]["avaliable_form_types"];else this.$scope_["json_forms"]=Object.keys(data[0]);if(this.$scope_["json_forms"].indexOf("datasources")!==-1)this.$scope_["json_forms"].splice(this.$scope_["json_forms"].indexOf("datasources"),1);if(goog.isDef(data[1])&&data[1]!==false){if(this["applicationName"]==="gtf")ajaxRequest({"method":"GET","url":data[1],"scope":this.$scope_,
"timeout":5E3,"responseType":"text","success":function(response){if(goog.isDefAndNotNull(response.data))oVFB.setJs(response.data.replace(/(?:\r\n|\r|\n)/g,"\r\n"))},"error":function(response){console.error("Problem on Loading javascript")}});else oVFB.setJs(oVFB.clone(data[1]));this.$rootScope_.$broadcast("updateJS",oVFB.getJs())}else{oVFB.setJs("");this.$rootScope_.$broadcast("updateJS",oVFB.getJs())}if(goog.isDef(data[2])&&data[2]!==false){oVFB.setCss(oVFB.clone(data[2]));this.$rootScope_.$broadcast("updateCSS",
data[2])}if(!goog.isDefAndNotNull(data[0][this.$rootScope_["selected_form_type"]])){console.error("Result[0]["+this.$rootScope_["selected_form_type"]+"] does not exist");return 0}var nbCols=data[0][this.$rootScope_["selected_form_type"]]["nb_cols"];var title=oVFB.getJsonOutput()[this.$rootScope_["selected_form_type"]]["title"];var event=oVFB.getJsonOutput()[this.$rootScope_["selected_form_type"]]["event"];var inputSize=oVFB.getJsonOutput()[this.$rootScope_["selected_form_type"]]["input_size"];oVFB.getJsonLoader().slider["setValue"](nbCols);
this.$scope_["title"]=title;this.$scope_["event"]=event;this.$scope_["input_size"]=inputSize;this.$rootScope_.$broadcast("updateEvent",oVFB.getJsonOutput());this.$rootScope_.$broadcast("updateInfos",oVFB.getJsonOutput());this.$rootScope_.$broadcast("update element",{type:"undefined"});if(goog.isFunction(successCallback))this.$scope_.$eval(successCallback);setTimeout(function(){oVFB.reloadForm();angular.element($("#studio_form_reader").children()).scope()["ctrl"].refreshForm()},1)};
nsVFB.JsonLoader.prototype.jsonLoaderController.prototype.load=function(mode,el){oVFB.log("load");if(mode==="Perso")this.loadWithEdit(true,mode,el);else this.loadWithoutEdit(mode,el)};nsVFB.JsonLoader.prototype.jsonLoaderController.prototype.updateSlider=function(){var nbCols=oVFB.getJsonOutput()[this.$rootScope_["selected_form_type"]]["nb_cols"];oVFB.getJsonLoader().slider["setValue"](nbCols)};
...
nsVFB.JsonLoader.prototype.jsonLoaderController.prototype.save=function(successCallback,errorCallback){oVFB.log("nsVFB.JsonLoader.jsonLoaderController.save");var this_=this;var data=new FormData;var tmp=oVFB.getJsonOutput();oVFB.unselectAll();tmp=oVFB.cleanFormToSave(tmp);if(this["applicationName"]==="gtf"){tmp[this_.$rootScope_["selected_form_type"]]["name"]="WSubform";tmp["display"]=tmp[this_.$rootScope_["selected_form_type"]];tmp["update"]=tmp[this_.$rootScope_["selected_form_type"]]}if(!goog.isDef(tmp["search"]))tmp["search"]=
...
oVFB.valid=this.isValid(tmp);if(oVFB.valid&&tmp!==null)this_.putRequest("Perso_Save",data,success,error);else $["notify"](this_.$scope_["text"]["Loader"]["Notify"]["Validation_Error"]+": "+oVFB.aNotValid,{"className":"error","autoHideDelay":oVFB.TimeNotify_});oVFB.Update=false};
nsVFB.JsonLoader.prototype.jsonLoaderController.prototype.publish=function(){oVFB.log("nsVFB.JsonLoader.prototype.jsonLoaderController.publish");oVFB.Update=false;var this_=this;var data=new FormData;var tmp=oVFB.getJsonOutput();oVFB.unselectAll();if(this["applicationName"]==="gtf"){tmp[this_.$rootScope_["selected_form_type"]]["name"]="WSubform";tmp["display"]=tmp[this_.$rootScope_["selected_form_type"]];tmp["update"]=tmp[this_.$rootScope_["selected_form_type"]]}data.append("Json",JSON.stringify(tmp));
data.append("Js",oVFB.getJs());data.append("Css",oVFB.getCss());var success=function(){var data="";var success=function(){this_.requestToJson();$["notify"](this_.$scope_["FormSelected"]+this_.$scope_["text"]["Loader"]["Notify"]["PublishAndSave"],{"className":"success","autoHideDelay":oVFB.TimeNotify_})};var error=function(data,status){$["notify"](this_.$scope_["text"]["Loader"]["Notify"]["Publish_Error"]+status,{"className":"error","autoHideDelay":oVFB.TimeNotify_})};this_.putRequest(this_.$scope_["FormSelected"]+
...
nsVFB.JsonLoader.prototype.jsonLoaderController.prototype.reset=function(){oVFB.log("nsVFB.JsonLoader.jsonLoaderController.reset");var this_=this;bootbox.confirm("<h4>"+this["text"]["InfoBulle"]["Perso"]["Reset"]+" ?</h4>"+"<h4>"+this["text"]["Confirmation"]["Lose"]+"</h4>",function(result){if(result===true)if(this_.$scope_["FormSelected"]==="Default"&&this_["applicationName"]!=="gtf"){if(goog.isDefAndNotNull(oVFB["config"]["reset_default_function"]))this_[oVFB["config"]["reset_default_function"]]()}else{var data=
"";var success=function(){$["notify"](this_.$scope_["FormSelected"]+this_.$scope_["text"]["Loader"]["Notify"]["Reset"],{"className":"success","autoHideDelay":oVFB.TimeNotify_});if(this_.$scope_["FormSelected"]==="Perso")this_.requestToJson()};var error=function(data,status){$["notify"](this_.$scope_["text"]["Loader"]["Notify"]["Reset_Error"]+status,{"className":"error","autoHideDelay":oVFB.TimeNotify_})};this_.putRequest(this_.$scope_["FormSelected"]+"_Reset",data,success,error)}})};
nsVFB.JsonLoader.prototype.jsonLoaderController.prototype.putRequest=function(cmd,data,functSuccess,functError){oVFB.log("nsVFB.JsonLoader.prototype.jsonLoaderController.prototype.putRequest");var url="";var params={};if(this["applicationName"]==="gtf"){url=oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/gtf/workspaces/"+oVFB.getId();params={"cmd":cmd}}else if(this["applicationName"]==="vmap"){var form=this.$scope_["FormSelected"]==="Perso"?"custom":angular.lowercase(this.$scope_["FormSelected"]);
url=oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/"+oVFB["config"]["get"]["api"]+"/"+oVFB["config"]["get"]["ressource"]+"/"+oVFB.getId()+"/forms/"+form;params={"cmd":cmd}}else if(this["applicationName"]==="wab")url=oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/"+oVFB["config"]["get"]["api"]+"/"+oVFB["config"]["get"]["ressource"]+"/"+oVFB.getId()+"/form";ajaxRequest({"method":"PUT","url":url,"headers":{"Content-Type":"undefined"},
"params":params,"data":data,"scope":this.$scope_,"success":functSuccess,"error":functError})};nsVFB.JsonLoader.prototype.jsonLoaderController.prototype.suggestGenerateForm=function(){oVFB.log("nsVFB.JsonLoader.prototype.jsonLoaderController.prototype.suggestGenerateForm");var this_=this;bootbox.confirm("<h4>"+this.$scope_["text"]["Loader"]["Notify"]["generate_form"]+"</h4>",function(result){if(result===true)this_.suggestColumnsToGenerateForm();else{this_["aSuggestedTableFields"]=[];this_.generateFormByBusinessObject()}})};
...
nsVFB.JsonLoader.prototype.jsonLoaderController.prototype.suggestColumnsToGenerateForm=function(){oVFB.log("nsVFB.JsonLoader.prototype.jsonLoaderController.prototype.suggestColumnsToGenerateForm");var url=oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/vmap/businessobjects/"+oVFB.getId()+"/fields/";if(oVFB.getApplication()==="wab")url=oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/wab/sections/"+oVFB.getId()+"/fields";var this_=this;
if(goog.isDef(this_["aSuggestedTableFields"]))goog.array.clear(this_["aSuggestedTableFields"]);ajaxRequest({"method":"GET","url":url,"headers":{"Accept":"application/x-vm-json"},"scope":this.$scope_,"success":function(response){if(!goog.isArray(response["data"])||response["data"].length===0){var $oMainScope=angular.element(vitisApp.appMainDrtv).scope();$oMainScope.$root["modalWindow"]("confirm",this_.$scope_["text"]["Loader"]["Notify"]["generate_form_error"],{"className":"modal-danger","message":this_.$scope_["text"]["Loader"]["Notify"]["generate_form_error_description"],
"buttons":{"cancel":{"label":'<i class="glyphicon glyphicon-remove"></i> Annuler',"className":"btn-default"},"confirm":{"label":'<i class="glyphicon glyphicon-ok"></i> Poursuivre quand m\u00eame',"className":"btn-danger"}},"callback":function(result){if(result===true){this_["aSuggestedTableFields"]=[];this_.generateFormByBusinessObject()}else $oMainScope["setMode"]("search")}})}else{this_["aSuggestedTableFields"]=response["data"];$("#select-fields-modal").modal("show")}},"error":function(response){$["notify"](this_.$scope_["text"]["Loader"]["Notify"]["CreateJson_Error"]+
...
form+"/";if(oVFB.getApplication()==="wab")url=oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/wab/sections/"+oVFB.getId()+"/form";var aFieldsToUse=[];var afieldsLabel={};for(var i=0;i<this_["aSuggestedTableFields"].length;i++){if(this_["aSuggestedTableFields"][i]["fields"][0]["bUseForGenerate"]===true)aFieldsToUse.push(this_["aSuggestedTableFields"][i]["fields"][0]["name"]);afieldsLabel[this_["aSuggestedTableFields"][i]["fields"][0]["name"]]=this_["aSuggestedTableFields"][i]["fields"][0]["label"]}var sFieldsToUse=
aFieldsToUse.toString().replace(/,/gi,"|");$("#select-fields-modal").modal("hide");if(this_.$scope_["FormSelected"]==="Default"){var success=function(){$["notify"](this_.$scope_["FormSelected"]+this_.$scope_["text"]["Loader"]["Notify"]["Reset"],{"className":"success","autoHideDelay":oVFB.TimeNotify_});if(this_.$scope_["FormSelected"]==="Default")this_.requestToJson()};var error=function(data,status){$["notify"](this_.$scope_["text"]["Loader"]["Notify"]["Reset_Error"]+status,{"className":"error","autoHideDelay":oVFB.TimeNotify_})};
this_.putRequest(this_.$scope_["FormSelected"]+"_Reset",{"field":sFieldsToUse,"label":afieldsLabel},success,error)}else ajaxRequest({"method":"POST","url":url,"data":{"fields":sFieldsToUse,"label":afieldsLabel},"scope":this.$scope_,"success":function(response){this_.requestToJson()},"error":function(response){$["notify"](this_.$scope_["text"]["Loader"]["Notify"]["CreateJson_Error"]+status,{"className":"error","autoHideDelay":oVFB.TimeNotify_})}})};
nsVFB.JsonLoader.prototype.jsonLoaderController.prototype.suggestColumnsSelectAllFields=function(){oVFB.log("nsVFB.JsonLoader.prototype.jsonLoaderController.prototype.suggestColumnsSelectAllFields");for(var i=0;i<this["aSuggestedTableFields"].length;i++)this["aSuggestedTableFields"][i]["fields"][0]["bUseForGenerate"]=true};
nsVFB.JsonLoader.prototype.jsonLoaderController.prototype.suggestColumnsUnselectAllFields=function(){oVFB.log("nsVFB.JsonLoader.prototype.jsonLoaderController.prototype.suggestColumnsSelectAllFields");for(var i=0;i<this["aSuggestedTableFields"].length;i++)this["aSuggestedTableFields"][i]["fields"][0]["bUseForGenerate"]=false};
nsVFB.JsonLoader.prototype.jsonLoaderController.prototype.suggestColumnsAreAllFieldsSelected=function(){oVFB.log("nsVFB.JsonLoader.prototype.jsonLoaderController.prototype.suggestColumnsAreAllFieldsSelected");var bAllSelected=true;for(var i=0;i<this["aSuggestedTableFields"].length;i++)if(this["aSuggestedTableFields"][i]["fields"][0]["bUseForGenerate"]!==true)bAllSelected=false;return bAllSelected};goog.provide("nsUtils");nsUtils={};nsUtils.bDebugMode_=false;nsUtils.log=function(message){if(goog.isDefAndNotNull(oVFB.AppProperties))if(oVFB.AppProperties["debug_mode"])if(nsUtils.bDebugMode_)console.log(message)};
...
nsVFB.Visualizer.prototype.visualizerController.prototype.loadDefaultJavascript=function(){oVFB.log("nsVFB.visualizerController.loadDefaultJavascript");var $scope=this.$scope_;var sJs=oVFB.getJs();if(goog.isDef(sJs))if(sJs!=="")bootbox.confirm("<h4>"+this["text"]["Confirmation"]["LoseJs"]+"</h4>",function(result){if(result===true)ajaxRequest({"method":"GET","url":oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["public_alias"]+"/studio/default.js","scope":this.$scope_,"timeout":5E3,"success":function(response){if(goog.isDefAndNotNull(response.data)){oVFB.setJs(response.data.replace(/(?:\r\n|\r|\n)/g,
"\r\n"));$scope.$broadcast("updateJS",oVFB.getJs())}},"error":function(response){console.error("Problem on Loading default javascript")}});else return})};nsVFB.Visualizer.prototype.getFormMode=function(){return nsVFB.Visualizer.oFormMode_};nsVFB.Visualizer.prototype.getJsonMode=function(){return nsVFB.Visualizer.oJsonMode_};nsVFB.Visualizer.prototype.getJsMode=function(){return nsVFB.Visualizer.oJsMode_};nsVFB.Visualizer.prototype.getCssMode=function(){return nsVFB.Visualizer.oCssMode_};goog.provide("nsVFB.nsVisualizerMode.CssMode");nsVFB.nsVisualizerMode.CssMode=function(){oVFB.log("nsVFB.nsVisualizerMode.CssMode");oVFB.module.directive("appEditorCss",this.editorCssModeDirective);oVFB.module.controller("AppEditorCssController",["$scope",this.editorCssModeController])};
...
function(event,data){oVFB.log(oVFB.getCss());$scope["CssModel"]=oVFB.getCss()})};goog.provide("nsVFB.nsVisualizerMode.FormMode");goog.require("goog.object");nsVFB.nsVisualizerMode.FormMode=function(){oVFB.log("nsVFB.nsVisualizerMode.FormMode");oVFB.module.directive("appVisualizermodeForm",this.visualizerFormModeDirective);oVFB.module.directive("appSpecificParam",["$timeout","$rootScope","$q",this.specificParameterDirective]);oVFB.module.controller("AppVisualizermodeFormController",["$scope",this.visualizerFormModeController])};
...
data)});$scope.$on("updateForm",function(event,data){oVFB.log("visualizerFormModeController updateForm");if(!goog.object.isEmpty(data[$scope["selected_form_type"]])){var oJson=data[$scope["selected_form_type"]];if(!goog.object.isEmpty(oJson["rows"]))for(var i=0;i<oJson["rows"].length;i++)for(var j=0;j<oJson["rows"][i]["fields"].length;j++)if(!oJson["rows"][i]["fields"][j]["id"])oJson["rows"][i]["fields"][j]["id"]=oJson["rows"][i]["fields"][j]["name"]+"_"+(i+1)+"_"+(j+1);$scope["oFormDefinition"]=
data;$scope["sFormDefinitionName"]=$scope["selected_form_type"];$scope["title"]=oJson["title"];if(!goog.isDef($scope["oFormValues"]))$scope["oFormValues"]={};if(!goog.isDef($scope["oFormValues"][$scope["sFormDefinitionName"]]))$scope["oFormValues"][$scope["sFormDefinitionName"]]={};angular.element($("#studio_form_reader").children()).scope()["ctrl"].refreshForm()}else oVFB.log("visualizerFormModeController. Load JSON form fail: "+$scope["selected_form_type"]+" undefined in ",data)});$scope.$on("updateDefautValue",
function(event,data){if(typeof data["default_value"]!="undefined"&&data["default_value"]!="")switch(data["type"]){case "select":if(!isNaN(data["default_value"]))data["default_value"]=Number(data["default_value"]);break;case "list":var $timeout=angular.element(vitisApp.appWorkspaceListDrtv).injector().get(["$timeout"]);$timeout(function(){var aValues=[];var oListElement=document.getElementById(data["id"]);if(oListElement!==null){if(String(data["default_value"]).indexOf("|")==-1)aValues.push(data["default_value"]);
else aValues=String(data["default_value"]).split("|");for(var i=0;i<oListElement.options.length;i++)if(aValues.indexOf(oListElement.options[i].value)!=-1)oListElement.options[i].selected=true;else oListElement.options[i].selected=false}});break}angular.element($("#studio_form_reader").children()).scope()["ctrl"].useDefaultValues(true,data["name"])})};
...
nsVFB.nsVisualizerMode.JsMode.prototype.editorJsModeController=function($scope){oVFB.log("nsVFB.VisualizerMode.JsMode.editorJsModeController");$scope["JsModel"]=oVFB.getJs();$scope["editorOptions"]={"mode":{"name":"javascript","json":false},"readOnly":false,"styleActiveLine":true,"lineNumbers":true,"lineSeparator":null,"indentWithTabs":true,"tabSize":2,"viewportMargin":50,"theme":"Veremes","foldGutter":true,"gutters":["CodeMirror-linenumbers","CodeMirror-foldgutter"],"autoCloseBrackets":true,"extraKeys":{"Ctrl-Space":"autocomplete",
"Cmd-Space":"autocomplete","Cmd-/":"toggleComment","Ctrl-/":"toggleComment"},"onLoad":function(cm_){$scope.$watch("JsModel",function(value,old){oVFB.log("jsmode model");oVFB.setJs(value)});$scope.$on("updateJS",function(){oVFB.log("Js");$scope["JsModel"]=oVFB.getJs()})}}};goog.provide("nsVFB.nsVisualizerMode.JsonMode");nsVFB.nsVisualizerMode.JsonMode=function(){oVFB.log("nsVFB.nsVisualizerMode.JsonMode");oVFB.module.directive("appVisualizermodeJson",this.visualizerJsonModeDirective);oVFB.module.controller("AppVisualizermodeJsonController",["$scope","$timeout","$rootScope",this.visualizerJsonModeController])};
...
"lineSeparator":null,"indentWithTabs":true,"tabSize":2,"viewportMargin":50,"theme":"Veremes","foldGutter":true,"gutters":["CodeMirror-linenumbers","CodeMirror-foldgutter"],"autoCloseBrackets":true,"onLoad":function(cm_){$scope.$on("updateEvent",function(event,data){cm_["setOption"]("readOnly",$("#Lock_Icon").hasClass("fa-lock"));$scope["JsonModel"]=oVFB.JsonStringifier(oVFB.getJsonOutput())});$scope.$watch("JsonModel",function(value,old){oVFB.log("jsonmode model");var error=false;try{JSON.parse(value)}catch(e){error=
true}finally{$scope.$parent["error_Json"]=error}});$scope.$on("updateJsonModel",function(event,data){if($scope["$parent"]["ctrl"]["json_"]){$scope.moment_=Date.now();$timeout(function(){if(Date.now()-$scope.moment_>=$scope.labs_)$scope["JsonModel"]=oVFB.JsonStringifier(oVFB.getJsonOutput())},$scope.labs_)}})}}};
