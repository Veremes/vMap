...
nsVFB.ElementForm.prototype.elementFormController=function($scope,$rootScope,$timeout,$http,$q){oVFB.log("nsVFB.ElementForm.elementFormController");var elementFormController=this;this["applicationName"]=oVFB.getApplication();$rootScope["Utils"]={"keys":Object.keys};$scope["model"]={};$scope["collapsed"]=oVFB.Collapse_[4];$scope["aWebServiceAttributs"]=[];$scope["aBusinessObjects"]=[];$scope["aBoAttributs"]=[];$scope["aFormAttributs"]=[];$scope.tmp={};var selectedElement={};var selectedElementInTreeview=
{};$scope.$watch("text",function(){oVFB.log("text updated");if(typeof $scope["text"]["Element"]!=="undefined"){elementFormController["text"]=$scope["text"]["Element"];$scope["Component_list"]=[];var aAcceptedTypes=oVFB["config"]["avaliable_elems"];for(var i=0;i<aAcceptedTypes.length;i++)$scope["Component_list"].push({"alias":elementFormController["text"]["Select_Type"][aAcceptedTypes[i]],"value":aAcceptedTypes[i]})}});$scope.$watch("model.id",function(){oVFB.log("model.id changed");$scope["jsonOutput"]=
oVFB.getJsonOutput();if(!goog.isDef($scope["jsonOutput"]["datasources"])||!goog.isObject($scope["jsonOutput"]["datasources"])||goog.isArray($scope["jsonOutput"]["datasources"]))$scope["jsonOutput"]["datasources"]={};$scope["datasources"]=$scope["jsonOutput"]["datasources"];elementFormController["updateDatasourceInfos"]();if($scope["model"]["type"]==="select"||$scope["model"]["type"]==="list"||$scope["model"]["type"]==="double_select")if(goog.isDefAndNotNull($scope["model"]["datasource"]["parents"])){$scope["cascade"]=
...
oVFB.getJsonOutput();var aElems=oVFB.utils_.getElementsByName($scope["jsonOutput"],$rootScope["selected_form_type"],current);if(aElems.length>1){$scope["model"]["name"]=$scope["model"]["name"]+"_2";$.notify($scope["text"]["Element"]["SameNameWarning"],"warning")}});$scope.$watch("model",function(current,old){$rootScope.$broadcast("update model",null);if(old["type"]!=="undefined")if(current["type"]!==old["type"]&&current["id"]===old["id"]){if(selectedElement.length>0){if(goog.isDefAndNotNull(selectedElement[0]))selectedElement[0]["icon"]=
elementFormController.convert($scope["model"],current["type"]);if(goog.isDefAndNotNull(selectedElementInTreeview[0]))selectedElementInTreeview[0]["icon"]=elementFormController.convert($scope["model"],current["type"])}if(current["type"]==="select")if(elementFormController["optionsSelect"]==="Text"){if(goog.isDef(current["web_service"]))goog.object.remove(current,"web_service");$scope["selectedRessource"]="";$scope["selectedValue"]="";$scope["selectedLabel"]=""}else if(elementFormController["optionsSelect"]===
"TableValues"){if(goog.isDef(current["options"]))goog.object.remove(current,"options");$scope["selectedRessource"]="";$scope["selectedValue"]="";$scope["selectedLabel"]=""}else if(elementFormController["optionsSelect"]==="WebService"){if(goog.isDef(current["options"]))goog.object.remove(current,"options");if(goog.isDef(current["web_service"]["parameters"]))goog.object.remove(current["web_service"],"parameters")}}else if(current["id"]===old["id"]&&current["name"]!==old["name"]){if(goog.isDef(selectedElement[0])&&
goog.isDef(selectedElementInTreeview[0])){selectedElement[0]["text"]=current["name"];selectedElementInTreeview[0]["text"]=current["name"];if(current["name"].match(/\s/g)!==null)angular.element($("#Element_Form_name_input")).addClass("invalid_input");else angular.element($("#Element_Form_name_input")).removeClass("invalid_input")}oVFB.updateElemNameInTabs(current,old);oVFB.updateElemNameInWab(current,old)}else if(current["id"]===old["id"]);$scope["model"]["selected"]=true;$timeout(function(){if(goog.isDef(selectedElement[0])&&
goog.isDef(selectedElementInTreeview[0])){if(goog.isDefAndNotNull(selectedElement[0]))$("#treeWithLines")["treeview"]("selectNode",[selectedElement[0]["nodeId"],{silent:false}]);if(goog.isDefAndNotNull(selectedElementInTreeview[0]))$("#treeView")["treeview"]("selectNode",[selectedElementInTreeview[0]["nodeId"],{silent:false}])}},10);$rootScope.$broadcast("updateJsonModel",oVFB.getJsonOutput());$rootScope.$broadcast("updateForm",oVFB.getJsonOutput())},true);$scope.$on("update element",function(event,
data){oVFB.log("update element");$scope["model"]=data;selectedElement=$("#treeWithLines")["treeview"]("getSelected");selectedElementInTreeview=$("#treeView")["treeview"]("getSelected");if(goog.isDef($scope.model))$("#select_element_to_add").val($scope.model.type);$scope["AllSelects"]=[];if(typeof oVFB.getJsonOutput()[$rootScope["selected_form_type"]]!=="undefined"){var obj=oVFB.getJsonOutput()[$rootScope["selected_form_type"]]["rows"];for(var i=0;i<obj.length;i++)for(var j=0;j<obj[i]["fields"].length;j++)if((obj[i]["fields"][j]["type"]===
"select"||obj[i]["fields"][j]["type"]==="list")&&obj[i]["fields"][j]["name"]!==$scope["model"]["name"])$scope["AllSelects"].push(obj[i]["fields"][j]["name"])}if($("#load_Perso_button").hasClass("active")&&oVFB.valid&&data["type"]!=="undefined")$scope["locker"]=false;else if(oVFB.valid){$scope["locker"]=true;if($scope["model"]["type"]==="map_vmap")$("#Vmap_Downloader_input").fileinput("disable")}if(!$scope.$$phase)$scope.$digest()});elementFormController.updateWSInfos=function(value){oVFB.log("elementFormController.updateWSInfos");
if(typeof value["selectedValue"]!=="undefined")$scope["model"]["web_service"]["id_key"]=value["selectedValue"].replace(/\s/g,"");if(typeof value["selectedLabel"]!=="undefined")$scope["model"]["web_service"]["label_key"]=value["selectedLabel"].replace(/\s/g,"");$scope["model"]["web_service"]["optionsSelect"]=elementFormController["optionsSelect"]};elementFormController["updateDatasourceInfos"]=function(){oVFB.log("elementFormController.updateDatasourceInfos");if(!goog.isDefAndNotNull($scope["model"]["datasource"]))return 0;
...
function(datasource){oVFB.log("updateWebServiceAttributs");var params={"token":oVFB.getToken(),"limit":1};if(goog.isObject(datasource["parameters"]))goog.object.extend(params,datasource["parameters"]);var bRequestOk=false;var canceler=$q.defer();$http({method:"GET",url:oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/"+datasource["ressource_id"],params:params,timeout:canceler.promise,headers:{"Accept":"application/x-vm-json"}}).then(function successCallback(response){bRequestOk=
true;if(!goog.isDefAndNotNull(response))return 0;if(!goog.isDefAndNotNull(response["data"]))return 0;if(!goog.isDefAndNotNull(response["data"]["data"]))return 0;if(!goog.isDefAndNotNull(response["data"]["data"][0]))return 0;var data;if(datasource["dataType"]==="businessObject"&&goog.isDefAndNotNull(datasource["businessObjectRequest"])){data=[];for(var i=0;i<response["data"]["data"].length;i++)data.push(response["data"]["data"][i]["bo_"+datasource["businessObjectRequest"]])}else data=response["data"]["data"];
$scope["aWebServiceAttributs"]=Object.keys(data[0])},function errorCallback(response){bRequestOk=true;console.error(response)});var showCancelRequestMessageAfterTimeout=function(){setTimeout(function(){if(bRequestOk===false)bootbox.confirm(elementFormController["text"]["Component"]["Select"]["Cancel"],function(result){if(result===true)canceler.resolve();else showCancelRequestMessageAfterTimeout()})},5E3)};showCancelRequestMessageAfterTimeout()};elementFormController["updateCascadeField"]=function(){oVFB.log("elementFormController.updateCascadeField");
if(!goog.isDefAndNotNull($scope["aParents"])){console.error("aParents not defined");return 0}if(!goog.isDefAndNotNull($scope["model"]["datasource"])){console.error("datasource not defined");return 0}if($scope["cascade"]===false)$scope["aParents"]=[];if(goog.isArray($scope["aParents"])){if($scope["aParents"].length===0){if(goog.isDef($scope["model"]["datasource"]["parents"]))delete $scope["model"]["datasource"]["parents"];return 0}if(!goog.isDefAndNotNull($scope["model"]["datasource"]["parents"]))$scope["model"]["datasource"]["parents"]=
[];if(goog.isArray($scope["model"]["datasource"]["parents"]))goog.array.clear($scope["model"]["datasource"]["parents"]);for(var i=0;i<$scope["aParents"].length;i++)$scope["model"]["datasource"]["parents"].push({"name":$scope["aParents"][i],"filter_attr":$scope["aParents"][i],"filter_equality":"=","wait_for_parent":true})}};elementFormController.updateVmapData=function(tree){oVFB.log("elementFormController.updateVmapData");if(tree["children"]){for(var i=0;i<tree["children"].length;i++)if(tree["children"][i]["view"]){$scope["model"]["map_options"]["center"]["coord"]=
tree["children"][i]["view"]["center"]?tree["children"][i]["view"]["center"]:[0,0];$scope["model"]["map_options"]["center"]["zoom"]=tree["children"][i]["view"]["zoom"]?tree["children"][i]["view"]["zoom"]:5;$scope["model"]["map_options"]["center"]["extent"]=tree["children"][i]["view"]["extent"]?tree["children"][i]["view"]["extent"]:[-2.002637639E7,-2.00489661E7,2.002637639E7,2.00489661E7];$scope["model"]["map_options"]["proj"]=tree["children"][i]["view"]["projection"]?tree["children"][i]["view"]["projection"]:
"EPSG:3857";$rootScope.$broadcast("updateMap","view")}$scope["model"]["map_options"]["tree"]=tree;$rootScope.$broadcast("updateMap","tree")}};elementFormController.updateWS=function(){oVFB.log("elementFormController.updateWS");$rootScope.$broadcast("updateOptions",{})};elementFormController["updateMap"]=function(carte,option){oVFB.log("elementFormController.updateMap");switch(option){case "center":var coord=ol.proj.transform($scope["model"]["map_options"]["center"]["coord"],$scope["model"]["map_options"]["proj"],
"EPSG:3857");var boundary=[-2.002637639E7,-2.00489661E7,2.002637639E7,2.00489661E7];if(!ol.extent.containsCoordinate(boundary,coord))$["notify"](elementFormController["text"]["Component"]["Map"]["Notify"]["OutRange"],{"className":"error","autoHideDelay":oVFB.TimeNotify_});else $rootScope.$broadcast("updateMap",option);break;case "zoom":if($scope["model"]["map_options"]["center"]["scale"]>0&&$scope["model"]["map_options"]["center"]["scale"]<=2E8)$rootScope.$broadcast("updateMap",option);else $["notify"](elementFormController["text"]["Component"]["Map"]["Notify"]["OutRange"],
{"className":"error","autoHideDelay":oVFB.TimeNotify_});break;case "extent":var extent=$scope.model.map_options.center.extent;if(extent[0]<=extent[2]&&extent[0]>=-2.002637639E7&&extent[0]<=2.002637639E7)if(extent[0]<=extent[2]&&extent[2]>=-2.002637639E7&&extent[2]<=2.002637639E7)if(extent[1]<=extent[3]&&extent[1]>=-2.00489661E7&&extent[1]<=2.00489661E7)if(extent[1]<=extent[3]&&extent[3]>=-2.00489661E7&&extent[3]<=2.00489661E7){$rootScope.$broadcast("updateMap",option);break}$["notify"](elementFormController["text"]["Component"]["Map"]["Notify"]["OutRange"],
{"className":"error","autoHideDelay":oVFB.TimeNotify_});break;case "multi_geom":break;default:$rootScope.$broadcast("updateMap",option);break}};elementFormController.convert=function(src,dst){oVFB.log("elementFormController.convert");var fields;var icon="icon-point";if(src["type"]!=="undefined"){switch(dst){case "button":fields=["type","id","class","name","nb_cols","buttons","default_value"];break;case "title":case "label":fields=["type","id","label","name","default_value","nb_cols"];dst==="label"?
icon="icon-eye":icon;break;case "hr":fields=["type","id","name","default_value"];dst==="hr"?icon="icon-minus":icon;break;case "hidden":fields=["type","id","name","default_value","nb_cols"];dst==="hidden"?icon="icon-user-secret":icon;break;case "subtitle":fields=["type","id","label","name","class","default_value","nb_cols"];break;case "checkbox":fields=["type","id","label","name","checked","default_value","nb_cols"];icon="icon-check-square";break;case "radio":fields=["type","id","label","name","checked",
"options","nb_cols","default_value"];icon="icon-dot-circle-o";break;case "textarea":fields=["type","id","label","name","required","nb_cols","default_value"];icon="icon-paragraph";break;case "select":fields=["type","id","label","name","required","nb_cols","multiple","options","web_service","child_select","default_value"];icon="icon-format_list_bulleted";break;case "list":fields=["type","id","label","name","required","nb_cols","multiple","options","web_service","child_select","default_value","size"];
icon="icon-format_list_bulleted";break;case "double_select":fields=["type","id","label","name","required","nb_cols","options","web_service","size","label_from","label_to","name_from","name_to","default_value"];icon="icon-format_list_bulleted";break;case "date":case "datetime":case "label_display":case "color_picker":fields=["type","id","label","name","nb_cols","default_value"];dst==="date"||dst==="datetime"?icon="icon-calendar-check-o":icon;dst==="label_display"?icon="icon-eye":icon;dst==="color_picker"?
icon="icon-paint":icon;break;case "upload":fields=["type","id","label","name","nb_cols","default_value","ext"];icon="icon-file";break;case "tinymce":fields=["type","id","label","name","nb_rows","options","default_value","nb_cols"];icon="icon-font";break;case "slider":fields=["type","id","label","name","options","default_value","nb_cols"];icon="icon-sliders";break;case "text":case "password":case "url":case "email":fields=["type","id","label","name","pattern","required","nb_cols","default_value"];
dst==="text"?icon="icon-font":icon;dst==="password"?icon="icon-key":icon;dst==="url"?icon="icon-chrome":icon;dst==="email"?icon="icon-at":icon;break;case "integer":case "float":case "number":fields=["type","id","label","name","min","max","nb_cols","default_value"];icon="icon-calculator";break;case "link":fields=["type","id","label","name","alias","folder","nb_cols","target"];icon="glyphicon glyphicon-link";break;case "image":fields=["type","id","label","name","alias","folder","nb_cols","width"];icon=
"glyphicon glyphicon-picture";break;case "map_osm":case "map_bing":case "map_vmap":fields=["type","id","label","name","required","map_options","nb_cols","style","default_value"];icon="icon-map";break;case "codemirror":fields=["type","id","label","ui","name","cm_options","nb_cols"];icon="icon-code";break}for(var srckey in src)if(goog.isDefAndNotNull(fields))if(fields.indexOf(srckey)===-1)delete src[srckey];switch(dst){case "list":case "double_select":var typic={"size":5};break;case "button":goog.isDef(src["class"])?
src["class"]:src["class"]="btn-ungroup btn-group-sm";goog.isDef(src["buttons"])?src["buttons"]:src["buttons"]=[];var typic={"type":"submit","name":"form_submit","label":"FORM_UPDATE","class":"btn-primary"};goog.isDef(src["buttons"][0])?src["buttons"][0]:src["buttons"][0]=typic;break;case "radio":var typic={"choices":[{label:"Oui",value:true},{label:"Non",value:false}]};goog.isDef(src["options"])?src["options"]:src["options"]=typic;goog.isDef(src["nb_cols"])?src["nb_cols"]:src["nb_cols"]=12;break;
case "slider":var typic={"min":0,"max":100,"precision":1,"step":1};goog.isDef(src["options"])?src["options"]:src["options"]=typic;goog.isDef(src["nb_cols"])?src["nb_cols"]:src["nb_cols"]=12;break;case "map_osm":goog.isDef(src["style"])?src["style"]:src["style"]={"height":"250px"};var typic={"proj":"EPSG:3857","type":"OSM","center":{"coord":[3E5,6E6],"zoom":5,"scale":25E6,"extent":[-2.002637639E7,-2.00489661E7,2.002637639E7,2.00489661E7]},"controls":{"MP":true,"ZO":true,"SL":true,"CP":true},"layers":[],
"interactions":{"multi_geometry":false,"full_screen":true,"RA":false,"RO":true,"ED":true,"DP":true,"DL":true,"DPol":true,"SE":true},"draw_color":"rgba(54,184,255,0.6)","contour_color":"rgba(0,0,0,0.4)","contour_size":2,"circle_radius":6,"features":[],"coord_accuracy":8};goog.isDef(src["map_options"])&&src["map_options"]["type"]==="OSM"?src["map_options"]:src["map_options"]=typic;break;case "map_bing":goog.isDef(src["style"])?src["style"]:src["style"]={"height":"250px"};var typic={"proj":"EPSG:3857",
"type":"bing","center":{"coord":[3E5,6E6],"zoom":5,"scale":25E6,"extent":[-2.002637639E7,-2.00489661E7,2.002637639E7,2.00489661E7]},"controls":{"MP":true,"ZO":true,"SL":true,"CP":true},"layers":[],"interactions":{"multi_geometry":false,"full_screen":true,"RA":false,"RO":true,"ED":true,"DP":true,"DL":true,"DPol":true,"SE":true},"draw_color":"rgba(54,184,255,0.6)","contour_color":"rgba(0,0,0,0.4)","contour_size":2,"circle_radius":6,"features":[],"coord_accuracy":8,"source":{"culture":"fr-FR","key":"",
"style":"Aerial"}};goog.isDef(src["map_options"])&&src["map_options"]["type"]==="bing"?src["map_options"]:src["map_options"]=typic;break;case "map_vmap":goog.isDef(src["style"])?src["style"]:src["style"]={"height":"250px"};var typic={"proj":"EPSG:3857","type":"vmap","center":{"coord":[3E5,6E6],"zoom":5,"scale":25E6,"extent":[-2.002637639E7,-2.00489661E7,2.002637639E7,2.00489661E7]},"controls":{"MP":true,"ZO":true,"SL":true,"CP":true},"layers":[],"interactions":{"multi_geometry":false,"full_screen":true,
"RA":false,"RO":true,"ED":true,"DP":true,"DL":true,"DPol":true,"SE":true},"draw_color":"rgba(54,184,255,0.6)","contour_color":"rgba(0,0,0,0.4)","contour_size":2,"circle_radius":6,"features":[],"coord_accuracy":8};goog.isDef(src["map_options"])&&src["map_options"]["type"]==="vmap"?src["map_options"]:src["map_options"]=typic;break;case "codemirror":var typic={"mode":"javascript","readOnly":false,"styleActiveLine":true,"lineNumbers":true,"lineSeparator":"\r\n","indentWithTabs":true,"tabSize":2,"viewportMargin":50,
"theme":"Veremes"};goog.isDef(src["cm_options"])?src["cm_options"]:src["cm_options"]=typic;goog.isDef(src["nb_cols"])?src["nb_cols"]:src["nb_cols"]=12;goog.isDef(src["ui"])?src["ui"]:src["ui"]=true;break;case "bo_grid":var typic={"enableRowSelection":true,"enableSelectAll":true,"enablePagination":false,"enablePaginationControls":false,"enableColumnMenus":false,"enableColumnResizing":false,"appHeader":true,"appHeaderTitleBar":true,"columnDefs":[],"data":[]};goog.isDef(src["options"])?src["options"]:
src["options"]=typic;goog.isDef(src["nb_cols"])?src["nb_cols"]:src["nb_cols"]=12;setTimeout(function(){oVFB.reloadForm()});break;case "section_grid":var typic={"enableRowSelection":true,"enableSelectAll":true,"enablePagination":false,"enablePaginationControls":false,"enableColumnMenus":false,"enableColumnResizing":false,"appHeader":true,"appHeaderTitleBar":true,"columnDefs":[],"data":[]};goog.isDef(src["options"])?src["options"]:src["options"]=typic;goog.isDef(src["nb_cols"])?src["nb_cols"]:src["nb_cols"]=
12;setTimeout(function(){oVFB.reloadForm()});break;default:if(!src["nb_cols"])src["nb_cols"]=12;break}return icon}};elementFormController["resize"]=function(){$scope["collapsed"]=!$scope["collapsed"];oVFB.Collapse_[4]=$scope["collapsed"];oVFB.resize()};elementFormController["updateBoList"]=function(){$scope["aBusinessObjects"]=[];var params={"token":oVFB.getToken()};var bRequestOk=false;var canceler=$q.defer();$http({method:"GET",url:oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+
"/vmap/businessobjects",params:params,timeout:canceler.promise,headers:{"Accept":"application/x-vm-json"}}).then(function successCallback(response){bRequestOk=true;if(!goog.isDefAndNotNull(response))return 0;if(!goog.isDefAndNotNull(response["data"]))return 0;if(!goog.isDefAndNotNull(response["data"]["data"]))return 0;if(!goog.isDefAndNotNull(response["data"]["data"][0]))return 0;$scope["aBusinessObjects"]=response["data"]["data"]},function errorCallback(response){bRequestOk=true;console.error(response)});
var showCancelRequestMessageAfterTimeout=function(){setTimeout(function(){if(bRequestOk===false)bootbox.confirm(elementFormController["text"]["Component"]["Select"]["Cancel"],function(result){if(result===true)canceler.resolve();else showCancelRequestMessageAfterTimeout()})},5E3)};showCancelRequestMessageAfterTimeout()};elementFormController["updateBoLinkLists"]=function(sBoId){if(!goog.isDefAndNotNull(sBoId))return 0;var aBoFields=[];var aFormFields=[];var params={"token":oVFB.getToken()};var bRequestOk=
false;var canceler=$q.defer();$http({method:"GET",url:oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/vmap/querys/"+sBoId,params:params,timeout:canceler.promise,headers:{"Accept":"application/x-vm-json"}}).then(function successCallback(response){bRequestOk=true;if(!goog.isDefAndNotNull(response))return 0;if(!goog.isDefAndNotNull(response["data"]))return 0;if(!goog.isDefAndNotNull(response["data"]["data"]))return 0;if(!goog.isDefAndNotNull(response["data"]["data"][0]))return 0;
if(!goog.isDefAndNotNull(response["data"]["data"][0]))return 0;var aBoFields=Object.keys(response["data"]["data"][0]);$scope["aBoAttributs"]=aBoFields},function errorCallback(response){bRequestOk=true;console.error(response)});var showCancelRequestMessageAfterTimeout=function(){setTimeout(function(){if(bRequestOk===false)bootbox.confirm(elementFormController["text"]["Component"]["Select"]["Cancel"],function(result){if(result===true)canceler.resolve();else showCancelRequestMessageAfterTimeout()})},
5E3)};showCancelRequestMessageAfterTimeout();if(typeof oVFB.getJsonOutput()[$rootScope["selected_form_type"]]!=="undefined"){var obj=oVFB.getJsonOutput()[$rootScope["selected_form_type"]]["rows"];for(var i=0;i<obj.length;i++)for(var j=0;j<obj[i]["fields"].length;j++)aFormFields.push(obj[i]["fields"][j]["name"])}$scope["aFormAttributs"]=aFormFields};elementFormController["updateSectionList"]=function(){$scope["aBusinessObjects"]=[];var params={"token":oVFB.getToken(),"filter":'"template"=\'workspaceListTpl.html\' AND "lang"=\''+
oVFB.AppProperties.language+"'"};var bRequestOk=false;var canceler=$q.defer();$http({method:"GET",url:oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/vitis/vitissections",params:params,timeout:canceler.promise,headers:{"Accept":"application/x-vm-json"}}).then(function successCallback(response){bRequestOk=true;if(!goog.isDefAndNotNull(response))return 0;if(!goog.isDefAndNotNull(response["data"]))return 0;if(!goog.isDefAndNotNull(response["data"]["data"]))return 0;if(!goog.isDefAndNotNull(response["data"]["data"][0]))return 0;
$scope["aSections"]=response["data"]["data"]},function errorCallback(response){bRequestOk=true;console.error(response)});var showCancelRequestMessageAfterTimeout=function(){setTimeout(function(){if(bRequestOk===false)bootbox.confirm(elementFormController["text"]["Component"]["Select"]["Cancel"],function(result){if(result===true)canceler.resolve();else showCancelRequestMessageAfterTimeout()})},5E3)};showCancelRequestMessageAfterTimeout()};elementFormController["updateSectionLinkLists"]=function(sSectionId){$scope["aBusinessObjects"]=
[];var params={"token":oVFB.getToken()};var bRequestOk=false;var canceler=$q.defer();$http({method:"GET",url:oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/vitis/vitissections/"+sSectionId,params:params,timeout:canceler.promise,headers:{"Accept":"application/x-vm-json"}}).then(function successCallback(response){bRequestOk=true;if(!goog.isDefAndNotNull(response))return 0;if(!goog.isDefAndNotNull(response["data"]))return 0;if(!goog.isDefAndNotNull(response["data"]["data"]))return 0;
if(!goog.isDefAndNotNull(response["data"]["data"][0]))return 0;var oSection=response["data"]["data"][0];if(goog.isDefAndNotNull(oSection["ressource_id"])){$http({method:"GET",url:oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/"+oSection["ressource_id"],params:params,timeout:canceler.promise,headers:{"Accept":"application/x-vm-json"}}).then(function successCallback(response){bRequestOk=true;if(!goog.isDefAndNotNull(response))return 0;if(!goog.isDefAndNotNull(response["data"]))return 0;
...
-1)$scope["wabGroups"].push(group);$scope["wab"]=oVFB.checkElemsOnWab($scope["wab"],$scope["elems"]);console.log("$scope['wab']: ",$scope["wab"]);$("#studio-wabmanager-modal").modal("show")};
...
"/fields?token="+oVFB.getToken();var this_=this;if(goog.isDef(this_["aSuggestedTableFields"]))goog.array.clear(this_["aSuggestedTableFields"]);this.$http_({url:url,method:"GET"}).success(function(data){console.log(data);if(!goog.isArray(data)||data.length===0){var $oMainScope=angular.element(vitisApp.appMainDrtv).scope();$oMainScope.$root["modalWindow"]("confirm",this_.$scope_["text"]["Loader"]["Notify"]["generate_form_error"],{"className":"modal-danger","message":this_.$scope_["text"]["Loader"]["Notify"]["generate_form_error_description"],
...
nsUtils.reloadForm=function(){nsUtils.log("util.reloadForm");var oFormReaderScope=angular.element($("#studio_form_reader").children()).scope();oFormReaderScope["oFormValues"][oFormReaderScope["sFormDefinitionName"]]={};setTimeout(function(){oFormReaderScope["compileTemplate"]();setTimeout(function(){oFormReaderScope["ctrl"].useDefaultValues(true);setTimeout(function(){oFormReaderScope["ctrl"].extractFormDefinitionInfos()},100)},100)},100)};nsUtils.clone=function(obj){nsUtils.log("util.clone");return obj};
