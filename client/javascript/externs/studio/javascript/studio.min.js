...
{};$scope["datasources"]=$scope["jsonOutput"]["datasources"];$scope["datasourcesArray"]=[];for(var key in $scope["datasources"]){$scope["datasources"][key]["id"]=key;$scope["datasourcesArray"].push($scope["datasources"][key])}elementFormController["updateDatasourceInfos"]();if($scope["model"]["type"]==="select"||$scope["model"]["type"]==="list"||$scope["model"]["type"]==="double_select")if(goog.isDefAndNotNull($scope["model"]["datasource"]["parents"])){$scope["cascade"]=true;$scope["aParents"]=[];
...
var datasource=$scope["datasources"][$scope["model"]["datasource"]["datasource_id"]];if(datasource["type"]==="web_service")goog.array.clear($scope["aWebServiceAttributs"]);elementFormController["updateDatasourceInfos"]()});$scope.$watch("model.default_value",function(){$rootScope.$broadcast("updateDefautValue",$scope["model"])});$scope.$watch("model.name",function(current,old){$scope["jsonOutput"]=oVFB.getJsonOutput();var aElems=oVFB.utils_.getElementsByName($scope["jsonOutput"],$rootScope["selected_form_type"],
current);if(aElems.length>1){$scope["model"]["name"]=$scope["model"]["name"]+"_2";$.notify($scope["text"]["Element"]["SameNameWarning"],"warning")}});$scope.$watch("model",function(current,old){$rootScope.$broadcast("update model",null);if(goog.isDefAndNotNull(current["type"])&&goog.isDefAndNotNull(current["id"]))if(current["type"]!==old["type"]&&current["id"]===old["id"]){oVFB.Update=true;if(selectedElement.length>0){if(goog.isDefAndNotNull(selectedElement[0]))selectedElement[0]["icon"]=elementFormController.convert($scope["model"],
current["type"]);if(goog.isDefAndNotNull(selectedElementInTreeview[0]))selectedElementInTreeview[0]["icon"]=elementFormController.convert($scope["model"],current["type"])}if(current["type"]==="select")if(elementFormController["optionsSelect"]==="Text"){if(goog.isDef(current["web_service"]))goog.object.remove(current,"web_service");$scope["selectedRessource"]="";$scope["selectedValue"]="";$scope["selectedLabel"]=""}else if(elementFormController["optionsSelect"]==="TableValues"){if(goog.isDef(current["options"]))goog.object.remove(current,
"options");$scope["selectedRessource"]="";$scope["selectedValue"]="";$scope["selectedLabel"]=""}else if(elementFormController["optionsSelect"]==="WebService"){if(goog.isDef(current["options"]))goog.object.remove(current,"options");if(goog.isDef(current["web_service"]["parameters"]))goog.object.remove(current["web_service"],"parameters")}}else if(current["id"]===old["id"]&&current["name"]!==old["name"]){oVFB.Update=true;if(goog.isDef(selectedElement[0])&&goog.isDef(selectedElementInTreeview[0])){selectedElement[0]["text"]=
current["name"];selectedElementInTreeview[0]["text"]=current["name"];if(current["name"].match(/\s/g)!==null)angular.element($("#Element_Form_name_input")).addClass("invalid_input");else angular.element($("#Element_Form_name_input")).removeClass("invalid_input")}oVFB.updateElemNameInTabs(current,old);oVFB.updateElemNameInWab(current,old)}else if(current["id"]===old["id"]);$scope["model"]["selected"]=true;$timeout(function(){if(goog.isDef(selectedElement[0])&&goog.isDef(selectedElementInTreeview[0])){if(goog.isDefAndNotNull(selectedElement[0]))$("#treeWithLines")["treeview"]("selectNode",
[selectedElement[0]["nodeId"],{silent:false}]);if(goog.isDefAndNotNull(selectedElementInTreeview[0]))$("#treeView")["treeview"]("selectNode",[selectedElementInTreeview[0]["nodeId"],{silent:false}])}},10);$rootScope.$broadcast("updateJsonModel",oVFB.getJsonOutput());$rootScope.$broadcast("updateForm",oVFB.getJsonOutput())},true);$scope.$on("update element",function(event,data){oVFB.log("update element");$scope["model"]=data;selectedElement=$("#treeWithLines")["treeview"]("getSelected");selectedElementInTreeview=
$("#treeView")["treeview"]("getSelected");if(goog.isDef($scope.model))$("#select_element_to_add").val($scope.model.type);$scope["AllSelects"]=[];if(goog.isDefAndNotNull(oVFB.getJsonOutput()[$rootScope["selected_form_type"]]))if(!goog.object.isEmpty(oVFB.getJsonOutput()[$rootScope["selected_form_type"]]["rows"])){var obj=oVFB.getJsonOutput()[$rootScope["selected_form_type"]]["rows"];for(var i=0;i<obj.length;i++)for(var j=0;j<obj[i]["fields"].length;j++)if((obj[i]["fields"][j]["type"]==="select"||obj[i]["fields"][j]["type"]===
"list")&&obj[i]["fields"][j]["name"]!==$scope["model"]["name"])$scope["AllSelects"].push(obj[i]["fields"][j]["name"])}if($("#load_Perso_button").hasClass("active")&&oVFB.valid&&data["type"]!=="undefined")$scope["locker"]=false;else if(oVFB.valid){$scope["locker"]=true;if($scope["model"]["type"]==="map_vmap")$("#Vmap_Downloader_input").fileinput("disable")}if(!$scope.$$phase)$scope.$digest()});elementFormController.updateWSInfos=function(value){oVFB.log("elementFormController.updateWSInfos");if(typeof value["selectedValue"]!==
"undefined")$scope["model"]["web_service"]["id_key"]=value["selectedValue"].replace(/\s/g,"");if(typeof value["selectedLabel"]!=="undefined")$scope["model"]["web_service"]["label_key"]=value["selectedLabel"].replace(/\s/g,"");$scope["model"]["web_service"]["optionsSelect"]=elementFormController["optionsSelect"]};elementFormController["updateDatasourceInfos"]=function(){oVFB.log("elementFormController.updateDatasourceInfos");if(!goog.isDefAndNotNull($scope["model"]["datasource"]))return 0;$scope["model"]["datasource"]["sort_order"]=
goog.isDefAndNotNull($scope["model"]["datasource"]["sort_order"])?$scope["model"]["datasource"]["sort_order"]:"ASC";$scope["model"]["datasource"]["distinct"]=goog.isDefAndNotNull($scope["model"]["datasource"]["distinct"])?$scope["model"]["datasource"]["distinct"]:"true";if(goog.isDefAndNotNull($scope["model"]["datasource"]["id_key"]))$scope["model"]["datasource"]["id_key"]=$scope["model"]["datasource"]["id_key"].replace(/\s/g,"");if(goog.isDefAndNotNull($scope["model"]["datasource"]["label_key"]))$scope["model"]["datasource"]["label_key"]=
$scope["model"]["datasource"]["label_key"].replace(/\s/g,"");if(goog.isDefAndNotNull($scope["model"]["datasource"]["id_key"])&&goog.isDefAndNotNull($scope["model"]["datasource"]["label_key"]))$scope["model"]["datasource"]["attributs"]=$scope["model"]["datasource"]["id_key"]+"|"+$scope["model"]["datasource"]["label_key"];if(goog.isDefAndNotNull($scope["model"]["datasource"]["label_key"]))$scope["model"]["datasource"]["order_by"]=$scope["model"]["datasource"]["label_key"]};elementFormController["updateWebServiceAttributs"]=
function(datasource){oVFB.log("updateWebServiceAttributs");var params={"limit":1};if(goog.isObject(datasource["parameters"]))goog.object.extend(params,datasource["parameters"]);console.log(datasource["parameters"]);var canceler=$q.defer();ajaxRequest({"method":"GET","url":oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/"+datasource["ressource_id"],"headers":{"Accept":"application/x-vm-json"},"params":params,"scope":$scope,"timeout":5E3,"abord":canceler.promise,"success":function(response){if(!goog.isDefAndNotNull(response))return 0;
...
goog.isDef(src["options"])?src["options"]:src["options"]=typic;goog.isDef(src["nb_cols"])?src["nb_cols"]:src["nb_cols"]=12;break;case "slider":var typic={"min":0,"max":100,"precision":1,"step":1};goog.isDef(src["options"])?src["options"]:src["options"]=typic;goog.isDef(src["nb_cols"])?src["nb_cols"]:src["nb_cols"]=12;break;case "map_osm":goog.isDef(src["style"])?src["style"]:src["style"]={"height":"250px"};var typic={"proj":"EPSG:3857","type":"OSM","center":{"coord":[3E5,6E6],"zoom":5,"scale":25E6,
"extent":[-2.002637639E7,-2.00489661E7,2.002637639E7,2.00489661E7]},"controls":{"MP":true,"ZO":true,"SL":true,"CP":true},"layers":[],"interactions":{"multi_geometry":false,"full_screen":true,"RA":false,"RO":true,"ED":true,"DP":true,"DL":true,"DPol":true,"SE":true},"draw_color":"rgba(54,184,255,0.6)","contour_color":"rgba(0,0,0,0.4)","contour_size":2,"circle_radius":6,"features":[],"coord_accuracy":8};goog.isDef(src["map_options"])&&src["map_options"]["type"]==="OSM"?src["map_options"]:src["map_options"]=
typic;break;case "map_bing":goog.isDef(src["style"])?src["style"]:src["style"]={"height":"250px"};var typic={"proj":"EPSG:3857","type":"bing","center":{"coord":[3E5,6E6],"zoom":5,"scale":25E6,"extent":[-2.002637639E7,-2.00489661E7,2.002637639E7,2.00489661E7]},"controls":{"MP":true,"ZO":true,"SL":true,"CP":true},"layers":[],"interactions":{"multi_geometry":false,"full_screen":true,"RA":false,"RO":true,"ED":true,"DP":true,"DL":true,"DPol":true,"SE":true},"draw_color":"rgba(54,184,255,0.6)","contour_color":"rgba(0,0,0,0.4)",
"contour_size":2,"circle_radius":6,"features":[],"coord_accuracy":8,"source":{"culture":"fr-FR","key":"","style":"Aerial"}};goog.isDef(src["map_options"])&&src["map_options"]["type"]==="bing"?src["map_options"]:src["map_options"]=typic;break;case "map_vmap":goog.isDef(src["style"])?src["style"]:src["style"]={"height":"250px"};var typic={"proj":"EPSG:3857","type":"vmap","center":{"coord":[3E5,6E6],"zoom":5,"scale":25E6,"extent":[-2.002637639E7,-2.00489661E7,2.002637639E7,2.00489661E7]},"controls":{"MP":true,
"ZO":true,"SL":true,"CP":true},"layers":[],"interactions":{"multi_geometry":false,"full_screen":true,"RA":false,"RO":true,"ED":true,"DP":true,"DL":true,"DPol":true,"SE":true},"draw_color":"rgba(54,184,255,0.6)","contour_color":"rgba(0,0,0,0.4)","contour_size":2,"circle_radius":6,"features":[],"coord_accuracy":8};goog.isDef(src["map_options"])&&src["map_options"]["type"]==="vmap"?src["map_options"]:src["map_options"]=typic;break;case "codemirror":var typic={"mode":"javascript","readOnly":false,"styleActiveLine":true,
"lineNumbers":true,"lineSeparator":"\r\n","indentWithTabs":true,"tabSize":2,"viewportMargin":50,"theme":"Veremes"};goog.isDef(src["cm_options"])?src["cm_options"]:src["cm_options"]=typic;goog.isDef(src["nb_cols"])?src["nb_cols"]:src["nb_cols"]=12;goog.isDef(src["ui"])?src["ui"]:src["ui"]=true;break;case "bo_grid":var typic={"enableRowSelection":true,"enableSelectAll":true,"enablePagination":false,"enablePaginationControls":false,"enableColumnMenus":false,"enableColumnResizing":false,"appHeader":true,
"appHeaderTitleBar":true,"columnDefs":[],"data":[]};goog.isDef(src["options"])?src["options"]:src["options"]=typic;goog.isDef(src["nb_cols"])?src["nb_cols"]:src["nb_cols"]=12;setTimeout(function(){oVFB.reloadForm()});break;case "section_grid":var typic={"enableRowSelection":true,"enableSelectAll":true,"enablePagination":false,"enablePaginationControls":false,"enableColumnMenus":false,"enableColumnResizing":false,"appHeader":true,"appHeaderTitleBar":true,"columnDefs":[],"data":[]};goog.isDef(src["options"])?
src["options"]:src["options"]=typic;goog.isDef(src["nb_cols"])?src["nb_cols"]:src["nb_cols"]=12;setTimeout(function(){oVFB.reloadForm()});break;default:if(!src["nb_cols"])src["nb_cols"]=12;break}return icon}};elementFormController["resize"]=function(){$scope["collapsed"]=!$scope["collapsed"];oVFB.Collapse_[4]=$scope["collapsed"];oVFB.resize()};elementFormController["updateBoList"]=function(){$scope["aBusinessObjects"]=[];var canceler=$q.defer();ajaxRequest({"method":"GET","url":oVFB.AppProperties["web_server_name"]+
"/"+oVFB.AppProperties["services_alias"]+"/vmap/businessobjects","headers":{"Accept":"application/x-vm-json"},"scope":$scope,"timeout":5E3,"abord":canceler.promise,"success":function(response){if(!goog.isDefAndNotNull(response))return 0;if(!goog.isDefAndNotNull(response["data"]))return 0;if(!goog.isDefAndNotNull(response["data"]["data"]))return 0;if(!goog.isDefAndNotNull(response["data"]["data"][0]))return 0;$scope["aBusinessObjects"]=response["data"]["data"]}})};elementFormController["updateBoLinkLists"]=
function(sBoId){if(!goog.isDefAndNotNull(sBoId))return 0;var aBoFields=[];var aFormFields=[];var canceler=$q.defer();ajaxRequest({"method":"GET","url":oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/vmap/querys/"+sBoId,"headers":{"Accept":"application/x-vm-json"},"scope":$scope,"timeout":5E3,"abord":canceler.promise,"success":function(response){if(!goog.isDefAndNotNull(response))return 0;if(!goog.isDefAndNotNull(response["data"]))return 0;if(!goog.isDefAndNotNull(response["data"]["data"]))return 0;
...
[];$scope["databases"]=[];$scope["tables"]=[];$scope["selectedDatasource"]={};$scope["editedDatasource"]={};$scope["newSource"]={"options":["Value one|1","Value two|2","Value three|3"],"result_options":[{"label":"Value one","value":"1"},{"label":"Value two","value":"2"},{"label":"Value three","value":"3"}]};$scope["editorOptions"]={"mode":"schema","lineNumbers":false,"lineSeparator":"\n","theme":"Veremes","onLoad":function(cm_){}};$scope["studioAddModalTestGridOptions"]={"data":[],"enableColumnMenus":false,
...
"business_object_id":aBusinessObjects[i]["business_object_id"],"sql_form":aBusinessObjects[i]["sql_form"],"sql_summary":aBusinessObjects[i]["sql_summary"],"sql_list":aBusinessObjects[i]["sql_list"]})}});$("#studio-datasource-modal").on("hide.bs.modal",function(e){oVFB.unselectAll();this_.clearDatasourcesGrid();oVFB.reloadForm()});$("#studio-datasource-add-modal").on("hide.bs.modal",function(e){$scope["aExternalDatabaseTables"].length=0;$scope["databases"].length=0;$scope["schemas"].length=0;$scope["tables"].length=
0;this_.clearTestGrids()});$("#studio-datasource-edit-modal").on("hide.bs.modal",function(e){$scope["aExternalDatabaseTables"].length=0;$scope["databases"].length=0;$scope["schemas"].length=0;$scope["tables"].length=0;this_.clearTestGrids()});$scope.$on("updateForm",function(event,data){$scope.$applyAsync(function(){$scope["jsonOutput"]=data})});$scope.$on("updateTestTabForm",function(event,data){if(data[$scope["selected_form_type"]]){var oJson=data[$scope["selected_form_type"]];for(var i=0;i<oJson["rows"].length;i++)for(var j=
0;j<oJson["rows"][i]["fields"].length;j++)if(!oJson["rows"][i]["fields"][j]["id"])oJson["rows"][i]["fields"][j]["id"]=oJson["rows"][i]["fields"][j]["name"]+"_"+(i+1)+"_"+(j+1);$scope["oTestTabFormDefinition"]=data;$scope["sTestTabFormDefinitionName"]=$scope["selected_form_type"];$scope["title"]=oJson["title"];if(!goog.isDef($scope["oTestTabFormValues"]))$scope["oTestTabFormValues"]={};if(!goog.isDef($scope["oTestTabFormValues"][$scope["sTestTabFormDefinitionName"]]))$scope["oTestTabFormValues"][$scope["sTestTabFormDefinitionName"]]=
{};angular.element($("#studio_test_tabs_form_reader").children()).scope()["ctrl"].refreshForm()}else oVFB.log("visualizerFormModeController. Load JSON form fail: "+$scope["selected_form_type"]+" undefined in ",data)});$scope.$on("loadWABGroups",function(event,data){$scope["module"]=data["module"];$scope["object"]=data["object"];this_.loadWabGoups($scope["module"])});$scope.$on("updateTestWabForm",function(event,data){if(data[$scope["selected_form_type"]]){var oJson=data[$scope["selected_form_type"]];
for(var i=0;i<oJson["rows"].length;i++)for(var j=0;j<oJson["rows"][i]["fields"].length;j++)if(!oJson["rows"][i]["fields"][j]["id"])oJson["rows"][i]["fields"][j]["id"]=oJson["rows"][i]["fields"][j]["name"]+"_"+(i+1)+"_"+(j+1);$scope["oTestWabFormDefinition"]=data;$scope["sTestWabFormDefinitionName"]=$scope["selected_form_type"];$scope["title"]=oJson["title"];if(!goog.isDef($scope["oTestWabFormValues"]))$scope["oTestWabFormValues"]={};if(!goog.isDef($scope["oTestWabFormValues"][$scope["sTestWabFormDefinitionName"]]))$scope["oTestWabFormValues"][$scope["sTestWabFormDefinitionName"]]=
{};var oFormReaderScope=angular.element($("#studio_test_wab_form_reader").children()).scope();if(goog.isDefAndNotNull(oFormReaderScope))oFormReaderScope.$applyAsync(function(){oFormReaderScope["wabState"]=$scope["wabSelectedState"];oFormReaderScope["wabGroup"]=$scope["wabSelectedGroup"];oFormReaderScope["ctrl"].refreshForm()})}else oVFB.log("visualizerFormModeController. Load JSON form fail: "+$scope["selected_form_type"]+" undefined in ",data)});$scope.$watch("tabs",function(){this_.updateTestTabForm()},
true);$scope.$watch("wabSelectedGroup",function(){if(goog.isString($scope["wabSelectedState"])&&goog.isString($scope["wabSelectedGroup"])&&$scope["wabSelectedGroup"]!==""&&$scope["wabSelectedState"]!=="")this_.updateTestWabForm()});$scope.$watch("wabSelectedState",function(){if(goog.isString($scope["wabSelectedState"])&&goog.isString($scope["wabSelectedGroup"])&&$scope["wabSelectedGroup"]!==""&&$scope["wabSelectedState"]!=="")this_.updateTestWabForm()})};
...
oVFB.studioMainController.prototype.loadWabStates=function(sModuleName,sObjectName){var $scope=this.$scope_;var $this=this;ajaxRequest({"method":"POST","url":oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/wab/states","headers":{"Accept":"application/x-vm-json","X-HTTP-Method-Override":"GET"},"scope":$scope,"data":{"order_by":"progress_lvl","distinct":true,"filter":{"relation":"AND","operators":[{"column":"module_id","compare_operator":"=","value":sModuleName},{"column":"bo_name",
"compare_operator":"=","value":sObjectName}]}},"success":function(response){if(!goog.isDefAndNotNull(response.data["data"])){console.error("response.data['data'] not defined");return}$scope["wabStates"]=[];for(var i=0;i<response.data["data"].length;i++)$scope["wabStates"].push(response.data["data"][i]["name"]);$this.createEmptyWabObject()}})};
...
$scope["tables"]=[];return}$scope["tables"]=response.data["data"]}})};
...
"value":"2"},{"label":"Value three","value":"3"}]};$scope["newSource"]=this.loadDatasourceDefaultValues($scope["newSource"]);$scope["aExternalDatabaseTables"].length=0;$scope["databases"].length=0;$scope["schemas"].length=0;$scope["tables"].length=0;$("#studio-datasource-add-modal").modal("show");$scope["aExternalDatabaseTablesSelected"]={};$scope["databaseSelected"]={};$scope["schemaSelected"]={};$scope["tableSelected"]={};$("#typeSourceTooltip")["popover"]({"trigger":"hover","container":"body",
"content":function(){var sContent=$scope["text"]["Element"]["Component"]["Select"]["TypeSourceContent"];return sContent},"placement":function(oPopoverNode,oElementNode){return $scope.$root["workspaceTooltipPlacement"](oElementNode)}})};
oVFB.studioMainController.prototype.showEditSourceModal=function(){oVFB.log("studioMainController.showEditSourceModal");var $scope=this.$scope_;if($scope["datasourcesGridApi"]["selection"]["getSelectedRows"]().length>0){if(Object.keys($scope["selectedDatasource"]).length===0)return 0;$scope["editedDatasource"]=angular.copy($scope["selectedDatasource"]);$scope["editedDatasource"]=this.loadDatasourceDefaultValues($scope["editedDatasource"]);$scope["databases"].length=0;$scope["schemas"].length=0;$scope["tables"].length=
0;if($scope["editedDatasource"]["type"]==="web_service")if(goog.isDefAndNotNull($scope["editedDatasource"]["webservice"]))if(goog.isDefAndNotNull($scope["editedDatasource"]["webservice"]["name"]))this.updateRessources($scope["editedDatasource"]["webservice"]);$("#studio-datasource-edit-modal").modal("show");var $timeout=angular.element(vitisApp.appMainDrtv).injector().get(["$timeout"]);$timeout(function(){$scope.refresh=true;$timeout(function(){$scope.refresh=false},100)},500);for(var i=0;i<$scope["databases"].length;i++)if($scope["databases"][i]["databasename"]===
$scope["editedDatasource"]["parameters"]["database"]){$scope["databaseSelected"]=$scope["databases"][i];this.reloadDatabaseList($scope["editedDatasource"]["parameters"]["database"])}for(var i=0;i<$scope["schemas"].length;i++)if($scope["schemas"][i]["schema_name"]===$scope["editedDatasource"]["parameters"]["schema"]){$scope["schemaSelected"]=$scope["schemas"][i];this.reloadTablesList($scope["editedDatasource"]["parameters"]["schema"])}$timeout(function(){for(var i=0;i<$scope["tables"].length;i++)if($scope["tables"][i]["table_name"]===
$scope["editedDatasource"]["parameters"]["table"])$scope["tableSelected"]=$scope["tables"][i]},500)}else bootbox.alert(this["text"]["Element"]["Component"]["Select"]["Datasource_NothingSelected"])};
...
oVFB.studioMainController.prototype.loadDatasourceDefaultValues=function(oDatasource){oVFB.log("studioMainController.loadDatasourceDefaultValues");if(!goog.isDefAndNotNull(oDatasource["parameters"]))oDatasource["parameters"]={};if(oDatasource["dataType"]==="externalDatabase"){if(this.isEmptyFilter(oDatasource["parameters"]["filter"])||!goog.isString(oDatasource["parameters"]["filter"]))oDatasource["parameters"].filter=""}else if(this.isEmptyFilter(oDatasource["parameters"]["filter"])||!goog.isObject(oDatasource["parameters"]["filter"]))oDatasource["parameters"].filter=
{"relation":"AND","operators":[{"column":"","compare_operator":"=","value":""}]};this.clearTestGrids();return oDatasource};
...
this["text"]["Element"]["Component"]["Select"]["Undefined"]);return 0}if(!goog.isDefAndNotNull(datasource["parameters"]["login"])){bootbox.alert("Login "+this["text"]["Element"]["Component"]["Select"]["Undefined"]);return 0}if(!goog.isDefAndNotNull(datasource["parameters"]["password"])){bootbox.alert("Password "+this["text"]["Element"]["Component"]["Select"]["Undefined"]);return 0}if(!goog.isDefAndNotNull(datasource["parameters"]["database"])){bootbox.alert("Database "+this["text"]["Element"]["Component"]["Select"]["Undefined"]);
return 0}if(!goog.isDefAndNotNull(datasource["parameters"]["schema"])){bootbox.alert("Schema "+this["text"]["Element"]["Component"]["Select"]["Undefined"]);return 0}var url=oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/vitis/genericquerys";var params={"limit":50,"login":datasource["parameters"]["login"],"password":datasource["parameters"]["password"],"server":datasource["parameters"]["server"],"port":datasource["parameters"]["port"],"sgbd":datasource["parameters"]["sgbd"],
"database":datasource["parameters"]["database"],"schema":datasource["parameters"]["schema"]};if(!goog.isDefAndNotNull(url)){console.error("url undefined");return 0}if(!goog.isDefAndNotNull(params)){console.error("params undefined");return 0}ajaxRequest({"method":"GET","url":url,"headers":{"Accept":"application/x-vm-json"},"params":params,"scope":this.$scope_,"timeout":5E3,"success":function(response){if(!goog.isDef(response["data"]["data"])){bootbox.alert(this_["text"]["Element"]["Component"]["Select"]["NoDataReturned"]);
...
nsVFB.FormTools.prototype.formToolsController.prototype.copy=function(){var oJson=oVFB.getJsonOutput();oVFB.Update=true;this.$scope_["copyPasteContainer"]=angular.copy(oJson[this.$rootScope_["selected_form_type"]])};nsVFB.FormTools.prototype.formToolsController.prototype.paste=function(){var oJson=oVFB.getJsonOutput();oVFB.Update=true;oJson[this.$rootScope_["selected_form_type"]]=angular.copy(this.$scope_["copyPasteContainer"]);oVFB.setJsonOutput(oJson);this.$rootScope_.$broadcast("updateEvent",oVFB.getJsonOutput())};
nsVFB.FormTools.prototype.formToolsController.prototype.labelizeForm=function(){oVFB.Update=true;this.$rootScope_.$broadcast("labelizeForm",null)};goog.provide("nsVFB.JsonLoader");goog.require("goog.array");nsVFB.JsonLoader=function(){oVFB.log("nsVFB.JsonLoader");this.slider=null;oVFB.module.directive("appJsonLoader",this.jsonLoaderDirective);oVFB.module.controller("AppJsonLoaderController",["$scope","$rootScope","$timeout",this.jsonLoaderController])};
...
$timeout(function(){angular.element($("#load_Perso_button")).click();if(oVFB.getApplication()=="wab")this_["buttonHide"]=true},500)}});$scope.state=false};
...
function(event,data){angular.element($("#studio_form_reader").children()).scope()["ctrl"].useDefaultValues(true,data["name"])})};
