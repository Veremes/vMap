...
if(!goog.isDefAndNotNull(scope["model"]["style"]["width"]))scope["model"]["style"]["width"]=0;$(element)["slider"]({"formatter":function(value){return value+"px"},"value":parseFloat(scope["model"]["width"])});$(element)["slider"]("on","change",function(){scope["model"]["width"]=$(element)["slider"]("getValue")+"px";scope.$apply()});scope.$watch("model.width",function(value,old){$(element)["slider"]("setValue",parseFloat(value))})}if(scope["locker"]===true)$(element)["slider"]("disable");else $(element)["slider"]("enable");
break}if(scope["model"]["type"]==="list"||scope["model"]["type"]==="double_select"||scope["model"]["type"]==="textarea"){$("#slider_Element_Row")["slider"]({"formatter":function(value){return value},"min":3,"max":20,"step":1,"value":5});$("#slider_Element_Row")["slider"]("on","change",function(){scope["model"]["size"]=$("#slider_Element_Row")["slider"]("getValue");scope.$apply()});scope.$watch("model.size",function(value,old){$("#slider_Element_Row")["slider"]("setValue",value)});if(scope["locker"]===
true)$("#slider_Element_Row")["slider"]("disable");if(scope["model"]["type"]==="double_select")scope.$watch("model.name",function(value){scope["model"]["name_to"]=value;scope["model"]["name_from"]=value+"_from"})}}}};
...
nsVFB.ElementForm.prototype.elementFormController=function($scope,$rootScope,$timeout,$http,$q){oVFB.log("nsVFB.ElementForm.elementFormController");var elementFormController=this;this["applicationName"]=oVFB.getApplication();$rootScope["Utils"]={"keys":Object.keys};$scope["model"]={};$scope["collapsed"]=oVFB.Collapse_[4];$scope["aWebServiceAttributs"]=[];$scope.tmp={};var selectedElement={};var selectedElementInTreeview={};$scope.$watch("text",function(){oVFB.log("text updated");if(typeof $scope["text"]["Element"]!==
"undefined"){elementFormController["text"]=$scope["text"]["Element"];$scope["Component_list"]=[];var aAcceptedTypes=oVFB["config"]["avaliable_elems"];for(var i=0;i<aAcceptedTypes.length;i++)$scope["Component_list"].push({"alias":elementFormController["text"]["Select_Type"][aAcceptedTypes[i]],"value":aAcceptedTypes[i]})}});$scope.$watch("model.id",function(){oVFB.log("model.id changed");$scope["jsonOutput"]=oVFB.getJsonOutput();if(!goog.isDef($scope["jsonOutput"]["datasources"])||!goog.isObject($scope["jsonOutput"]["datasources"])||
goog.isArray($scope["jsonOutput"]["datasources"]))$scope["jsonOutput"]["datasources"]={};$scope["datasources"]=$scope["jsonOutput"]["datasources"];elementFormController.updateDatasourceInfos();if($scope["model"]["type"]==="select"||$scope["model"]["type"]==="list"||$scope["model"]["type"]==="double_select")if(goog.isDefAndNotNull($scope["model"]["datasource"]["parents"])){$scope["cascade"]=true;$scope["aParents"]=[];for(var i=0;i<$scope["model"]["datasource"]["parents"].length;i++)$scope["aParents"].push($scope["model"]["datasource"]["parents"][i]["name"])}else{$scope["cascade"]=
false;$scope["aParents"]=[]}oVFB.setJsonOutput($scope["jsonOutput"])});$scope.$watch("model.datasource.datasource_id",function(){oVFB.log("model.datasource.datasource_id changed");if(!goog.isDefAndNotNull($scope["model"]["datasource"]))return 0;if(!goog.isDefAndNotNull($scope["model"]["datasource"]["datasource_id"]))return 0;if(!goog.isDefAndNotNull($scope["datasources"][$scope["model"]["datasource"]["datasource_id"]]))return 0;var datasource=$scope["datasources"][$scope["model"]["datasource"]["datasource_id"]];
if(datasource["type"]==="web_service")goog.array.clear($scope["aWebServiceAttributs"]);elementFormController.updateDatasourceInfos()});$scope.$watch("model.default_value",function(){$rootScope.$broadcast("updateDefautValue",$scope["model"])});$scope.$watch("model.name",function(current,old){$scope["jsonOutput"]=oVFB.getJsonOutput();var aElems=oVFB.utils_.getElementsByName($scope["jsonOutput"],$rootScope["selected_form_type"],current);if(aElems.length>1){$scope["model"]["name"]=$scope["model"]["name"]+
"_2";$.notify($scope["text"]["Element"]["SameNameWarning"],"warning")}});$scope.$watch("model",function(current,old){$rootScope.$broadcast("update model",null);if(old["type"]!=="undefined")if(current["type"]!==old["type"]&&current["id"]===old["id"]){if(selectedElement.length>0){if(goog.isDefAndNotNull(selectedElement[0]))selectedElement[0]["icon"]=elementFormController.convert($scope["model"],current["type"]);if(goog.isDefAndNotNull(selectedElementInTreeview[0]))selectedElementInTreeview[0]["icon"]=
elementFormController.convert($scope["model"],current["type"])}if(current["type"]==="select")if(elementFormController["optionsSelect"]==="Text"){if(goog.isDef(current["web_service"]))goog.object.remove(current,"web_service");$scope["selectedRessource"]="";$scope["selectedValue"]="";$scope["selectedLabel"]=""}else if(elementFormController["optionsSelect"]==="TableValues"){if(goog.isDef(current["options"]))goog.object.remove(current,"options");$scope["selectedRessource"]="";$scope["selectedValue"]=
"";$scope["selectedLabel"]=""}else if(elementFormController["optionsSelect"]==="WebService"){if(goog.isDef(current["options"]))goog.object.remove(current,"options");if(goog.isDef(current["web_service"]["parameters"]))goog.object.remove(current["web_service"],"parameters")}}else if(current["id"]===old["id"]&&current["name"]!==old["name"]){if(goog.isDef(selectedElement[0])&&goog.isDef(selectedElementInTreeview[0])){selectedElement[0]["text"]=current["name"];selectedElementInTreeview[0]["text"]=current["name"];
if(current["name"].match(/\s/g)!==null)angular.element($("#Element_Form_name_input")).addClass("invalid_input");else angular.element($("#Element_Form_name_input")).removeClass("invalid_input")}}else if(current["id"]===old["id"]);$scope["model"]["selected"]=true;$timeout(function(){if(goog.isDef(selectedElement[0])&&goog.isDef(selectedElementInTreeview[0])){if(goog.isDefAndNotNull(selectedElement[0]))$("#treeWithLines")["treeview"]("selectNode",[selectedElement[0]["nodeId"],{silent:false}]);if(goog.isDefAndNotNull(selectedElementInTreeview[0]))$("#treeView")["treeview"]("selectNode",
[selectedElementInTreeview[0]["nodeId"],{silent:false}])}},10);$rootScope.$broadcast("updateJsonModel",oVFB.getJsonOutput());$rootScope.$broadcast("updateForm",oVFB.getJsonOutput())},true);$scope.$on("update element",function(event,data){oVFB.log("update element");$scope["model"]=data;selectedElement=$("#treeWithLines")["treeview"]("getSelected");selectedElementInTreeview=$("#treeView")["treeview"]("getSelected");if(goog.isDef($scope.model))$("#select_element_to_add").val($scope.model.type);$scope["AllSelects"]=
[];if(typeof oVFB.getJsonOutput()[$rootScope["selected_form_type"]]!=="undefined"){var obj=oVFB.getJsonOutput()[$rootScope["selected_form_type"]]["rows"];for(var i=0;i<obj.length;i++)for(var j=0;j<obj[i]["fields"].length;j++)if((obj[i]["fields"][j]["type"]==="select"||obj[i]["fields"][j]["type"]==="list")&&obj[i]["fields"][j]["name"]!==$scope["model"]["name"])$scope["AllSelects"].push(obj[i]["fields"][j]["name"])}if($("#load_Perso_button").hasClass("active")&&oVFB.valid&&data["type"]!=="undefined")$scope["locker"]=
false;else if(oVFB.valid){$scope["locker"]=true;if($scope["model"]["type"]==="map_vmap")$("#Vmap_Downloader_input").fileinput("disable")}if(!$scope.$$phase)$scope.$digest()});elementFormController.updateWSInfos=function(value){oVFB.log("elementFormController.updateWSInfos");if(typeof value["selectedValue"]!=="undefined")$scope["model"]["web_service"]["id_key"]=value["selectedValue"].replace(/\s/g,"");if(typeof value["selectedLabel"]!=="undefined")$scope["model"]["web_service"]["label_key"]=value["selectedLabel"].replace(/\s/g,
"");$scope["model"]["web_service"]["optionsSelect"]=elementFormController["optionsSelect"]};elementFormController.updateDatasourceInfos=function(){oVFB.log("elementFormController.updateDatasourceInfos");if(!goog.isDefAndNotNull($scope["model"]["datasource"]))return 0;$scope["model"]["datasource"]["sort_order"]=goog.isDefAndNotNull($scope["model"]["datasource"]["sort_order"])?$scope["model"]["datasource"]["sort_order"]:"ASC";$scope["model"]["datasource"]["distinct"]=goog.isDefAndNotNull($scope["model"]["datasource"]["distinct"])?
$scope["model"]["datasource"]["distinct"]:"true";if(goog.isDefAndNotNull($scope["model"]["datasource"]["id_key"]))$scope["model"]["datasource"]["id_key"]=$scope["model"]["datasource"]["id_key"].replace(/\s/g,"");if(goog.isDefAndNotNull($scope["model"]["datasource"]["label_key"]))$scope["model"]["datasource"]["label_key"]=$scope["model"]["datasource"]["label_key"].replace(/\s/g,"");if(goog.isDefAndNotNull($scope["model"]["datasource"]["id_key"])&&goog.isDefAndNotNull($scope["model"]["datasource"]["label_key"]))$scope["model"]["datasource"]["attributs"]=
...
default:$rootScope.$broadcast("updateMap",option);break}};elementFormController.convert=function(src,dst){oVFB.log("elementFormController.convert");var fields;var icon="icon-point";if(src["type"]!=="undefined"){switch(dst){case "button":fields=["type","id","class","name","nb_cols","buttons","default_value"];break;case "title":case "label":fields=["type","id","label","name","default_value"];dst==="label"?icon="icon-eye":icon;break;case "hr":case "hidden":fields=["type","id","name","default_value"];
dst==="hr"?icon="icon-minus":icon;dst==="hidden"?icon="icon-user-secret":icon;break;case "subtitle":fields=["type","id","label","name","class","default_value"];break;case "checkbox":fields=["type","id","label","name","checked","default_value"];icon="icon-check-square";break;case "radio":fields=["type","id","label","name","checked","options","nb_cols","default_value"];icon="icon-dot-circle-o";break;case "textarea":fields=["type","id","label","name","required","nb_cols","default_value"];icon="icon-paragraph";
break;case "select":fields=["type","id","label","name","required","nb_cols","multiple","options","web_service","child_select","default_value"];icon="icon-format_list_bulleted";break;case "list":fields=["type","id","label","name","required","nb_cols","multiple","options","web_service","child_select","default_value","size"];icon="icon-format_list_bulleted";break;case "double_select":fields=["type","id","label","name","required","nb_cols","options","web_service","size","label_from","label_to","name_from",
"name_to","default_value"];icon="icon-format_list_bulleted";break;case "date":case "datetime":case "label_display":case "color_picker":fields=["type","id","label","name","nb_cols","default_value"];dst==="date"||dst==="datetime"?icon="icon-calendar-check-o":icon;dst==="label_display"?icon="icon-eye":icon;dst==="color_picker"?icon="icon-paint":icon;break;case "upload":fields=["type","id","label","name","nb_cols","default_value","ext"];icon="icon-file";break;case "tinymce":fields=["type","id","label",
"name","nb_rows","options","default_value"];icon="icon-font";break;case "codemirror":fields=["type","id","label","name","options","default_value"];icon="icon-code";break;case "slider":fields=["type","id","label","name","options","default_value"];icon="icon-sliders";break;case "text":case "password":case "url":case "email":fields=["type","id","label","name","pattern","required","nb_cols","default_value"];dst==="text"?icon="icon-font":icon;dst==="password"?icon="icon-key":icon;dst==="url"?icon="icon-chrome":
icon;dst==="email"?icon="icon-at":icon;break;case "integer":case "float":case "number":fields=["type","id","label","name","min","max","nb_cols","default_value"];icon="icon-calculator";break;case "link":fields=["type","id","label","name","alias","folder","nb_cols","target"];icon="glyphicon glyphicon-link";break;case "image":fields=["type","id","label","name","alias","folder","nb_cols","width"];icon="glyphicon glyphicon-picture";break;case "map_osm":case "map_bing":case "map_vmap":fields=["type","id",
"label","name","required","map_options","nb_cols","style","default_value"];icon="icon-map";break}for(var srckey in src)if(goog.isDefAndNotNull(fields))if(fields.indexOf(srckey)===-1)delete src[srckey];switch(dst){case "list":case "double_select":var typic={"size":5};break;case "button":goog.isDef(src["class"])?src["class"]:src["class"]="btn-ungroup btn-group-sm";goog.isDef(src["buttons"])?src["buttons"]:src["buttons"]=[];var typic={"type":"submit","name":"form_submit","label":"FORM_UPDATE","class":"btn-primary"};
goog.isDef(src["buttons"][0])?src["buttons"][0]:src["buttons"][0]=typic;break;case "radio":var typic={"choices":[{label:"Oui",value:true},{label:"Non",value:false}]};goog.isDef(src["options"])?src["options"]:src["options"]=typic;goog.isDef(src["nb_cols"])?src["nb_cols"]:src["nb_cols"]=12;break;case "slider":var typic={"min":0,"max":100,"precision":1,"step":1};goog.isDef(src["options"])?src["options"]:src["options"]=typic;goog.isDef(src["nb_cols"])?src["nb_cols"]:src["nb_cols"]=12;break;case "codemirror":goog.isDef(src["options"])?
src["options"]:src["options"]={};goog.isDef(src["nb_cols"])?src["nb_cols"]:src["nb_cols"]=12;break;case "map_osm":goog.isDef(src["style"])?src["style"]:src["style"]={"height":"250px"};var typic={"proj":"EPSG:3857","type":"OSM","center":{"coord":[3E5,6E6],"zoom":5,"scale":25E6,"extent":[-2.002637639E7,-2.00489661E7,2.002637639E7,2.00489661E7]},"controls":{"MP":true,"ZO":true,"SL":true,"CP":true},"layers":[],"interactions":{"multi_geometry":false,"full_screen":true,"RA":false,"RO":true,"ED":true,"DP":true,
"DL":true,"DPol":true,"SE":true},"draw_color":"rgba(54,184,255,0.6)","contour_color":"rgba(0,0,0,0.4)","contour_size":2,"circle_radius":6,"features":[],"coord_accuracy":8};goog.isDef(src["map_options"])&&src["map_options"]["type"]==="OSM"?src["map_options"]:src["map_options"]=typic;break;case "map_bing":goog.isDef(src["style"])?src["style"]:src["style"]={"height":"250px"};var typic={"proj":"EPSG:3857","type":"bing","center":{"coord":[3E5,6E6],"zoom":5,"scale":25E6,"extent":[-2.002637639E7,-2.00489661E7,
2.002637639E7,2.00489661E7]},"controls":{"MP":true,"ZO":true,"SL":true,"CP":true},"layers":[],"interactions":{"multi_geometry":false,"full_screen":true,"RA":false,"RO":true,"ED":true,"DP":true,"DL":true,"DPol":true,"SE":true},"draw_color":"rgba(54,184,255,0.6)","contour_color":"rgba(0,0,0,0.4)","contour_size":2,"circle_radius":6,"features":[],"coord_accuracy":8,"source":{"culture":"fr-FR","key":"","style":"Aerial"}};goog.isDef(src["map_options"])&&src["map_options"]["type"]==="bing"?src["map_options"]:
src["map_options"]=typic;break;case "map_vmap":goog.isDef(src["style"])?src["style"]:src["style"]={"height":"250px"};var typic={"proj":"EPSG:3857","type":"vmap","center":{"coord":[3E5,6E6],"zoom":5,"scale":25E6,"extent":[-2.002637639E7,-2.00489661E7,2.002637639E7,2.00489661E7]},"controls":{"MP":true,"ZO":true,"SL":true,"CP":true},"layers":[],"interactions":{"multi_geometry":false,"full_screen":true,"RA":false,"RO":true,"ED":true,"DP":true,"DL":true,"DPol":true,"SE":true},"draw_color":"rgba(54,184,255,0.6)",
"contour_color":"rgba(0,0,0,0.4)","contour_size":2,"circle_radius":6,"features":[],"coord_accuracy":8};goog.isDef(src["map_options"])&&src["map_options"]["type"]==="vmap"?src["map_options"]:src["map_options"]=typic;break;default:if(!src["nb_cols"])src["nb_cols"]=12;break}return icon}};elementFormController["resize"]=function(){$scope["collapsed"]=!$scope["collapsed"];oVFB.Collapse_[4]=$scope["collapsed"];oVFB.resize()}};goog.provide("oVFB");goog.require("nsUtils");goog.require("nsVFB.properties");goog.require("nsVFB.JsonLoader");goog.require("nsVFB.FormTools");goog.require("nsVFB.Visualizer");goog.require("nsVFB.ElementForm");goog.require("nsVitisComponent.Map");var oVFB={};oVFB["config"]={};oVFB.init=function(config){console.log("oVFB.init: ",config);oVFB["config"]=config};oVFB.resize=function(bFullScreen){};oVFB.getDevTools=function(){return oVFB.oDevTools_};oVFB.getTreeView=function(){return oVFB.oTreeView_};
...
oVFB.getJsonOutput=function(){if(!goog.isDef(oVFB.oJsonOutput_))console.error("oVFB.oJsonOutput_ undefined");if(goog.object.isEmpty(oVFB.oJsonOutput_))console.error("oVFB.oJsonOutput_ is empty");return oVFB.oJsonOutput_};oVFB.setJsonOutput=function(oJson){oVFB.oJsonOutput_=oJson};oVFB.getTreeDnD=function(){return oVFB.tree_};oVFB.setTreeDnD=function(str){oVFB.tree_=str};oVFB.getToken=function(){return oVFB.token};oVFB["setToken"]=function(token){oVFB.token=token};oVFB.getId=function(){return oVFB.id};
...
oVFB.studioMainController=function($scope,$http,$q,$timeout){oVFB.log("nsVFB.MainController");var this_=this;this.$scope_=$scope;this.$http_=$http;this.$q_=$q;this.$timeout_=$timeout;this.iLocalDatabaseChangeCounter_=0;this["text"]={};$scope["aCollapsed"]=oVFB.Collapse_;$scope["text"]={};$scope["jsonOutput"]={};$scope["datasources"]={};$scope["selectedDatasource"]={};$scope["webservices"]=[];$scope["businessObjects"]=[];$scope["ressources"]=[];$scope["aExternalDatabaseTables"]=[];$scope["schemas"]=
[];$scope["tables"]=[];$scope["selectedDatasource"]={};$scope["editedDatasource"]={};$scope["newSource"]={"options":["Value one|1","Value two|2","Value three|3"],"result_options":[{"label":"Value one","value":"1"},{"label":"Value two","value":"2"},{"label":"Value three","value":"3"}]};$scope["editorOptions"]={"mode":"schema","lineNumbers":false,"lineSeparator":"\n","theme":"Veremes","onLoad":function(cm_){}};$scope["studioAddModalTestGridOptions"]={"data":[],"enableColumnMenus":false,"enableColumnResizing":true,
"onRegisterApi":function(studioAddModalTestGridApi){$scope.studioAddModalTestGridApi=studioAddModalTestGridApi}};$scope["studioEditModalTestGridOptions"]={"data":[],"enableColumnMenus":false,"enableColumnResizing":true,"onRegisterApi":function(studioEditModalTestGridApi){$scope.studioEditModalTestGridApi=studioEditModalTestGridApi}};$scope["studioDatasourcesGridOptions"]={"data":[],"columnDefs":[{"field":"name","displayName":"Nom"}],"enableColumnMenus":false,"enableColumnResizing":true,"enableRowSelection":true,
"multiSelect":false,"enableFullRowSelection":true,"onRegisterApi":function(datasourcesGridApi){$scope["datasourcesGridApi"]=datasourcesGridApi;datasourcesGridApi["selection"]["on"]["rowSelectionChanged"]($scope,function(row){$scope["selectedDatasource"]=angular.copy($scope["datasources"][row["entity"]["id"]]);$scope["selectedDatasource"]["id"]=row["entity"]["id"];this_.setDatasourceContent($scope["selectedDatasource"])})}};$scope["applicationName"]=oVFB.getApplication();$http.get("javascript/externs/studio/lang/lang-"+
...
return}$scope["webservices"]=response.data["data"]},function errorCallback(response){console.error("Problem with vitis/webservices contact support");console.error(response)});$http({method:"GET",url:oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/vmap/businessobjects",params:{"token":oVFB.getToken(),"attributs":"title|business_object_id|sql_summary|sql_list"},headers:{"Accept":"application/x-vm-json"}}).then(function successCallback(response){if(!goog.isDefAndNotNull(response.data["data"])){console.error("response.data['data'] not defined");
return}$scope["businessObjects"]=response.data["data"]},function errorCallback(response){console.error("Problem with vitis/webservices contact support");console.error(response)});$("#studio-datasource-modal").on("hide.bs.modal",function(e){this_.clearDatasourcesGrid();oVFB.reloadLists()});$("#studio-datasource-add-modal").on("hide.bs.modal",function(e){$scope["aExternalDatabaseTables"].length=0;$scope["schemas"].length=0;$scope["tables"].length=0;this_.clearTestGrids()});$("#studio-datasource-edit-modal").on("hide.bs.modal",
function(e){$scope["aExternalDatabaseTables"].length=0;$scope["schemas"].length=0;$scope["tables"].length=0;this_.clearTestGrids()})};oVFB.module.controller("AppStudioMainController",["$scope","$http","$q",oVFB.studioMainController]);
...
"port":datasource["parameters"]["port"],"sgbd":datasource["parameters"]["sgbd"],"database":datasource["parameters"]["database"],"schema":datasource["parameters"]["schema"]};if(!goog.isDefAndNotNull(url)){console.error("url undefined");return 0}if(!goog.isDefAndNotNull(params)){console.error("params undefined");return 0}var bRequestOk=false;var canceler=$q.defer();$http({method:"GET",url:url,params:params,timeout:canceler.promise,headers:{"Accept":"application/x-vm-json"}}).then(function successCallback(response){console.log("response: ",
response);bRequestOk=true;if(!goog.isDef(response["data"])){bootbox.alert("response['data'] undefined");return 0}if(goog.isDef(response["data"]["errorMessage"])){bootbox.alert(response["data"]["errorType"]+": "+response["data"]["errorMessage"]);return 0}if(!goog.isDef(response["data"]["data"])){bootbox.alert(this_["text"]["Element"]["Component"]["Select"]["NoDataReturned"]);return 0}if(!goog.isArray($scope["aExternalDatabaseTables"]))$scope["aExternalDatabaseTables"]=[];else $scope["aExternalDatabaseTables"].length=
0;for(var i=0;i<response["data"]["data"].length;i++)$scope["aExternalDatabaseTables"].push(response["data"]["data"][i]["table_name"])},function errorCallback(response){bootbox.alert(response)});var showCancelRequestMessageAfterTimeout=function(){setTimeout(function(){if(bRequestOk===false)bootbox.confirm(this_["text"]["Element"]["Component"]["Select"]["Cancel"],function(result){if(result===true)canceler.resolve();else showCancelRequestMessageAfterTimeout()})},5E3)};showCancelRequestMessageAfterTimeout()};
oVFB.utils_=nsUtils;if(oProperties["mode"]==="dev")oVFB.utils_.bDebugMode=true;oVFB.log=oVFB.utils_.log;oVFB.unselectAll=oVFB.utils_.unselectAll;oVFB.reloadLists=oVFB.utils_.reloadLists;oVFB.removeWebServiceFromFields=oVFB.utils_.removeWebServiceFromFields;oVFB.JsonStringifier=oVFB.utils_.JsonStringifier;oVFB.JsonOptimizer=oVFB.utils_.JsonOptimizer;oVFB.clone=oVFB.utils_.clone;oVFB.oDevTools_={};oVFB.oJsonLoader_={};oVFB.oTreeView_={};oVFB.oFormTools_={};oVFB.oElementForm_={};oVFB.oCompoList_={};
...
FormToolsController.convertJson=function(displayLines){oVFB.log("nsVFB.FormTools.convertJson");var oJson=oVFB.getJsonOutput();var aTree=[];if(!goog.isDef(oJson)){console.error("No JSON founded");return aTree}displayLines=goog.isDef(displayLines)?displayLines:true;if(oJson[$rootScope["selected_form_type"]]){for(var i=0;i<oJson[$rootScope["selected_form_type"]]["rows"].length;i++){if(displayLines===true)aTree.push({"text":this["text"]["Row"]+" "+i,"state":{"expanded":true},"tags":[0],"nodes":[],"row":i,
"draggable":true});for(var j=0;j<oJson[$rootScope["selected_form_type"]]["rows"][i]["fields"].length;j++){var glyphicon;switch(oJson[$rootScope["selected_form_type"]]["rows"][i]["fields"][j]["type"]){case "codemirror":glyphicon="icon-code";break;case "select":case "list":case "double_select":glyphicon="icon-format_list_bulleted";break;case "text":case "tinymce":glyphicon="icon-font";break;case "upload":glyphicon="icon-file";break;case "checkbox":glyphicon="icon-check-square";break;case "radio":glyphicon=
"icon-dot-circle-o";break;case "date":case "datetime":glyphicon="icon-calendar-check-o";break;case "password":glyphicon="icon-key";break;case "email":glyphicon="icon-at";break;case "url":glyphicon="icon-chrome";break;case "integer":case "float":case "number":glyphicon="icon-calculator";break;case "color_picker":glyphicon="icon-paint";break;case "slider":glyphicon="icon-sliders";break;case "textarea":glyphicon="icon-paragraph";break;case "hr":glyphicon="icon-minus";break;case "label":glyphicon="icon-eye";
break;case "email":glyphicon="icon-at";break;case "hidden":glyphicon="icon-user-secret";break;case "link":glyphicon="glyphicon glyphicon-link";break;case "image":glyphicon="glyphicon glyphicon-picture";break;case "map_bing":case "map_osm":case "map_vmap":glyphicon="icon-map";break;default:glyphicon="icon-point"}if(displayLines===true){aTree[i]["tags"][0]+=1;aTree[i]["nodes"].push({"text":"  "+oJson[$rootScope["selected_form_type"]]["rows"][i]["fields"][j]["name"],"type":oJson[$rootScope["selected_form_type"]]["rows"][i]["fields"][j]["type"],
"icon":glyphicon,"row":i,"field":j,"draggable":true})}else aTree.push({"text":"  "+oJson[$rootScope["selected_form_type"]]["rows"][i]["fields"][j]["name"],"type":oJson[$rootScope["selected_form_type"]]["rows"][i]["fields"][j]["type"],"icon":glyphicon,"row":i,"field":j,"draggable":true})}}return aTree}};$scope.$on("updateEvent",function(){var dataWithLines=FormToolsController.convertJson(true);var collapsedNodes=$("#treeWithLines")["treeview"]("getCollapsed");if(!goog.isDef(dataWithLines)){console.error("No JSON converted founded");
return 0}if(!goog.isDef(collapsedNodes)){console.error("collapsedNodes undefined");return 0}for(var i=0;i<dataWithLines.length;i++)dataWithLines[i]["collapsed"]=false;for(var i=0;i<collapsedNodes.length;i++)if(goog.isDef(collapsedNodes[i]["nodes"]))for(var ii=0;ii<dataWithLines.length;ii++)if(collapsedNodes[i]["row"]===dataWithLines[ii]["row"])dataWithLines[ii]["collapsed"]=true;$("#treeWithLines")["treeview"]({"data":dataWithLines,"showTags":true,"showBorder":false,"expandIcon":"glyphicon glyphicon-menu-right",
"collapseIcon":"glyphicon glyphicon-menu-down","onNodeSelected":function(event,data){if(goog.isDef(data["field"])){$timeout(function(){var node=$("#treeView")["treeview"]("search",[data["text"],{"ignoreCase":true,"exactMatch":true,"revealResults":false}]);$("#treeView")["treeview"]("selectNode",[node[0]["nodeId"],{silent:true}]);$("#treeView")["treeview"]("clearSearch")});FormToolsController.setSelectedElemInFormReader({"type":"element","element_name":data["text"].replace(/\s/g,"")});$rootScope.$broadcast("update element",
oVFB.getJsonOutput()[$rootScope["selected_form_type"]]["rows"][data["row"]]["fields"][data["field"]])}else FormToolsController.setSelectedElemInFormReader({"type":"row","row_id":parseInt(data["row"])})},"onNodeUnselected":function(event,data){oVFB.unselectAll()},"onNodeHoverIn":function(event){var elem=event["currentTarget"];if($(elem)[0]["childNodes"][2]["data"]){var tmp=$(elem)[0]["childNodes"][2]["data"].split(" ");var hoveredInTree_={"type":"row","row_id":parseInt(tmp[1])}}else var hoveredInTree_=
{"type":"element","element_name":$(elem)[0]["textContent"].replace(/\s/g,"")};FormToolsController.setHoveredElemInFormReader(hoveredInTree_)},"onNodeHoverOut":function(event){FormToolsController.setHoveredElemInFormReader({})},"highlightSelected":true,"multiSelect":false});$("#treeView")["treeview"]({"data":FormToolsController.convertJson(false),"showTags":false,"showBorder":false,"levels":1,"showIcon":true,"onNodeSelected":function(event,data){if(typeof data["field"]!=="undefined"){var node=$("#treeWithLines")["treeview"]("search",
[data["text"],{"ignoreCase":true,"exactMatch":true,"revealResults":false}]);var row=$("#treeWithLines")["treeview"]("search",[$scope.$parent["text"]["FormTools"]["Row"]+" "+node[0]["row"],{"ignoreCase":true,"exactMatch":true,"revealResults":false}]);$("#treeWithLines")["treeview"]("expandNode",[row[0]["nodeId"],{silent:true}]);$("#treeWithLines")["treeview"]("selectNode",[node[0]["nodeId"],{silent:false}]);$("#treeWithLines")["treeview"]("clearSearch")}},"onNodeUnselected":function(event,data){oVFB.unselectAll()},
"onNodeHoverIn":function(event){var elem=event["currentTarget"];FormToolsController.setHoveredElemInFormReader({"type":"element","element_name":$(elem)[0]["textContent"].replace(/\s/g,"")})},"onNodeHoverOut":function(event){FormToolsController.setHoveredElemInFormReader({})},"highlightSelected":true,"multiSelect":false});var elements=$("#treeWithLines")["treeview"]("getEnabled");for(var i=0;i<elements.length;i++)if(elements[i]["collapsed"]===true)$("#treeWithLines")["treeview"]("collapseNode",elements[i])});
var src={};var dst={};var ascend=true;document.getElementById("treeWithLines").addEventListener("dragstart",function(event){if($("#load_Perso_button").hasClass("active")){event["dataTransfer"]["setData"]("Text",event["target"]["id"]);event["target"]["style"]["opacity"]="0.5";event["dataTransfer"]["effectAllowed"]="move";src=event["target"];oVFB.setTreeDnD("FormTools")}});document.getElementById("treeWithLines").addEventListener("dragover",function(event){event["preventDefault"]()});document.getElementById("treeWithLines").addEventListener("dragenter",
function(event){if($("#load_Perso_button").hasClass("active")){var tmp=event["target"];if(oVFB.getTreeDnD()==="FormTools"){$(tmp).trigger("mouseover");if(tmp!==src&&$(tmp)[0].tagName==="LI"){$(src).remove();if(parseInt($(dst).attr("data-nodeid"))<parseInt($(tmp).attr("data-nodeid")))$(src).insertAfter(tmp);else $(src).insertBefore(tmp);$(dst).trigger("mouseout");dst=tmp}}}});document.getElementById("treeWithLines").addEventListener("drop",function(event){if($("#load_Perso_button").hasClass("active")){var dst_index=
0;var src_index=0;var oJson=oVFB.getJsonOutput();var tmp=0;if(oVFB.getTreeDnD()==="FormTools"){src["style"]["opacity"]="1";if(src["childNodes"][0]["className"]==="indent"){var src_id=0;var src_row=0;if(dst["childNodes"][0]["className"]==="indent"){src_id=parseInt($(src).attr("data-nodeid"));src_row=$("#treeWithLines")["treeview"]("getNode",src_id)["row"];src_index=$("#treeWithLines")["treeview"]("getNode",src_id)["field"];var dst_id=parseInt($(dst).attr("data-nodeid"));var dst_row=$("#treeWithLines")["treeview"]("getNode",
dst_id)["row"];dst_index=$("#treeWithLines")["treeview"]("getNode",dst_id)["field"];tmp=oJson[$rootScope["selected_form_type"]]["rows"][src_row]["fields"][src_index];oJson[$rootScope["selected_form_type"]]["rows"][src_row]["fields"].splice(src_index,1);oJson[$rootScope["selected_form_type"]]["rows"][dst_row]["fields"].splice(dst_index,0,tmp);if(src_row!==dst_row){FormToolsController.resizeElementsOnRow(oJson[$rootScope["selected_form_type"]]["rows"][src_row]);FormToolsController.resizeElementsOnRow(oJson[$rootScope["selected_form_type"]]["rows"][dst_row])}if(oJson[$rootScope["selected_form_type"]]["rows"][src_row]["fields"].length===
0)oJson[$rootScope["selected_form_type"]]["rows"].splice(src_row,1)}else{src_id=parseInt($(src).attr("data-nodeid"));src_row=$("#treeWithLines")["treeview"]("getNode",src_id)["row"];src_index=$("#treeWithLines")["treeview"]("getNode",src_id)["field"];dst_index=parseInt(dst["childNodes"][2]["data"].split(" ")[1]);tmp=oJson[$rootScope["selected_form_type"]]["rows"][src_row]["fields"][src_index];oJson[$rootScope["selected_form_type"]]["rows"][src_row]["fields"].splice(src_index,1);oJson[$rootScope["selected_form_type"]]["rows"][dst_index]["fields"].unshift(tmp);
FormToolsController.resizeElementsOnRow(oJson[$rootScope["selected_form_type"]]["rows"][src_row]);FormToolsController.resizeElementsOnRow(oJson[$rootScope["selected_form_type"]]["rows"][dst_index]);if(oJson[$rootScope["selected_form_type"]]["rows"][src_row]["fields"].length===0)oJson[$rootScope["selected_form_type"]]["rows"].splice(src_row,1)}}else if(dst["childNodes"][0]["className"]==="indent")$["notify"](FormToolsController["text"]["Notify"]["Error_ElOnRow"],{"className":"error","autoHideDelay":oVFB.TimeNotify_});
else{src_index=parseInt(src["childNodes"][2]["data"].split(" ")[1]);dst_index=parseInt(dst["childNodes"][2]["data"].split(" ")[1]);tmp=oJson[$rootScope["selected_form_type"]]["rows"][src_index];oJson[$rootScope["selected_form_type"]]["rows"].splice(src_index,1);oJson[$rootScope["selected_form_type"]]["rows"].splice(dst_index,0,tmp)}}oVFB.setJsonOutput(oJson);$(".hover-in-tree").each(function(index,element){$(element).removeClass("hover-in-tree")});$(".selected-in-tree").removeClass("selected-in-tree");
$rootScope.$broadcast("updateEvent",oVFB.getJsonOutput());oVFB.setTreeDnD("no tree");$scope.$apply()}});document.getElementById("treeView").addEventListener("dragstart",function(event){if($("#load_Perso_button").hasClass("active")){event["dataTransfer"]["setData"]("Text",event["target"]["id"]);event["target"]["style"]["opacity"]="0.5";event["dataTransfer"]["effectAllowed"]="move";src=event["target"];oVFB.setTreeDnD("treeView")}});document.getElementById("treeView").addEventListener("dragover",function(event){event["preventDefault"]()});
document.getElementById("treeView").addEventListener("dragenter",function(event){if($("#load_Perso_button").hasClass("active")){var tmp=event["target"];if(oVFB.getTreeDnD()==="treeView"){$(tmp).trigger("mouseover");if(tmp!==src&&$(tmp)[0].tagName==="LI")if(parseInt($(dst).attr("data-nodeid"))<parseInt($(tmp).attr("data-nodeid"))){$(src).remove();$(src).insertAfter(tmp);$(dst).trigger("mouseout");dst=tmp;ascend=false}else{$(src).remove();$(src).insertBefore(tmp);$(dst).trigger("mouseout");dst=tmp;
ascend=true}}}});document.getElementById("treeView").addEventListener("drop",function(event){if($("#load_Perso_button").hasClass("active")){var dst_index=0;var src_index=0;var oJson=oVFB.getJsonOutput();var tmp=0;if(oVFB.getTreeDnD()==="treeView"){src["style"]["opacity"]="1";var src_id=parseInt($(src).attr("data-nodeid"));var src_row=$("#treeView")["treeview"]("getNode",src_id)["row"];var src_index=$("#treeView")["treeview"]("getNode",src_id)["field"];var dst_id=parseInt($(dst).attr("data-nodeid"));
var dst_row=$("#treeView")["treeview"]("getNode",dst_id)["row"];dst_index=$("#treeView")["treeview"]("getNode",dst_id)["field"];tmp=oJson[$rootScope["selected_form_type"]]["rows"][src_row]["fields"][src_index];oJson[$rootScope["selected_form_type"]]["rows"][src_row]["fields"].splice(src_index,1);if(!ascend)dst_index+=1;oJson[$rootScope["selected_form_type"]]["rows"][dst_row]["fields"].splice(dst_index,0,tmp);if(oJson[$rootScope["selected_form_type"]]["rows"][src_row]["fields"].length===0)oJson[$rootScope["selected_form_type"]]["rows"].splice(src_row,
1)}oVFB.setJsonOutput(oJson);$(".hover-in-tree").each(function(index,element){$(element).removeClass("hover-in-tree")});$(".selected-in-tree").removeClass("selected-in-tree");$rootScope.$broadcast("updateEvent",oVFB.getJsonOutput());$scope.$apply();oVFB.setTreeDnD("no tree")}});FormToolsController["resize"]=function(){$scope["colapse_tools"]=!$scope["colapse_tools"];oVFB.Collapse_[2]=$scope["colapse_tools"];oVFB.resize()}};
...
1){var checkElementAdded=function(rows){for(var i=0;i<rows.length;i++){if(!goog.isDef(rows[i]["fields"]))continue;for(var ii=0;ii<rows[i]["fields"].length;ii++)if(rows[i]["fields"][ii]["name"]==="Element_"+this_.elemAdded_){this_.elemAdded_++;checkElementAdded(rows);return 0}}};var rows=oVFB.getJsonOutput()[this.$rootScope_["selected_form_type"]]["rows"];checkElementAdded(rows);var newElem={"type":"text","name":"Element_"+this.elemAdded_,"label":"","required":false,"pattern":"","nb_cols":12};var selectedNodeId=
aSelectedNode[0]["nodeId"];if(this.bTreeWithLines_){data[this.$rootScope_["selected_form_type"]]["rows"][aSelectedNode[0]["row"]]["fields"].splice(aSelectedNode[0]["field"]+1,0,newElem);selectedNodeId=selectedNodeId+1}else if(!lineAdded){this.addLine();data[this.$rootScope_["selected_form_type"]]["rows"][aSelectedNode[0]["row"]+1]["fields"].push(newElem);var lastLine=this.getLastLine()["nodeId"]+1;if(aSelectedNode[0]["row"]+1===data[this.$rootScope_["selected_form_type"]]["rows"].length-1)selectedNodeId=
selectedNodeId+2+data[this.$rootScope_["selected_form_type"]]["rows"][aSelectedNode[0]["row"]]["fields"].length;else selectedNodeId=selectedNodeId+1+data[this.$rootScope_["selected_form_type"]]["rows"][aSelectedNode[0]["row"]]["fields"].length}else{data[this.$rootScope_["selected_form_type"]]["rows"][aSelectedNode[0]["row"]]["fields"].push(newElem);selectedNodeId=selectedNodeId+2}var row=data[this.$rootScope_["selected_form_type"]]["rows"][aSelectedNode[0]["row"]];this.resizeElementsOnRow(row);oVFB.setJsonOutput(data);
this.$rootScope_.$broadcast("updateEvent",oVFB.getJsonOutput());$("#TreeView_Button_Group")["confirmation"]("destroy");$("#treeWithLines")["treeview"]("selectNode",[selectedNodeId,{silent:false}])}else $["notify"](this["text"]["Notify"]["Forgotten"],{"className":"error","autoHideDelay":oVFB.TimeNotify_})};
...
{"className":"error","autoHideDelay":oVFB.TimeNotify_});return 0}if(aSelectedNode.length===1)bootbox.confirm("<h4>"+this["text"]["Confirmation"]["Remove"]+"</h4>",function(result){if(result===true){var row=data[this_.$rootScope_["selected_form_type"]]["rows"][aSelectedNode[0]["row"]];row["fields"].splice(aSelectedNode[0]["field"],1);if(data[this_.$rootScope_["selected_form_type"]]["rows"][aSelectedNode[0]["row"]]["fields"].length===0)data[this_.$rootScope_["selected_form_type"]]["rows"].splice(aSelectedNode[0]["row"],
1);this_.resizeElementsOnRow(row);$(".selected-in-tree").each(function(index,element){$(element).removeClass("selected-in-tree")});oVFB.unselectAll();oVFB.setJsonOutput(data);this_.$rootScope_.$broadcast("updateEvent",oVFB.getJsonOutput());this_.$scope_.$apply();$("#Formtools_buttons")["confirmation"]("destroy");$["notify"](this_["text"]["Notify"]["Remove"],{"className":"success","autoHideDelay":oVFB.TimeNotify_})}});else $["notify"](this["text"]["Notify"]["Forgotten"],{"className":"error","autoHideDelay":oVFB.TimeNotify_})};
...
nsVFB.JsonLoader.prototype.jsonLoaderController=function($http,$scope,$rootScope,$timeout){oVFB.log("nsVFB.JsonLoader.jsonLoaderController");var this_=this;this.$http_=$http;this.$scope_=$scope;this.$rootScope_=$rootScope;this.tmpForm_={};this["applicationName"]=oVFB.getApplication();$scope["collapsed"]=oVFB.Collapse_[0];$scope["iWorkspaceId"]=oVFB.getId();$scope["iWorkSpace"]=oVFB.getId();$scope["default"]="Default";$scope["FormSelected"]="Perso";$scope["token"]=oVFB.getToken();$rootScope["selected_form_type"]=
oVFB["config"]["default_form_type"];$rootScope.$watch("selected_form_type",function(value){oVFB.log("jsonLoaderController selected_form_type");var oJSONform=oVFB.getJsonOutput();if(!goog.isDefAndNotNull(oJSONform[value])){oJSONform[value]={"name":"custom-form","title":"","input_size":"xxs","nb_cols":12,"javascript":false,"rows":[]};oVFB.setJsonOutput(oJSONform)}$rootScope.$broadcast("updateEvent",oVFB.getJsonOutput());$rootScope.$broadcast("update element",{type:"undefined"})});oVFB.getJsonLoader().slider=
...
var tmp=oVFB.getJsonOutput();if(tmp[$rootScope["selected_form_type"]])tmp[$rootScope["selected_form_type"]]["title"]=value;oVFB.setJsonOutput(tmp);$rootScope.$broadcast("updateJsonModel",oVFB.getJsonOutput());$rootScope.$broadcast("updateForm",oVFB.getJsonOutput())});$scope.$watch("token",function(value){oVFB.log("jsonLoaderController token");oVFB["setToken"](value)});$scope.$watch("iWorkspaceId",function(value){oVFB.log("jsonLoaderController id");oVFB["setId"](value)});$scope.$watch("text",function(){oVFB.log("jsonLoaderController text");
if(typeof $scope["text"]["Loader"]!=="undefined"){this_["text"]=$scope["text"]["Loader"];$timeout(function(){angular.element($("#load_Perso_button")).click()},500)}});$scope.state=false};
nsVFB.JsonLoader.prototype.jsonLoaderController.prototype.requestToJson=function(successCallback,errorCallback){oVFB.log("nsVFB.JsonLoader.prototype.jsonLoaderController.requestToJson");var this_=this;var $http=this.$http_;angular.element($("#FormModal")).scope()["ctrl"].emptyFormValues();this.$scope_["iWorkSpace"]=this.$scope_["iWorkspaceId"];this.$scope_["default"]=this.$scope_["FormSelected"];this.$scope_["collapsed"]=oVFB.Collapse_[0];if(this["applicationName"]==="gtf"){var url=oVFB.AppProperties["web_server_name"]+
"/"+oVFB.AppProperties["services_alias"]+"/gtf/workspaces/"+oVFB.getId()+"?token="+oVFB.getToken()+"&form="+this.$scope_["FormSelected"];var config={headers:{"Content-Type":"application/json"}}}else{var form=this.$scope_["FormSelected"]==="Perso"?"custom":angular.lowercase(this.$scope_["FormSelected"]);var url=oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/"+oVFB["config"]["get"]["api"]+"/"+oVFB["config"]["get"]["ressource"]+"/"+oVFB.getId()+"/forms/"+form+"/?token="+
oVFB.getToken();var config={headers:{"Content-Type":undefined}}}showAjaxLoader();$http({method:"GET",url:url,headers:config.headers}).success(function(data){hideAjaxLoader();if(!goog.isDefAndNotNull(data[0]))if(goog.isDefAndNotNull(data["data"]))data=data["data"];if(this_["applicationName"]==="gtf"){if(!goog.isDefAndNotNull(data["workspaces"])&&!goog.isDefAndNotNull(data["data"])){var cmd;switch(this_.$scope_["FormSelected"]){case "Default":cmd="Default_Reset";break;case "Published":cmd="Default_Published";
break;case "Perso":cmd="Perso_Reset";break}this_.putRequest(cmd,undefined,function(data){if(goog.isDefAndNotNull(data["errorMessage"])){bootbox.alert("<h4>"+data["errorMessage"]+"</h4>",function(){});return 0}this_.requestToJson()},function(data,status){$["notify"](this_.$scope_["text"]["Loader"]["Notify"]["GetJson_Error"]+status,{"className":"error","autoHideDelay":oVFB.TimeNotify_})})}}else if(!goog.isDefAndNotNull(data[0])){if(goog.isDefAndNotNull(oVFB["config"]["get"]["no_result_function"]))this_[oVFB["config"]["get"]["no_result_function"]](data,
successCallback);return 0}this_.useJson(data,successCallback)}).error(function(data,status){hideAjaxLoader();if(goog.isFunction(errorCallback))this_.$scope_.$eval(errorCallback);$["notify"](this_.$scope_["text"]["Loader"]["Notify"]["GetJson_Error"]+status,{"className":"error","autoHideDelay":oVFB.TimeNotify_})})};
...
this.$rootScope_["selected_form_type"]+"] does not exist");return 0}var nbCols=data[0][this.$rootScope_["selected_form_type"]]["nb_cols"];var title=oVFB.getJsonOutput()[this.$rootScope_["selected_form_type"]]["title"];var inputSize=oVFB.getJsonOutput()[this.$rootScope_["selected_form_type"]]["input_size"];oVFB.getJsonLoader().slider["setValue"](nbCols);this.$scope_["title"]=title;this.$scope_["input_size"]=inputSize;this.$rootScope_.$broadcast("updateEvent",oVFB.getJsonOutput());this.$rootScope_.$broadcast("updateInfos",
oVFB.getJsonOutput());this.$rootScope_.$broadcast("update element",{type:"undefined"});if(goog.isFunction(successCallback))this.$scope_.$eval(successCallback);setTimeout(function(){oVFB.reloadLists()})};nsVFB.JsonLoader.prototype.jsonLoaderController.prototype.load=function(mode,el){oVFB.log("load");if(mode==="Perso")this.loadWithEdit(true,mode,el);else this.loadWithoutEdit(mode,el)};
...
nsVFB.JsonLoader.prototype.jsonLoaderController.prototype.putRequest=function(cmd,data,functSuccess,functError){oVFB.log("nsVFB.JsonLoader.prototype.jsonLoaderController.prototype.putRequest");if(this["applicationName"]==="gtf")var url=oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/gtf/workspaces/"+oVFB.getId()+"?token="+oVFB.getToken()+"&cmd="+cmd;else{var form=this.$scope_["FormSelected"]==="Perso"?"custom":angular.lowercase(this.$scope_["FormSelected"]);var url=
oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/"+oVFB["config"]["get"]["api"]+"/"+oVFB["config"]["get"]["ressource"]+"/"+oVFB.getId()+"/forms/"+form+"/?token="+oVFB.getToken()+"&cmd="+cmd}this.$http_({url:url,method:"PUT",data:data,headers:{"Content-Type":"undefined"}}).success(functSuccess).error(functError)};
nsVFB.JsonLoader.prototype.jsonLoaderController.prototype.suggestGenerateForm=function(){oVFB.log("nsVFB.JsonLoader.prototype.jsonLoaderController.prototype.suggestGenerateForm");var this_=this;bootbox.confirm("<h4>"+this.$scope_["text"]["Loader"]["Notify"]["generate_form"]+"</h4>",function(result){if(result===true)this_.suggestColumnsToGenerateForm()})};
...
nsVFB.JsonLoader.prototype.jsonLoaderController.prototype.suggestColumnsToGenerateForm=function(){oVFB.log("nsVFB.JsonLoader.prototype.jsonLoaderController.prototype.suggestColumnsToGenerateForm");var url=oVFB.AppProperties["web_server_name"]+"/"+oVFB.AppProperties["services_alias"]+"/vmap/businessobjects/"+oVFB.getId()+"/fields/?token="+oVFB.getToken();var this_=this;if(goog.isDef(this_["aSuggestedTableFields"]))goog.array.clear(this_["aSuggestedTableFields"]);this.$http_({url:url,method:"GET"}).success(function(data){this_["aSuggestedTableFields"]=
data;$("#select-fields-modal").modal("show")}).error(function(){$["notify"](this_.$scope_["text"]["Loader"]["Notify"]["CreateJson_Error"]+status,{"className":"error","autoHideDelay":oVFB.TimeNotify_})})};
...
form+"/?token="+oVFB.getToken();var aFieldsToUse=[];for(var i=0;i<this_["aSuggestedTableFields"].length;i++)if(this_["aSuggestedTableFields"][i]["fields"][0]["bUseForGenerate"]===true)aFieldsToUse.push(this_["aSuggestedTableFields"][i]["fields"][0]["name"]);var sFieldsToUse=aFieldsToUse.toString().replace(/,/gi,"|");$("#select-fields-modal").modal("hide");if(this_.$scope_["FormSelected"]==="Default"){var success=function(){$["notify"](this_.$scope_["FormSelected"]+this_.$scope_["text"]["Loader"]["Notify"]["Reset"],
{"className":"success","autoHideDelay":oVFB.TimeNotify_});if(this_.$scope_["FormSelected"]==="Default")this_.requestToJson()};var error=function(data,status){$["notify"](this_.$scope_["text"]["Loader"]["Notify"]["Reset_Error"]+status,{"className":"error","autoHideDelay":oVFB.TimeNotify_})};this_.putRequest(this_.$scope_["FormSelected"]+"_Reset",{"field":sFieldsToUse},success,error)}else this.$http_({url:url,method:"POST",data:{"fields":sFieldsToUse}}).success(function(){this_.requestToJson()}).error(function(){$["notify"](this_.$scope_["text"]["Loader"]["Notify"]["CreateJson_Error"]+
status,{"className":"error","autoHideDelay":oVFB.TimeNotify_})})};goog.provide("nsUtils");nsUtils={};nsUtils.bDebugMode_=false;nsUtils.log=function(message){if(goog.isDefAndNotNull(oVFB.AppProperties))if(oVFB.AppProperties["debug_mode"])if(nsUtils.bDebugMode_)console.log(message)};
nsUtils.unselectAll=function(){nsUtils.log("util.unselectAll");angular.element($("#studio_container")).scope().$broadcast("update element",{});angular.element($("#FormModal")).scope()["ctrl"].removeSelectedAndHoveredElem();$("#treeView")["treeview"]("unselectNode",[$("#treeView")["treeview"]("getSelected"),{silent:true}]);$("#treeWithLines")["treeview"]("unselectNode",[$("#treeWithLines")["treeview"]("getSelected"),{silent:true}])};
nsUtils.reloadLists=function(){nsUtils.log("util.reloadLists");var oFormReaderScope=angular.element($("#studio_form_reader").children()).scope();oFormReaderScope["oFormValues"][oFormReaderScope["sFormDefinitionName"]]={};setTimeout(function(){oFormReaderScope["ctrl"].useDefaultValues(true);setTimeout(function(){oFormReaderScope["ctrl"].extractFormDefinitionInfos()},100)},100)};nsUtils.clone=function(obj){nsUtils.log("util.clone");return obj};
nsUtils.JsonStringifier=function(oJson){nsUtils.log("util.JsonStringifier");var strJson=JSON.stringify(oJson,null,2);return strJson};
...
"lineSeparator":"\n","indentWithTabs":true,"tabSize":2,"viewportMargin":50,"theme":"Veremes","onLoad":function(cm_){$scope.$on("updateEvent",function(event,data){cm_["setOption"]("readOnly",$("#Lock_Icon").hasClass("fa-lock"));$scope["JsonModel"]=oVFB.JsonStringifier(oVFB.getJsonOutput())});$scope.$watch("JsonModel",function(value,old){oVFB.log("jsonmode model");var error=false;try{JSON.parse(value)}catch(e){error=true}finally{$scope.$parent["error_Json"]=error}});$scope.$on("updateJsonModel",function(event,
data){if($scope["$parent"]["ctrl"]["json_"]){$scope.moment_=Date.now();$timeout(function(){if(Date.now()-$scope.moment_>=$scope.labs_)$scope["JsonModel"]=oVFB.JsonStringifier(oVFB.getJsonOutput())},$scope.labs_)}})}}};
