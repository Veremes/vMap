goog.provide("nsVitisComponent");nsVitisComponent={};goog.provide("nsVitisComponent.Map");goog.require("nsVitisComponent");goog.require("ol");goog.require("ol.Map");goog.require("ol.View");goog.require("ol.proj");goog.require("ol.proj.Projection");goog.require("ol.Collection");goog.require("ol.layer.Tile");goog.require("ol.layer.Vector");goog.require("ol.layer.Image");goog.require("ol.layer.Group");goog.require("ol.source.OSM");goog.require("ol.source.BingMaps");goog.require("ol.source.Vector");goog.require("ol.format.WKT");goog.require("ol.control");
goog.require("ol.control.MousePosition");goog.require("ol.control.Control");goog.require("ol.control.ScaleLine");goog.require("ol.style.Style");goog.require("ol.style.Fill");goog.require("ol.style.Stroke");goog.require("ol.style.Circle");goog.require("ol.animation");goog.require("ol.easing");goog.require("ol.interaction.Interaction");goog.require("ol.interaction.Select");goog.require("ol.interaction.Modify");goog.require("ol.interaction.Draw");goog.require("ol.events.condition");goog.require("ol.Extent");
goog.require("ol.Sphere");var featureID=0;
nsVitisComponent.Map=function(id,options,hiddenFieldId,oFormValues,sFormName,oProj){this.$propertiesSrvc_=angular.element(vitisApp.appHtmlFormDrtv).injector().get(["propertiesSrvc"]);this.$translate_=angular.element(vitisApp.appHtmlFormDrtv).injector().get(["$translate"]);this.$log_=angular.element(vitisApp.appHtmlFormDrtv).injector().get(["$log"]);var this_=this;this.$log_.log("nsVitisComponent.Map",options);this.appColor=$(".banner_line").css("background-color");if(goog.isDef(this.appColor))if(this.appColor.charAt(3)!==
"a"){this.appColor=this.appColor.split("(");this.appColor=this.appColor[1].split(")");this.appColor="rgba("+this.appColor[0]+",0.58)"}var aTranslates=["COMPONENT_MAP_LAYERSTREE","COMPONENT_MAP_FULLSCREEN","COMPONENT_MAP_ZOOM_EXTENT","COMPONENT_MAP_DELETE_FEATURE","COMPONENT_MAP_DELETE_ALL","COMPONENT_MAP_MODIFY_FEATURE","COMPONENT_MAP_DRAW_POINT","COMPONENT_MAP_DRAW_LINE","COMPONENT_MAP_DRAW_POLYGON","COMPONENT_MAP_ZOOM_EXTENT","COMPONENT_MAP_OVERSIZED_EXTENT"];this.$translate_(aTranslates).then(function(translations){this_.oTranslates=
translations});this.addCustomProjections();var center,extent;if(goog.isDef(options["center"]["coord"]))center=options["center"]["coord"];if(goog.isDef(options["center"]["extent"]))extent=options["center"]["extent"];this.proj_=options["proj"];if(options["center"]["coord"])this.View=new ol.View({center:center,zoom:5,projection:ol.proj.get(options["proj"])});else this.View=new ol.View({center:[(extent[0]+extent[2])/2,(extent[1]+extent[3])/2],zoom:5,projection:ol.proj.get(options["proj"])});this.oTranslates=
{};this.Id=id;this.Target=document.getElementById(id);this.Accuracy=options["coord_accuracy"];this.Controls=[];this.Layers=[];this.Features=new ol.Collection;this.interaction="none";this.Config={};this.oProj_=oProj;this.multiGeom=options["interactions"]["multi_geometry"];this["bLayersTreeOpen"]=false;if(options["type"]==="OSM")this.Layers.push(new ol.layer.Tile({source:new ol.source.OSM}));else if(options["type"]==="bing")this.Layers.push(new ol.layer.Tile({source:new ol.source.BingMaps({culture:options["source"]["culture"],
key:options["source"]["key"],imagerySet:options["source"]["style"]})}));this.element=document.createElement("DIV");$(this.element).addClass("ol-current-projection-studio ol-unselectable");if(options["controls"]["MP"])this.Controls.push(new ol.control.MousePosition({coordinateFormat:function(coordinate){return[coordinate[0].toPrecision(this_.Accuracy),coordinate[1].toPrecision(this_.Accuracy)]}}));if(options["controls"]["SL"])this.Controls.push(new ol.control.ScaleLine);if(options["controls"]["CP"])this.Controls.push(new ol.control.Control({element:this_.element,
render:function(){var currentProj="";for(var i=0;i<this_.oProj_["projections"].length;i++)if(this_.proj_===this_.oProj_["projections"][i]["code"])currentProj=this_.oProj_["projections"][i]["name"];if(currentProj!=="")$(".ol-current-projection-studio").html(currentProj);else $(".ol-current-projection-studio").html(this_.proj_)}}));if(options["controls"]["ZO"])setTimeout(function(){this_.ZoomGroup=this_.zoomControls(this_.Target)},500);setTimeout(function(){this_.IntractionsGroup=this_.addInteractions(this_.Target,
options["interactions"])});if(Array.isArray(options["layers"]))if(options["layers"].length>0)this.Layers.push(options["layers"]);this.MapObject=new ol.Map({target:this_.Target,layers:this_.Layers,view:this_.View,controls:this_.Controls});if(goog.isDefAndNotNull(options["center"]["scale"]))this.setScale(options["center"]["scale"]);if(goog.isDefAndNotNull(options["center"]["extent"]))this.setExtent(options["center"]["extent"]);if(options["type"]==="vmap"&&options["tree"])this["aTree"]=this.loadTree(options["tree"]);
else this["aTree"]=[{"service":options["type"],"layers":this.Layers}];var projExtent=ol.proj.get(options["proj"]).getExtent();var viewExtent=this.View.calculateExtent(this.MapObject.getSize());if(this.isInsideProjExtent(viewExtent,projExtent))this.Extent=this.View.calculateExtent(this.MapObject.getSize());else{setTimeout(function(){$.notify(this_.oTranslates["COMPONENT_MAP_OVERSIZED_EXTENT"],"warn")},500);this.MapObject.getView().fit(projExtent,this.MapObject.getSize())}$(".ol-mouse-position").css("bottom",
"8px");$(".ol-mouse-position").css("top","auto");$(".ol-mouse-position").css("background",this.appColor);$(".ol-mouse-position").css("color","#ffffff");$(".ol-mouse-position").css("border-radius","4px");$(".ol-scale-line").css("background",this.appColor);$(".ol-current-projection-studio").css("background",this.appColor);options["draw_color"]=goog.isDef(options["draw_color"])?options["draw_color"]:"rgba(54,184,255,0.6)";options["contour_color"]=goog.isDef(options["contour_color"])?options["contour_color"]:
"rgba(0,0,0,0.4)";options["contour_size"]=goog.isDef(options["contour_size"])?options["contour_size"]:2;options["circle_radius"]=goog.isDef(options["circle_radius"])?options["circle_radius"]:7;var style=new ol.style.Style({fill:new ol.style.Fill({color:options["draw_color"]}),stroke:new ol.style.Stroke({color:options["contour_color"],width:options["contour_size"]}),image:new ol.style.Circle({radius:options["circle_radius"],fill:new ol.style.Fill({color:options["draw_color"]}),stroke:new ol.style.Stroke({color:options["contour_color"],
width:options["contour_size"]})})});this.FeatureOverlay=new ol.layer.Vector({style:style,updateWhileAnimating:true,updateWhileInteracting:true,source:new ol.source.Vector({features:this_.Features})});setTimeout(function(){this_.MapObject.addLayer(this_.FeatureOverlay)},500);this.selectHover_=new ol.interaction.Select({source:this.FeatureOverlay.getSource(),condition:ol.events.condition.pointerMove});this.MapObject.addInteraction(this.selectHover_);var wktFormat=new ol.format.WKT;var hiddenFeatures=
$("#"+hiddenFieldId).val();if(goog.isDef(hiddenFeatures))if(!goog.string.isEmpty(hiddenFeatures)&&hiddenFeatures!=="[object Object]"){var aFeatures=wktFormat.readFeatures(hiddenFeatures);for(var i=0;i<aFeatures.length;i++)this.FeatureOverlay.getSource().addFeature(aFeatures[i]);var featuresExtent=this.FeatureOverlay.getSource().getExtent();this.MapObject.getView().fit(featuresExtent,this.MapObject.getSize());if(aFeatures.length===1)if(aFeatures[0].getGeometry().getType()==="Point")this.MapObject.getView().setZoom(12)}this.Features.on("change",
...
$scope["sFormDefinitionName"]:"";this.initFormTabs();setTimeout(function(){this_.extractFormDefinitionInfos()});$scope["sFormUniqueName"]=Date.now();if(typeof $rootScope["oFormPromises"]=="undefined")$rootScope["oFormPromises"]={};$rootScope["oFormPromises"][$scope["sFormDefinitionName"]]=[];var clearListener=$rootScope.$on("formDefinitionLoaded",function(){this_.loadForm();clearListener()})};formReader.module.controller("AppFormReaderController",formReader.formReaderController);
...
formReader.formReaderController.prototype.loadForm=function(){var this_=this;this.initFormTabs();this.useDefaultValues(false);setTimeout(function(){this_.extractFormDefinitionInfos();var formDefinition=this_.$scope_["oFormDefinition"];var formDefinitionName=this_.$scope_["sFormDefinitionName"];if(!goog.isDefAndNotNull(formDefinition))return 0;if(!goog.isDefAndNotNull(formDefinitionName))return 0;if(!goog.isDefAndNotNull(formDefinition[formDefinitionName]))return 0;if(!goog.isDefAndNotNull(formDefinition[formDefinitionName]["initEvent"]))return 0;
var initEvent=formDefinition[formDefinitionName]["initEvent"];if(this_.formReaderService_["isFunctionCall"](initEvent))setTimeout(function(){this_.formReaderService_["callFunction"](initEvent)},100)});this.$scope_.$broadcast("loadForm")};formReader.formReaderController.prototype.refreshForm=function(){this.initFormTabs();this.$scope_.$broadcast("$$rebind::refresh")};
...
formReader.formReaderController.prototype.useDefaultValues=function(bEraseFormValues,sElementName){var $scope=this.$scope_;var oFormValues=$scope["oFormValues"];var oFormDefinition=$scope["oFormDefinition"];var sFormDefinitionName=$scope["sFormDefinitionName"];if(!goog.isDefAndNotNull(oFormValues))return 0;if(!goog.isDefAndNotNull(oFormDefinition))return 0;if(!goog.isDefAndNotNull(sFormDefinitionName))return 0;if(!goog.isDefAndNotNull(oFormDefinition[sFormDefinitionName]))return 0;var aRows=oFormDefinition[sFormDefinitionName]["rows"];
if(!goog.isDefAndNotNull(aRows))return 0;for(var i=0;i<aRows.length;i++)if(goog.isDefAndNotNull(aRows[i]["fields"]))for(var ii=0;ii<aRows[i]["fields"].length;ii++){if(aRows[i]["fields"][ii]["type"]==="bo_grid"||aRows[i]["fields"][ii]["type"]==="ui_grid"||aRows[i]["fields"][ii]["type"]==="section_grid")continue;var field;try{field=angular.copy(aRows[i]["fields"][ii])}catch(e){console.error("try angular.copy failed");continue}if(goog.isDefAndNotNull(sElementName))if(field["name"]!==sElementName)continue;
if(goog.isDefAndNotNull(field["default_value"])&&field["default_value"]!=="")if(this.formReaderService_["isFunctionCall"](field["default_value"]))field["default_value"]=this.formReaderService_["callFunction"](field["default_value"]);if(field["type"]==="select"){if(goog.isDefAndNotNull(field["default_value"])&&field["default_value"]!=="")field["default_value"]={"value":field["default_value"]}}else if(field["type"]==="list"||field["type"]==="double_select")if(goog.isDefAndNotNull(field["default_value"])&&
field["default_value"]!==""){var aDefaultKeys=field["default_value"].split("|");var aDefaults=[];for(var iii=0;iii<aDefaultKeys.length;iii++)aDefaults.push({"value":aDefaultKeys[iii]});field["default_value"]=aDefaults}if(this.useDefaultValue(field,bEraseFormValues))if(field["type"]==="select"||field["type"]==="list"){if(!goog.isDefAndNotNull(oFormValues[sFormDefinitionName][field["name"]]))oFormValues[sFormDefinitionName][field["name"]]={};oFormValues[sFormDefinitionName][field["name"]]["selectedOption"]=
...
if(!goog.isDefAndNotNull(oFormValues[sFormDefinitionName][oField["name"]]["selectedOption"]))return true;if(!goog.isDefAndNotNull(oFormValues[sFormDefinitionName][oField["name"]]["selectedOption"]["value"]))return true;if(oFormValues[sFormDefinitionName][oField["name"]]["selectedOption"]["value"].length===0)return true}else if(oField["type"]==="list"){if(!goog.isDefAndNotNull(oFormValues[sFormDefinitionName][oField["name"]]))return true;if(!goog.isArray(oFormValues[sFormDefinitionName][oField["name"]]["selectedOption"]))return true;
if(oFormValues[sFormDefinitionName][oField["name"]]["selectedOption"].length===0)return true;if(oFormValues[sFormDefinitionName][oField["name"]]["selectedOption"][0]["value"].length===0)return true}else if(oField["type"]==="double_select"){if(!goog.isDefAndNotNull(oFormValues[sFormDefinitionName][oField["name_to"]]))return true;if(!goog.isArray(oFormValues[sFormDefinitionName][oField["name_to"]]["selectedOption"]))return true;if(oFormValues[sFormDefinitionName][oField["name_to"]]["selectedOption"].length===
0)return true;if(oFormValues[sFormDefinitionName][oField["name_to"]]["selectedOption"][0]["value"].length===0)return true}else if(oField["type"]==="editable_double_select"){if(!goog.isDefAndNotNull(oFormValues[sFormDefinitionName][oField["name_available_tags"]]))return true;if(!goog.isArray(oFormValues[sFormDefinitionName][oField["name_available_tags"]]["selectedOption"]))return true;if(oFormValues[sFormDefinitionName][oField["name_available_tags"]]["selectedOption"].length===0)return true;if(oFormValues[sFormDefinitionName][oField["name_available_tags"]]["selectedOption"][0]["value"].length===
0)return true}else{if(!goog.isDefAndNotNull(oFormValues[sFormDefinitionName][oField["name"]]))return true;if(oFormValues[sFormDefinitionName][oField["name"]].length===0)return true}else return false};
...
aUsedOptions);else if(typeof formFieldValue!=="object"){selectValue=formFieldValue;if(elem["type"]==="select")oFormValues[elem["name"]]={"selectedOption":{"value":selectValue},"options":[]};else oFormValues[elem["name"]]={"selectedOption":[{"value":selectValue}],"options":[]}}break;case "editable_double_select":elem["name_available_tags"]=elem["name"]+"_available";elem["id_available_tags"]=elem["id"]+"_available";if(typeof elem["web_service"]!=="undefined")this.formReaderService_["setWebServiceTags"](elem,
this.$scope_["sFormDefinitionName"],this.$scope_["oFormValues"]);break;case "title":case "subtitle":case "label":case "button":break;default:break}delete elem}this.$q_.all(this.$scope_.$root["oFormPromises"][this.$scope_["sFormDefinitionName"]]).then(function(values){this_.$scope_.$root.$emit("formExtracted");if(goog.isDefAndNotNull(oFormDefinition)){var sFormName=oFormDefinition["name"];var oFormElement=angular.element("form[name='"+sFormName+"']");if(oFormElement.length>0){var oFormElementScope=
oFormElement.scope();if(goog.isDefAndNotNull(oFormElementScope[sFormName]))oFormElementScope[sFormName].$setPristine()}}});deferred.resolve()};
...
formReader.formReaderDirective=function($q,formReaderService,propertiesSrvc,$log,$http,$compile){return{restrict:"A",scope:{"oProperties":"=?appProperties","sToken":"=?appToken","oFormValues":"=?appFormValues","oFormDefinition":"=?appFormDefinition","sFormDefinitionName":"=?appFormDefinitionName","oFormEventsContainer":"=?appFormEventsContainer","wabState":"=?appWabState","wabGroup":"=?appWabGroup","showTabs":"=?appTabs"},controller:"AppFormReaderController",controllerAs:"ctrl",templateUrl:propertiesSrvc["web_server_name"]+
"/"+sessionStorage["appEnv"]+"/javascript/externs/formReader/formReader.html",link:function(scope,element,attrs){$log.log("formReader.formReaderDirective.link");scope["showTabs"]=scope["showTabs"]==="false"||scope["showTabs"]===false?scope["showTabs"]=false:scope["showTabs"]=true;scope["wabState"]=goog.isDefAndNotNull(scope["wabState"])&&scope["wabState"]!==""?scope["wabState"]:null;scope["wabGroup"]=goog.isDefAndNotNull(scope["wabGroup"])&&scope["wabGroup"]!==""?scope["wabGroup"]:null;scope["submitButton"]=
false;scope["oFormEventsContainer"]=goog.isDef(scope["oFormEventsContainer"])?scope["oFormEventsContainer"]:scope;formReaderService["oProperties"]=scope["oProperties"];formReaderService["sToken"]=scope["sToken"];formReaderService["oFormValues"]=scope["oFormValues"];formReaderService["oFormDefinition"]=scope["oFormDefinition"];formReaderService["sFormDefinitionName"]=scope["sFormDefinitionName"];formReaderService["oFormEventsContainer"]=scope["oFormEventsContainer"];scope["isFormTextElement"]=function(sFormElementType){var aFormText=
new Array("text","password","datetime","datetime-local","date","month","time","week","integer","float","number","email","url","search","tel","color");if(aFormText.indexOf(sFormElementType)!==-1)return true;else return false};scope["sendForm"]=function(){$log.log("scope.sendForm");var sFormName=scope["oFormDefinition"][scope["sFormDefinitionName"]]["name"];if(scope[sFormName]["$valid"]===true){var deferred=$q.defer();var promise=deferred.promise;var beforeEvent=scope["oFormDefinition"][scope["sFormDefinitionName"]]["beforeEvent"];
if(typeof beforeEvent!=="undefined")if(goog.isString(beforeEvent))$q.when(formReaderService["callFunction"](beforeEvent)).then(function(){deferred.resolve()});else{if(goog.isFunction(beforeEvent))$q.when(beforeEvent(scope["oFormValues"])).then(function(){deferred.resolve()})}else deferred.resolve();promise.then(function(){deferred=$q.defer();promise=deferred.promise;var submitEvent=scope["oFormDefinition"][scope["sFormDefinitionName"]]["event"];if(typeof submitEvent!=="undefined")if(goog.isString(submitEvent))$q.when(formReaderService["callFunction"](submitEvent)).then(function(){deferred.resolve()});
else{if(goog.isFunction(submitEvent))$q.when(submitEvent(scope["oFormValues"])).then(function(){deferred.resolve()})}else deferred.resolve();promise.then(function(){var afterEvent=scope["oFormDefinition"][scope["sFormDefinitionName"]]["afterEvent"];if(typeof afterEvent!=="undefined")if(goog.isString(afterEvent))formReaderService["callFunction"](afterEvent);else if(goog.isFunction(afterEvent))afterEvent(scope["oFormValues"])})})}};scope["executeButtonEvent"]=function($event,buttonEvent){$log.log("scope.executeButtonEvent");
if(goog.isString(buttonEvent))formReaderService["callFunction"](buttonEvent);else if(goog.isFunction(buttonEvent))buttonEvent(scope["oFormValues"])};scope["getValidationCssClass"]=function(sFieldName){if(typeof sFieldName!="undefined"){var sFormName=scope["oFormDefinition"][scope["sFormDefinitionName"]]["name"];if(typeof scope[sFormName]!="undefined")if(typeof scope[sFormName][sFieldName]!="undefined")if(scope[sFormName][sFieldName].$invalid&&scope[sFormName][sFieldName].$dirty)return"has-error"}};
scope["switchSelectedOptions"]=function(sFormDefinitionName,oFieldDefinition,sFromSelectName,sToSelectName){$log.log("scope.switchSelectedOptions");var oFromSelect=scope["oFormValues"][sFormDefinitionName][sFromSelectName];scope["oFromSelect"]=oFromSelect;if(typeof oFieldDefinition["switch_event"]!=="undefined")if(formReaderService["isFunctionCall"](oFieldDefinition["switch_event"]))if(goog.isString(oFieldDefinition["switch_event"]))formReaderService["callFunction"](oFieldDefinition["switch_event"]);
...
scope["reloadSelectField"]=function(oParentSelect,sFormDefinitionName){formReaderService["reloadSelectField"](oParentSelect,sFormDefinitionName,scope["oFormValues"],scope["oFormDefinition"])};scope["isStringNotEmpty"]=function(element){if(!goog.isDefAndNotNull(element))return false;if(!goog.isString(element))return false;if(!element.length>0)return false;return true};scope["getLinkFileName"]=function(url){if(!goog.isString(url))return url;var sName=url;if(sName.lastIndexOf("/")!==-1)sName=sName.substr(sName.lastIndexOf("/")+
...
true;return useWab};scope["compileTemplate"]=function(){$http({method:"GET",url:propertiesSrvc["web_server_name"]+"/"+sessionStorage["appEnv"]+"/javascript/externs/formReader/formReader.html"}).then(function successCallback(response){if(!goog.isDefAndNotNull(response["data"])){console.error("template not founded");return 0}var sTemplate=response["data"];var compiledTemplate=$compile(sTemplate)(scope);element.children().replaceWith(compiledTemplate)},function errorCallback(response){})};scope.$on("$destroy",
function(){scope.$root["clearFormData"](scope["sFormDefinitionName"],scope)})}}};formReader.module.directive("appFormReader",formReader.formReaderDirective);
...
false);break;case "map_bing":case "map_osm":case "map_vmap":var initMap=function(){if(goog.isDef(scope["Map"])){scope["Map"].MapObject.setTarget(null);delete scope["Map"]}scope["Map"]=new nsVitisComponent.Map(scope["field"]["id"]+"_map",scope["field"]["map_options"],scope["field"]["id"],scope["oFormValues"][scope["sFormDefinitionName"]],scope["field"]["name"],scope.$root["proj"]);scope.$root.$emit("OpenLayersMapCreated",scope["Map"]);scope["Map"].on("moveend",function(){scope["field"]["map_options"]["center"]=
scope["Map"]["getConfig"]();scope.$apply()},scope["Map"])};$timeout(function(){initMap();scope.$watch("field.nb_cols",function(value,old){$timeout(function(){scope["Map"].MapObject.updateSize()})});scope.$watch("field.style",function(value,old){$timeout(function(){scope["Map"].MapObject.updateSize()})},true);scope.$watch("Map.bLayersTreeOpen",function(value,old){$timeout(function(){scope["Map"].MapObject.updateSize()})},true);scope.$on("studio resized",function(){scope["Map"].MapObject.updateSize()});
scope.$on("updateMap",function(event,data){switch(data){case "center":scope["Map"]["setCenter"]([scope["field"]["map_options"]["center"]["coord"][0],scope["field"]["map_options"]["center"]["coord"][1]]);break;case "zoom":scope["Map"]["setZoom"](scope["field"]["map_options"]["center"]["zoom"]);break;case "source":scope["Map"]["setBingSource"](scope["field"]["map_options"]["source"]);break;case "extent":scope["Map"]["setExtent"](scope["field"]["map_options"]["center"]["extent"]);break;case "controls":scope["Map"]["setControls"](scope["field"]["map_options"]["controls"]);
break;case "interact":scope["Map"]["setInteractions"](scope["field"]["map_options"]["interactions"]);break;case "proj":var extent=scope["Map"]["getExtent"]();var center=scope["Map"]["getCenter"]();var proj=scope["Map"]["getProj"]();scope["field"]["map_options"]["center"]["extent"]=scope["Map"]["transformExtent"](extent,proj,scope["field"]["map_options"]["proj"]);scope["field"]["map_options"]["center"]["coord"]=scope["Map"]["transformer"](center,proj,scope["field"]["map_options"]["proj"]);initMap();
break;default:initMap();break}})},10);break;case "slider":scope["field"]["options"]["id"]=scope["field"]["id"]+"_slider";var mySlider=$(element)["slider"](scope["field"]["options"]);if(!goog.isDefAndNotNull(scope["oFormValues"][scope["sFormDefinitionName"]][scope["field"]["name"]]))scope["oFormValues"][scope["sFormDefinitionName"]][scope["field"]["name"]]=0;$(element).on("slide",function(){scope["oFormValues"][scope["sFormDefinitionName"]][scope["field"]["name"]]=$(element)["slider"]("getValue")});
scope.$watch("oFormValues."+scope["sFormDefinitionName"]+'["'+scope["field"]["name"]+'"]',function(){setTimeout(function(){if(goog.isDefAndNotNull($(element)))if(goog.isDefAndNotNull(scope["oFormValues"]))if(goog.isDefAndNotNull(scope["oFormValues"][scope["sFormDefinitionName"]]))if(goog.isDefAndNotNull(scope["oFormValues"][scope["sFormDefinitionName"]][scope["field"]["name"]]))$(element)["slider"]("setValue",parseFloat(scope["oFormValues"][scope["sFormDefinitionName"]][scope["field"]["name"]]))},
10)});scope.$watch("field.options",function(){setTimeout(function(){mySlider=$(element)["slider"](scope["field"]["options"])},10)},true);break;case "color_picker":$(element)["colorpicker"]({format:"rgba"});$(element).on("changeColor",function(){scope["oFormValues"][scope["sFormDefinitionName"]][scope["field"]["name"]]=$(element)["colorpicker"]("getValue")});scope.$watch("oFormValues."+scope["sFormDefinitionName"]+'["'+scope["field"]["name"]+'"]',function(){setTimeout(function(){if(goog.isDefAndNotNull($(element)))if(goog.isDefAndNotNull(scope["oFormValues"]))if(goog.isDefAndNotNull(scope["oFormValues"][scope["sFormDefinitionName"]]))if(goog.isDefAndNotNull(scope["oFormValues"][scope["sFormDefinitionName"]][scope["field"]["name"]]))$(element)["colorpicker"]("setValue",
scope["oFormValues"][scope["sFormDefinitionName"]][scope["field"]["name"]])},10)});break;case "double_select":var ilineHeight=parseInt($(element).css("line-height"))-1;if(typeof scope["field"]["size"]!=="undefined")element[0].style.minHeight=ilineHeight*scope["field"]["size"]+"px";break;case "password":if(typeof scope["field"]["autocomplete"]==="undefined"||scope["field"]["autocomplete"]===false){element[0].readOnly=true;var clearListener=scope.$root.$on("endFormNgRepeat",function(event){$timeout(function(){element[0].readOnly=
false},300);clearListener()})}break}if(typeof scope["field"]["read_only"]!="undefined"&&scope["field"]["read_only"]==true)element[0].readOnly=true}}};formReader.module.directive("appFormFieldSpecificParams",formReader.appFormFieldSpecificParamsDrtv);
...
formReader.appSubformDrtv=function($compile,$log){return{link:function(scope,element,attrs){$log.log("formReader.appSubformDrtv.link");var content;var template="<div app-form-reader ";template+='app-form-definition-name="sSubformFormDefinitionName" ';template+='app-form-values="oSubformFormValues" ';template+='app-form-definition="oSubformFormDefinition"';template+='app-form-events-container="oSubformFormScope"';template+='app-properties="oProperties"';template+='app-token="sToken"></div>';scope["loadSubForm"]=
function(opt_options){$log.log("formReader.loadSubForm");if(!goog.isDefAndNotNull(opt_options["sFormDefinitionName"])){console.error("opt_options['sFormDefinitionName'] undefined");return 0}if(!goog.isDefAndNotNull(opt_options["oFormDefinition"])){console.error("opt_options['oFormDefinition'] undefined");return 0}if(!goog.isDefAndNotNull(opt_options["oFormValues"])){console.error("opt_options['oFormValues'] undefined");return 0}if(!goog.isDefAndNotNull(opt_options["oProperties"])){console.error("opt_options['oProperties'] undefined");
return 0}if(!goog.isDefAndNotNull(opt_options["sToken"])){console.error("opt_options['sToken'] undefined");return 0}scope.$applyAsync(function(){scope["sSubformFormDefinitionName"]=opt_options["sFormDefinitionName"];scope["oSubformFormDefinition"]=opt_options["oFormDefinition"];scope["oSubformFormValues"]=opt_options["oFormValues"];scope["oProperties"]=opt_options["oProperties"];scope["sToken"]=opt_options["sToken"];content=$compile(template)(scope);element.html("");element.append(content)})}}}};
formReader.module.directive("appSubform",formReader.appSubformDrtv);goog.provide("formReader.service.formReaderService");goog.require("formReader");
...
"value":aData[j][sValueKey]});else oNotSelectedOptions["options"].push({"label":aData[j][sLabelKey],"value":aData[j][sValueKey]});j++}if(sFromSelectName!=="")oFormValues[sFromSelectName]=oNotSelectedOptions;else{if(goog.isDefAndNotNull(oSelectedOption))oSelectedOptions["selectedOption"]=oSelectedOption;else{if(oFormElementDefinition["type"]==="select"){var selectedOptionValue=angular.copy(oFormValues[oFormElementDefinition["name"]]);var selectedOptionLabel;if(selectedOptionValue!==null&&goog.isObject(selectedOptionValue)){selectedOptionValue=
selectedOptionValue["selectedOption"]["value"];selectedOptionLabel=oFormValues[oFormElementDefinition["name"]]["selectedOption"]["label"]}if(typeof selectedOptionValue==="undefined"&&typeof oFormElementDefinition["default_value"]!=="undefined"){selectedOptionValue=oFormElementDefinition["default_value"];if(goog.isString(selectedOptionValue))if(this_["isFunctionCall"](selectedOptionValue))selectedOptionValue=this_["callFunction"](selectedOptionValue)}selectedOptionValue=goog.isDef(selectedOptionValue)?
selectedOptionValue:null;selectedOptionLabel=goog.isDef(selectedOptionLabel)?selectedOptionLabel:"";oSelectedOptions["selectedOption"]={"value":selectedOptionValue,"label":selectedOptionLabel}}if(oFormElementDefinition["type"]==="list"){var selectedOptionValue=angular.copy(oFormValues[oFormElementDefinition["name"]]);if(goog.isDefAndNotNull(selectedOptionValue))if(goog.isArray(selectedOptionValue["selectedOption"]))selectedOptionValue=selectedOptionValue["selectedOption"];if(goog.isDefAndNotNull(selectedOptionValue)&&
typeof oFormElementDefinition["default_value"]!=="undefined"){selectedOptionValue=oFormElementDefinition["default_value"];if(goog.isString(selectedOptionValue))if(this_["isFunctionCall"](selectedOptionValue))selectedOptionValue=this_["callFunction"](selectedOptionValue)}if(goog.isArray(selectedOptionValue)&&selectedOptionValue.length>0)oSelectedOptions["selectedOption"]=selectedOptionValue;else oSelectedOptions["selectedOption"]={"value":null,"label":""}}}if(goog.isDefAndNotNull(oFormElementDefinition["options"])){var i=
0,aOptions,aOptionsKeys=[],aOptionsValues=[];while(i<oFormElementDefinition["options"].length){aOptions=oFormElementDefinition["options"][i].split("|");aOptionsKeys.push(aOptions[0]);aOptionsValues.push(aOptions[1]);i++}$translate(aOptionsKeys).then(function(translations){var aTranslationsKeys=Object.keys(translations);i=0;while(i<aTranslationsKeys.length){oSelectedOptions["options"].unshift({"label":translations[aTranslationsKeys[i]],"value":aOptionsValues[i]});i++}})}if(oFormElementDefinition["required"]!==
true&&oFormElementDefinition["required"]!=="true"){var isEmptyValuePresent=false;for(var i=0;i<oSelectedOptions["options"].length;i++)if(oSelectedOptions["options"][i]["value"]==="")isEmptyValuePresent=true;if(!isEmptyValuePresent)oSelectedOptions["options"].unshift({"label":"","value":""})}}oFormValues[sToSelectName]=oSelectedOptions;var selectedIsInOption=false;for(var i=0;i<oFormValues[sToSelectName]["options"].length;i++)if(oSelectedOptions["selectedOption"]["value"]===oFormValues[sToSelectName]["options"][i]["value"]){oSelectedOptions["selectedOption"]["label"]=
oFormValues[sToSelectName]["options"][i]["label"];selectedIsInOption=true}if(!selectedIsInOption){delete oFormValues[sToSelectName]["selectedOption"]["value"];delete oFormValues[sToSelectName]["selectedOption"]["label"]}if(!goog.isDefAndNotNull(oFormValues[sToSelectName]))oFormValues[sToSelectName]={};if(!goog.isDefAndNotNull(oFormValues[sToSelectName]["selectedOption"]))oFormValues[sToSelectName]["selectedOption"]={};if(!goog.isDefAndNotNull(oFormValues[sToSelectName]["selectedOption"]["value"])){oFormValues[sToSelectName]["selectedOption"]["value"]=
"";oFormValues[sToSelectName]["selectedOption"]["label"]=""}if(oFormElementDefinition["type"]==="list")if(goog.isObject(oFormValues[sToSelectName]["selectedOption"])&&!goog.isArray(oFormValues[sToSelectName]["selectedOption"]))oFormValues[sToSelectName]["selectedOption"]=[oFormValues[sToSelectName]["selectedOption"]];this["oFormValues"]=oFormValues;this["sFormDefinitionName"]=sFormDefinitionName;envSrvc["oFormDefaultValues"][sFormDefinitionName]=this_["copyFormValues"](oFormValues);if(typeof oWebService["callback"]!==
"undefined")if(this_["isFunctionCall"](oWebService["callback"]))this_["callFunction"](oWebService["callback"]);$rootScope.$emit("webServiceSelectOptionsloaded");deferred.resolve()})},"getWebServiceData":function(oWebService){var sToken=this["sToken"];var oProperties=this["oProperties"];var oWebService=oWebService;var oParams={};var deferred=$q.defer();if(!goog.isDefAndNotNull(oWebService)){console.error("oWebService not defined");return 0}if(goog.isDef(oWebService["parameters"])){oParams=oWebService["parameters"];
if(oWebService["dataType"]==="businessObject")if(goog.isDefAndNotNull(oParams["attributs"]))delete oParams["attributs"];if(goog.isDef(oWebService["parameters"]["filter"])||goog.isDef(oWebService["parameters"]["filter_extension"])){if(goog.isObject(oWebService["parameters"]["filter"]))oWebService["parameters"]["filter"]=this["parseFilter"](oWebService["parameters"]["filter"]);if(oWebService["parameters"]["filter"]==="")delete oWebService["parameters"]["filter"]}}oParams["token"]=sToken;$http({method:"POST",
...
aParams[i].trim()}for(var i=0;i<aParams.length;i++)if(this["isFunctionCall"](aParams[i]))aParams[i]="'"+this["callFunction"](aParams[i])+"'";if(aParams.length>0)sFunction=sFunction.replace(sParams,aParams.join(","));var sFunctionName=sFunction.substr(0,sFunction.indexOf("("));var returnMessage;this["eventsContainer"]=this["oFormEventsContainer"];this["rootScope_"]=$rootScope;if(typeof eval("window."+sFunctionName)=="function")try{$log.log(sFunction);returnMessage=eval(sFunction)}catch(e){}else if(typeof this["eventsContainer"]!=
"undefined"&&typeof this["eventsContainer"][sFunctionName]=="function")try{$log.log("eventsContainer."+sFunction);returnMessage=eval("this.eventsContainer."+sFunction)}catch(e){}else if(typeof this[sFunctionName]=="function")try{$log.log("this."+sFunction);returnMessage=eval("this."+sFunction)}catch(e){}else if(typeof this["rootScope_"]!="undefined"&&typeof this["rootScope_"][sFunctionName]=="function")try{$log.log("$rootScope."+sFunction);returnMessage=eval("this.rootScope_."+sFunction)}catch(e){}return returnMessage},
"isFunctionCall":function(sString,bCheckFunction){if(typeof bCheckFunction=="undefined")bCheckFunction=true;var bReturn=false;if(typeof sString==="string"){aMatchResult=sString.match(/(.+)\((.*)\)/);if(aMatchResult!==null)if(bCheckFunction){var sFunctionName=sString.substr(0,sString.indexOf("("));var returnMessage;if(typeof sFunctionName=="function")bReturn=true;else if(typeof this["oFormEventsContainer"]!="undefined"&&typeof this["oFormEventsContainer"][sFunctionName]=="function")bReturn=true;else if(typeof this[sFunctionName]==
"function")bReturn=true;else if(typeof $rootScope[sFunctionName]=="function")bReturn=true;if(!bReturn)$log.warn("Function not found : "+sString)}else bReturn=true}return bReturn},"selectFirstOption":function(sFormElementName,oFormValues,sFormDefinitionName){oFormValues=goog.isDef(oFormValues)?oFormValues:this["oFormValues"];sFormDefinitionName=goog.isDef(sFormDefinitionName)?sFormDefinitionName:this["sFormDefinitionName"];oFormValues=oFormValues[sFormDefinitionName];if(oFormValues[sFormElementName]["options"].length>
0)oFormValues[sFormElementName]["selectedOption"]["value"]=oFormValues[sFormElementName]["options"][0]["value"]},"setSelectOptions":function(oFormElementDefinition,sFormDefinitionName,oFormValues,aOptions){$log.log("setSelectOptions");var getFormatedOptions=function(aKeys,aValues){var aOptions=[];if(!goog.isArray(aKeys)){console.error("getFormatedOptions: aKeys is not an array",aKeys);return aOptions}if(!goog.isArray(aValues)){console.error("getFormatedOptions: aKeys is not an array",aValues);return aOptions}if(aKeys.length!==
aValues.length){console.error("getFormatedOptions: aKeys.length !== aValues.length",aKeys,aValues);return aOptions}for(var i=0;i<aKeys.length;i++)aOptions.push({"label":aKeys[i],"value":aValues[i]});return aOptions};var i=0,sLabel,sValue,aOption;var oSelectOptions={"selectedOption":{"value":""},"options":[]};var sSelectName,sFromSelectName,sToSelectName;aOptions=goog.isDef(aOptions)?aOptions:oFormElementDefinition["options"];var aOptionsKeys=[];var aOptionsValues=[];if(goog.isString(aOptions))aOptions=
aOptions.split(",");while(i<aOptions.length){aOption=aOptions[i].split("|");sLabel=aOption[0];aOptionsKeys.push(sLabel);if(aOption.length>1)sValue=aOption[1];else sValue=sLabel;aOptionsValues.push(sValue);i++}var selectValue=oFormValues[sFormDefinitionName][oFormElementDefinition["name"]];if(oFormElementDefinition["type"]==="list"&&goog.isString(selectValue)){var aSelectedValues=selectValue.split("|");if(aSelectedValues.length>0){var aSelectedOptions=[];for(var i=0;i<aSelectedValues.length;i++)aSelectedOptions.push({"label":"",
"value":aSelectedValues[i]});selectValue={"selectedOption":aSelectedOptions}}}if(typeof selectValue==="undefined"&&typeof oFormElementDefinition["default_value"]!=="undefined"){selectValue=oFormElementDefinition["default_value"];if(typeof selectValue==="string")if(this["isFunctionCall"](selectValue))selectValue=this["callFunction"](selectValue)}if(goog.isDefAndNotNull(selectValue))if(goog.isDefAndNotNull(selectValue["selectedOption"])){if(goog.isDefAndNotNull(selectValue["selectedOption"]["value"]))oSelectOptions=
selectValue;if(goog.isArray(selectValue["selectedOption"]))oSelectOptions=selectValue}else oSelectOptions["selectedOption"]["value"]=String(selectValue);$translate(aOptionsKeys).then(function(translations){for(var i=0;i<aOptionsKeys.length;i++)aOptionsKeys[i]=translations[aOptionsKeys[i]];oSelectOptions["options"]=getFormatedOptions(aOptionsKeys,aOptionsValues);if(oFormElementDefinition["type"]==="double_select"){sFromSelectName=oFormElementDefinition["name_from"];sToSelectName=oFormElementDefinition["name_to"];
var aFromOptionsKeys=angular.copy(aOptionsKeys);var aFromOptionsValues=angular.copy(aOptionsValues);var aToOptionsKeys=[];var aToOptionsValues=[];if(goog.isDefAndNotNull(selectValue))for(var i=aFromOptionsValues.length-1;i>=0;i--)if(selectValue.indexOf(aFromOptionsValues[i])!==-1){aToOptionsValues.unshift(aFromOptionsValues[i]);aToOptionsKeys.unshift(aFromOptionsKeys[i]);aFromOptionsValues.splice(i,1);aFromOptionsKeys.splice(i,1)}var oFromSelectOptions={"options":getFormatedOptions(aFromOptionsKeys,
aFromOptionsValues)};var oToSelectOptions={"options":getFormatedOptions(aToOptionsKeys,aToOptionsValues)};oFormValues[sFormDefinitionName][sFromSelectName]=oFromSelectOptions;oFormValues[sFormDefinitionName][sToSelectName]=oToSelectOptions}else{sSelectName=oFormElementDefinition["name"];oFormValues[sFormDefinitionName][sSelectName]=oSelectOptions}if(typeof envSrvc["oFormDefaultValues"][sFormDefinitionName]==="undefined")envSrvc["oFormDefaultValues"][sFormDefinitionName]={};if(typeof envSrvc["oFormDefaultValues"][sFormDefinitionName][sSelectName]==
"undefined")envSrvc["oFormDefaultValues"][sFormDefinitionName][sSelectName]=angular.copy(oSelectOptions)})},"selectOptionAfterLoad":function(oFormValues,oFormElementDefinition,sFormDefinitionName,sFieldName,value){if(oFormElementDefinition["type"]==="double_select");else if(oFormElementDefinition["type"]==="select"){var opt=oFormValues[sFormDefinitionName][sFieldName].options;for(var i=0;i<opt.length;i++)if(opt[i].value===value){oFormValues[sFormDefinitionName][sFieldName].selectedOption=opt[i];break}}else if(oFormElementDefinition["type"]===
"list");else console.error("this element is not a select")},"extractWebServiceData":function(sController,oWebServiceData){var sKey=sController;var aData=[];if(typeof oWebServiceData[sKey]!=="undefined")if(Array.isArray(oWebServiceData[sKey])){if(oWebServiceData[sKey].length>0)aData=oWebServiceData[sKey]}else aData=oWebServiceData[sKey];return aData},"parseFilter":function(filter){if(goog.isObject(filter)){var aFilters=[];var aFiltersKeys=Object.keys(filter),sFilterValue;var i=0;for(var i=0;i<aFiltersKeys.length;i++)if(aFiltersKeys[i]===
"stringFilter")aFilters.push(filter[aFiltersKeys[i]]);else{sFilterValue=filter[aFiltersKeys[i]];if(sFilterValue!==""){if(this["isFunctionCall"](sFilterValue))sFilterValue=this["callFunction"](sFilterValue);if(typeof sFilterValue!="undefined")if(goog.isString(sFilterValue))if(sFilterValue.toLowerCase().indexOf("null")!=-1)aFilters.push(aFiltersKeys[i]+" "+sFilterValue);else aFilters.push(aFiltersKeys[i]+"='"+sFilterValue+"'");else aFilters.push(aFiltersKeys[i]+"="+sFilterValue)}}if(aFilters.length>
0)filter=aFilters.join(" AND ");else filter=""}return filter},"setWebServiceTags":function(oFormElementDefinition,sFormDefinitionName,oFormValues){var oWebService=angular.copy(oFormElementDefinition["web_service"]);var aRessourceId=oWebService["ressource_id"].split("/");var oFormValues=oFormValues[sFormDefinitionName],selectedOptionValue;var oFormSrvc=this;var oParams={};if(typeof oWebService["parameters"]!=="undefined"){oParams=oWebService["parameters"];if(typeof oWebService["parameters"]["filter"]!==
"undefined"){oWebService["parameters"]["filter"]=this["parseFilter"](oWebService["parameters"]["filter"]);if(oWebService["parameters"]["filter"]==="")delete oWebService["parameters"]["filter"]}}oParams["token"]=this["sToken"];$http({method:"POST",url:this["oProperties"]["web_server_name"]+"/"+this["oProperties"]["services_alias"]+"/"+oWebService["ressource_id"],headers:{"X-HTTP-Method-Override":"GET"},data:oParams}).then(function(response){var data=response["data"];var j,oNotSelectedOptions,oSelectedOptions,
sFromSelectName="",sToSelectName;if(data["status"]===1){var aData=oFormSrvc["extractWebServiceData"](aRessourceId[1],data);var aTags=[];if(typeof oFormValues[oFormElementDefinition["name"]]==="string")aTags=oFormValues[oFormElementDefinition["name"]].split(",");var sLabelKey=oWebService["label_key"];var sValueKey=oWebService["id_key"];var i=0;var aAvailableTags=[];while(i<aData.length){if(aTags.indexOf(aData[i][sValueKey])===-1)aAvailableTags.push(aData[i][sValueKey]);i++}oFormValues[oFormElementDefinition[["name_available_tags"]]]=
aAvailableTags.join(",");if(typeof envSrvc["oFormDefaultValues"][sFormDefinitionName][oFormElementDefinition[["name_available_tags"]]]=="undefined")envSrvc["oFormDefaultValues"][sFormDefinitionName][oFormElementDefinition[["name_available_tags"]]]=angular.copy(oFormValues[oFormElementDefinition[["name_available_tags"]]]);$rootScope.$emit("webServiceTagsloaded",oFormElementDefinition)}})},"checkFormModifications":function(sFormDefinitionName){var deferred=$q.defer();var promise=deferred.promise;if((envSrvc["sMode"]==
"insert"||envSrvc["sMode"]=="update")&&typeof sFormDefinitionName!="undefined"){var oFormDefinition=envSrvc["oFormDefinition"][sFormDefinitionName];if(typeof oFormDefinition!="undefined"&&typeof oFormDefinition["event"]!="undefined"){var sFormName=oFormDefinition["name"];var oFormElement=angular.element("form[name='"+sFormName+"']");if(oFormElement.length>0){var formScope=oFormElement.scope();if(formScope[sFormName]["$dirty"]&&!formScope[sFormName]["$submitted"]){var oOptions={"className":"modal-warning",
"message":"FORM_MODIFIED_WARNING","callback":function(bResponse){if(bResponse)deferred.resolve()}};formScope.$root["modalWindow"]("confirm","FORM_MODIFIED_WARNING_TITLE",oOptions)}else deferred.resolve()}else deferred.resolve()}else deferred.resolve()}else deferred.resolve();return promise},"getFormValueFromObject":function(oFormValues,sFormFieldName){$log.info("getFormValueFromObject");var sReturn=this["copyFormValues"](oFormValues);var aFormFieldName=sFormFieldName.split(".");aFormFieldName.forEach(function(sParameter){sReturn=
sReturn[sParameter]});return sReturn},"copyFormValues":function(oFormValues){$log.info("copyFormValues");var oReturn={};try{oReturn=angular.copy(oFormValues)}catch(e){for(var key in oFormValues)try{oReturn[key]=angular.copy(oFormValues[key])}catch(e){if(goog.isObject(oFormValues[key])){oReturn[key]={};for(var key2 in oFormValues[key])try{oReturn[key][key2]=angular.copy(oFormValues[key][key2])}catch(e){}}}}return oReturn},"getFormValue":function(sName){$log.info("getFormValue");if(!goog.isString(sName))return null;
sName=sName.trim();var oFormValues=this["oFormValues"][this["sFormDefinitionName"]];var sValue=oFormValues[sName.trim()];if(goog.isDefAndNotNull(sValue["selectedOption"]))if(goog.isDefAndNotNull(sValue["selectedOption"]["value"]))sValue=sValue["selectedOption"]["value"];return sValue},"concat":function(){$log.info("concat");var sResult="";for(var i=0;i<arguments.length;i++)if(goog.isString(arguments[i]))sResult+=arguments[i];return sResult},"getBusinessObjectDef":function(sBusinessObject){var sToken=
this["sToken"];var oProperties=this["oProperties"];var deferred=$q.defer();$http({method:"GET",url:oProperties["api_url"]+"/vmap/businessobjects/"+sBusinessObject,params:{"token":sToken}}).then(function(response){if(!goog.isDef(response["data"])){$translate("NO_VALUES_RETURNED").then(function(sTranslation){bootbox.alert(sTranslation)});return 0}if(!goog.isDef(response["data"]["data"])){$translate("NO_BUSINESS_OBJECT_FOUNDED").then(function(sTranslation){bootbox.alert(sTranslation)});return 0}if(!goog.isDef(response["data"]["data"][0])){$translate("NO_BUSINESS_OBJECT_FOUNDED").then(function(sTranslation){bootbox.alert(sTranslation)});
return 0}if(!goog.isDef(response["data"]["data"][0]["business_object_id"])){$translate("NO_BUSINESS_OBJECT_FOUNDED").then(function(sTranslation){bootbox.alert(sTranslation)});return 0}if(goog.isDef(response["data"]["errorMessage"])){bootbox.alert(response["data"]["errorMessage"]);return 0}deferred.resolve(response["data"]["data"][0])},function(response){console.error(response);bootbox.alert(response)});return deferred.promise},"showModalSubform":function(sModalId,oSubformScope,oSubformDefinition,
oSubformDefinitionName,oSubformValues){var sToken=this["sToken"];var oProperties=this["oProperties"];if(goog.isDefAndNotNull(oSubformScope["loadSubForm"])){oSubformScope["loadSubForm"]({"sFormDefinitionName":oSubformDefinitionName,"oFormDefinition":oSubformDefinition,"oFormValues":oSubformValues,"oProperties":oProperties,"sToken":sToken});$("#"+sModalId).modal("show")}},"updateBoElement":function(sBusinessObject,oObject,oSubformDefinition,sSubformDefinitionName,oFormValuesResulted,successCallback){var sToken=
this["sToken"];var oProperties=this["oProperties"];var envSrvc=angular.element(vitisApp.appMainDrtv).injector().get(["envSrvc"]);var formSrvc=angular.element(vitisApp.appMainDrtv).injector().get(["formSrvc"]);var aParamsToSave=[oObject["bo_id_field"]];envSrvc["oFormValues"]=oFormValuesResulted;envSrvc["oFormDefinition"]=oSubformDefinition;envSrvc["sFormDefinitionName"]=sSubformDefinitionName;var oValues=formSrvc["getFormData"](sSubformDefinitionName,true);for(var i=0;i<aParamsToSave.length;i++)if(goog.isDefAndNotNull(oFormValuesResulted[sSubformDefinitionName][aParamsToSave[i]]))oValues[aParamsToSave[i]]=
oFormValuesResulted[sSubformDefinitionName][aParamsToSave[i]];var oFormData=new FormData;for(var key in oValues)if(goog.isDefAndNotNull(oValues[key]["aFiles"])){oFormData.append(key+"_attached_content",oValues[key]["aFiles"][0]);oFormData.append(key,oValues[key]["aFiles"][0]["name"])}else oFormData.append(key,oValues[key]);$http({method:"PUT",url:oProperties["api_url"]+"/vmap/querys/"+sBusinessObject,data:oFormData,params:{"token":sToken}}).then(function(response){if(!goog.isDef(response["data"])){$translate("NO_VALUES_RETURNED").then(function(sTranslation){bootbox.alert(sTranslation)});
return 0}if(goog.isDef(response["data"]["errorMessage"])){bootbox.alert(response["data"]["errorMessage"]);return 0}if(goog.isDef(response["status"]))if(response["status"]===500){bootbox.alert(response["statusText"]);return 0}successCallback.call()},function(response){bootbox.alert(response);console.error(response)})},"insertBoElement":function(sBusinessObject,oBusinessObject,oSubformDefinition,sSubformDefinitionName,oFormValuesResulted,successCallback){var sToken=this["sToken"];var oProperties=this["oProperties"];
var envSrvc=angular.element(vitisApp.appMainDrtv).injector().get(["envSrvc"]);var formSrvc=angular.element(vitisApp.appMainDrtv).injector().get(["formSrvc"]);var aParamsToSave=[oBusinessObject["id_field"]];envSrvc["oFormValues"]=oFormValuesResulted;envSrvc["oFormDefinition"]=oSubformDefinition;envSrvc["sFormDefinitionName"]=sSubformDefinitionName;var oValues=formSrvc["getFormData"](sSubformDefinitionName,true);for(var i=0;i<aParamsToSave.length;i++)if(goog.isDefAndNotNull(oFormValuesResulted[sSubformDefinitionName][aParamsToSave[i]]))oValues[aParamsToSave[i]]=
oFormValuesResulted[sSubformDefinitionName][aParamsToSave[i]];var oFormData=new FormData;for(var key in oValues)if(goog.isDefAndNotNull(oValues[key]["aFiles"])){oFormData.append(key+"_attached_content",oValues[key]["aFiles"][0]);oFormData.append(key,oValues[key]["aFiles"][0]["name"])}else oFormData.append(key,oValues[key]);$http({method:"POST",url:oProperties["api_url"]+"/vmap/querys/"+sBusinessObject,data:oFormData,params:{"token":sToken}}).then(function(response){if(!goog.isDef(response["data"])){$translate("NO_VALUES_RETURNED").then(function(sTranslation){bootbox.alert(sTranslation)});
return 0}if(goog.isDef(response["data"]["errorMessage"])){bootbox.alert(response["data"]["errorMessage"]);return 0}if(goog.isDef(response["status"]))if(response["status"]===500){bootbox.alert(response["statusText"]);return 0}successCallback.call()},function(response){bootbox.alert(response);console.error(response)})},"deleteBoElements":function(sBusinessObject,aIds,successCallback){var sToken=this["sToken"];var oProperties=this["oProperties"];$http({method:"DELETE",url:oProperties["api_url"]+"/vmap/querys/"+
sBusinessObject,params:{"token":sToken,"idList":aIds.join("|")}}).then(function(response){if(!goog.isDef(response["data"])){$translate("NO_VALUES_RETURNED").then(function(sTranslation){bootbox.alert(sTranslation)});return 0}if(goog.isDef(response["data"]["errorMessage"])){bootbox.alert(response["data"]["errorMessage"]);return 0}if(goog.isDef(response["status"]))if(response["status"]===500){bootbox.alert(response["statusText"]);return 0}successCallback.call()},function(response){bootbox.alert(response);
console.error(response)})},"getSectionDef":function(sSection){var sToken=this["sToken"];var oProperties=this["oProperties"];var deferred=$q.defer();$http({method:"GET",url:oProperties["api_url"]+"/vitis/vitissections/"+sSection,params:{"token":sToken}}).then(function(response){if(!goog.isDef(response["data"])){$translate("NO_VALUES_RETURNED").then(function(sTranslation){bootbox.alert(sTranslation)});return 0}if(!goog.isDef(response["data"]["data"])){$translate("NO_SECTION_FOUNDED").then(function(sTranslation){bootbox.alert(sTranslation)});
return 0}if(!goog.isDef(response["data"]["data"][0])){$translate("NO_SECTION_FOUNDED").then(function(sTranslation){bootbox.alert(sTranslation)});return 0}if(!goog.isDef(response["data"]["data"][0]["ressource_id"])){$translate("NO_SECTION_FOUNDED").then(function(sTranslation){bootbox.alert(sTranslation)});return 0}if(goog.isDef(response["data"]["errorMessage"])){bootbox.alert(response["data"]["errorMessage"]);return 0}deferred.resolve(response["data"]["data"][0])},function(response){bootbox.alert(response);
console.error(response)});return deferred.promise},"getSectionRessourceTab":function(sRessourceId){var sToken=this["sToken"];var oProperties=this["oProperties"];var deferred=$q.defer();$http({method:"GET",url:oProperties["api_url"]+"/vitis/ressources/"+sRessourceId,headers:{"Accept":"application/json"},params:{"token":sToken}}).then(function(response){if(!goog.isDef(response["data"])){$translate("NO_VALUES_RETURNED").then(function(sTranslation){bootbox.alert(sTranslation)});return 0}if(!goog.isDef(response["data"]["ressources"])){$translate("NO_VALUES_RETURNED").then(function(sTranslation){bootbox.alert(sTranslation)});
return 0}if(!goog.isDef(response["data"]["ressources"][0])){$translate("NO_VALUES_RETURNED").then(function(sTranslation){bootbox.alert(sTranslation)});return 0}if(goog.isDef(response["data"]["errorMessage"])){bootbox.alert(response["data"]["errorMessage"]);return 0}deferred.resolve(response["data"]["ressources"][0])},function(response){bootbox.alert(response);console.error(response)});return deferred.promise},"getSectionRessourceData":function(sRessourceId,sFilter){var sToken=this["sToken"];var oProperties=
this["oProperties"];var deferred=$q.defer();var oParmams={"token":sToken,"limit":100};if(goog.isDefAndNotNull(sFilter)&&sFilter!=="")oParmams["filter"]=sFilter;$http({method:"GET",url:oProperties["api_url"]+"/"+sRessourceId,params:oParmams}).then(function(response){if(!goog.isDef(response["data"])){$translate("NO_VALUES_RETURNED").then(function(sTranslation){bootbox.alert(sTranslation)});return 0}if(!goog.isDef(response["data"]["data"])){$translate("NO_VALUES_RETURNED").then(function(sTranslation){bootbox.alert(sTranslation)});
return 0}if(!goog.isDef(response["data"]["data"][0])){$translate("NO_VALUES_RETURNED").then(function(sTranslation){bootbox.alert(sTranslation)});return 0}if(goog.isDef(response["data"]["errorMessage"])){bootbox.alert(response["data"]["errorMessage"]);return 0}deferred.resolve(response["data"]["data"])},function(response){bootbox.alert(response);console.error(response)});return deferred.promise},"getSectionForm":function(oSection,bHideButtons){bHideButtons=goog.isDefAndNotNull(bHideButtons)?bHideButtons:
false;var deferred=$q.defer();var sFileName;if(oSection["index"]>1)sFileName=oSection["mode_id"]+"_"+oSection["tab_name"]+"_"+oSection["name"]+".json";else sFileName=oSection["mode_id"]+"_"+oSection["tab_name"]+".json";$http({method:"GET",url:"modules/"+oSection["module_name"]+"/forms/"+oSection["mode_id"]+"/"+sFileName}).then(function(response){if(!goog.isDef(response["data"])){$translate("NO_FORM_FOUNDED").then(function(sTranslation){bootbox.alert(sTranslation)});return 0}var oForm=response["data"];
if(bHideButtons)for(var key in oForm)if(goog.isDefAndNotNull(oForm[key]))if(goog.isDefAndNotNull(oForm[key]["rows"]))for(var i=oForm[key]["rows"].length-1;i>=0;i--)if(goog.isDefAndNotNull(oForm[key]["rows"][i]["fields"]))for(var ii=oForm[key]["rows"][i]["fields"].length-1;ii>=0;ii--)if(oForm[key]["rows"][i]["fields"][ii]["type"]==="button")oForm[key]["rows"][i]["fields"].splice(ii,1);deferred.resolve(oForm)},function(response){bootbox.alert(response);console.error(response)});return deferred.promise},
"updateSectionElement":function(sRessourceId,sIdField,oSubformDefinition,sSubformDefinitionName,oFormValuesResulted){var deferred=$q.defer();var sToken=this["sToken"];var oProperties=this["oProperties"];var sId=oFormValuesResulted[sSubformDefinitionName][sIdField];var sUrl;var envSrvc=angular.element(vitisApp.appMainDrtv).injector().get(["envSrvc"]);var formSrvc=angular.element(vitisApp.appMainDrtv).injector().get(["formSrvc"]);envSrvc["oFormValues"]=oFormValuesResulted;envSrvc["oFormDefinition"]=
oSubformDefinition;envSrvc["sFormDefinitionName"]=sSubformDefinitionName;var oFormData=formSrvc["getFormData"](envSrvc["sFormDefinitionName"]);if(goog.isDefAndNotNull(sId))sUrl=oProperties["api_url"]+"/"+sRessourceId+"/"+sId;else sUrl=oProperties["api_url"]+"/"+sRessourceId;$http({method:"PUT",url:sUrl,data:oFormData,params:{"token":sToken}}).then(function(response){if(!goog.isDef(response["data"])){$translate("NO_VALUES_RETURNED").then(function(sTranslation){bootbox.alert(sTranslation)});return 0}if(goog.isDef(response["data"]["errorMessage"])){bootbox.alert(response["data"]["errorMessage"]);
return 0}if(goog.isDef(response["status"]))if(response["status"]===500){bootbox.alert(response["statusText"]);return 0}deferred.resolve(response["data"])},function(response){console.error(response);bootbox.alert(response)});return deferred.promise},"insertSectionElement":function(sRessourceId,sIdField,oSubformDefinition,sSubformDefinitionName,oFormValuesResulted){var deferred=$q.defer();var sToken=this["sToken"];var oProperties=this["oProperties"];var sId=oFormValuesResulted[sSubformDefinitionName][sIdField];
var sUrl;var envSrvc=angular.element(vitisApp.appMainDrtv).injector().get(["envSrvc"]);var formSrvc=angular.element(vitisApp.appMainDrtv).injector().get(["formSrvc"]);envSrvc["oFormValues"]=oFormValuesResulted;envSrvc["oFormDefinition"]=oSubformDefinition;envSrvc["sFormDefinitionName"]=sSubformDefinitionName;var oFormData=formSrvc["getFormData"](envSrvc["sFormDefinitionName"]);$http({method:"POST",url:oProperties["api_url"]+"/"+sRessourceId,data:oFormData,params:{"token":sToken}}).then(function(response){if(!goog.isDef(response["data"])){$translate("NO_VALUES_RETURNED").then(function(sTranslation){bootbox.alert(sTranslation)});
return 0}if(goog.isDef(response["data"]["errorMessage"])){bootbox.alert(response["data"]["errorMessage"]);return 0}if(goog.isDef(response["status"]))if(response["status"]===500){bootbox.alert(response["statusText"]);return 0}deferred.resolve(response["data"])},function(response){console.error(response);bootbox.alert(response)});return deferred.promise},"deleteSectionElements":function(sRessourceId,aIds){var deferred=$q.defer();var sToken=this["sToken"];var oProperties=this["oProperties"];$http({method:"DELETE",
url:oProperties["api_url"]+"/"+sRessourceId,params:{"token":sToken,"idList":aIds.join("|")}}).then(function(response){if(!goog.isDef(response["data"])){$translate("NO_VALUES_RETURNED").then(function(sTranslation){bootbox.alert(sTranslation)});return 0}if(goog.isDef(response["data"]["errorMessage"])){bootbox.alert(response["data"]["errorMessage"]);return 0}if(goog.isDef(response["status"]))if(response["status"]===500){bootbox.alert(response["statusText"]);return 0}deferred.resolve(response["data"])},
function(response){console.error(response);bootbox.alert(response)});return deferred.promise},"disableFormElement":function(oFormDefinition,sElement,sFormDefinitionName){if(!goog.isDefAndNotNull(oFormDefinition)){console.error("oFormDefinition not defined");return null}if(!goog.isDefAndNotNull(sElement)){console.error("sElement not defined");return null}var aFormNames=[];if(goog.isDefAndNotNull(sFormDefinitionName))aFormNames=[sFormDefinitionName];else aFormNames=Object.keys(oFormDefinition);for(var i=
0;i<aFormNames.length;i++)if(goog.isDefAndNotNull(oFormDefinition[aFormNames[i]]))if(goog.isArray(oFormDefinition[aFormNames[i]]["rows"]))for(var ii=0;ii<oFormDefinition[aFormNames[i]]["rows"].length;ii++)if(goog.isDefAndNotNull(oFormDefinition[aFormNames[i]]["rows"][ii]["fields"]))for(var iii=0;iii<oFormDefinition[aFormNames[i]]["rows"][ii]["fields"].length;iii++)if(oFormDefinition[aFormNames[i]]["rows"][ii]["fields"][iii]["name"]===sElement)oFormDefinition[aFormNames[i]]["rows"][ii]["fields"][iii]["disabled"]=
true;return oFormDefinition}}};formReader.module.service("formReaderService",formReader.formReaderService);
