...
visible:false}];if(goog.isArray(scope["field"]["attributes_def"]))for(var i=0;i<scope["field"]["attributes_def"].length;i++)aColumns.push({name:scope["field"]["attributes_def"][i]["name"],displayName:scope["field"]["attributes_def"][i]["label"]});scope["gridOptions"]={"enableSorting":true,"enableRowSelection":true,"enableSelectAll":true,"multiSelect":false,"columnDefs":aColumns,"data":[]};scope["featureForm"]={"draw":{"symbol":"fa-home","color":"red","outline_color":"white","size":4,"dash":20},"text":{"text":"",
"size":14,"font":"Arial","color":"white","outline_color":"black","outline_size":1,"offsetX":0,"offsetY":10,"rotation":0},"attributes":{}};scope["aIcons"]=[];scope["olSelectedFeature"]=null;scope["sSelectedFeatureType"]=null;scope["bFormDisabled"]=true;scope["aFeatures"]=[];scope["custom_form"]=null;scope["oSubformScope"]=null;var lastRowChange=Date.now();var lastRowChangeTimeout=100;var lastRowSelected=null;scope["clearSelectedFeature"]=function(bremoveInteraction){scope["olSelectedFeature"]=null;
if(goog.isDefAndNotNull(scope["oMap"].interaction))if(goog.isFunction(scope["oMap"].interaction.getFeatures))if(goog.isDefAndNotNull(scope["oMap"].interaction.getFeatures())){scope["oMap"].interaction.getFeatures().clear();if(bremoveInteraction)scope["oMap"].MapObject.removeInteraction(scope["oMap"].interaction)}lastRowSelected=null;scope["gridApi"]["selection"]["clearSelectedRows"]()};scope["closeModal"]=function(identifier){$(identifier).modal("hide")};scope["gridOptions"]["onRegisterApi"]=function(gridApi){scope["gridApi"]=
...
nsVitisComponent.Map.prototype.getFeaturesByString=function(sFeatures){this.$log_.info("nsVitisComponent.Map.getFeaturesByString");var aFeatures=[];if(this.outputFormat_==="wkt")try{aFeatures=this.wktFormat.readFeatures(sFeatures,{dataProjection:this.baseProj_,featureProjection:this.proj_})}catch(e){console.error("cannot read the geometry on format wkt")}else if(this.outputFormat_==="geojson")try{aFeatures=this.geoJSONFormat.readFeatures(sFeatures,{dataProjection:this.baseProj_,featureProjection:this.proj_});
for(var i=0;i<aFeatures.length;i++)if(goog.isDefAndNotNull(aFeatures[i].get("style")))aFeatures[i].setStyleByJSON(aFeatures[i].get("style"))}catch(e){console.error("cannot read the geometry on format geojson")}return aFeatures};
nsVitisComponent.Map.prototype.getStringByFeatures=function(aFeatures){this.$log_.info("nsVitisComponent.Map.getStringByFeatures");var sFeatures="";for(var i=0;i<aFeatures.length;i++){var olStyle=aFeatures[i].getStyleAsJSON();aFeatures[i].setProperties({style:goog.isDefAndNotNull(olStyle)?olStyle:null})}if(this.outputFormat_==="wkt")sFeatures=this.wktFormat.writeFeatures(aFeatures,{dataProjection:this.baseProj_,featureProjection:this.proj_});else if(this.outputFormat_==="geojson")sFeatures=this.geoJSONFormat.writeFeatures(aFeatures,
{dataProjection:this.baseProj_,featureProjection:this.proj_});return sFeatures};
...
formReader.formReaderController.prototype.getNewFilter=function(oFormValues,aParents,originalFilter){var additionalFilter={"relation":"AND","operators":[]};aParents=goog.isDefAndNotNull(aParents)?aParents:[];if(!goog.isDefAndNotNull(originalFilter)||originalFilter.trim()==="")originalFilter={"relation":"AND","operators":[]};for(var i=0;i<aParents.length;i++){if(!goog.isDef(oFormValues[aParents[i]["name"]]))continue;if(!goog.isDef(oFormValues[aParents[i]["name"]]["selectedOption"]))continue;if(!goog.isDef(oFormValues[aParents[i]["name"]]["selectedOption"]["value"])){if(!goog.isArray(oFormValues[aParents[i]["name"]]["selectedOption"]))continue;
if(oFormValues[aParents[i]["name"]]["selectedOption"].length<1)continue}var aParentValues=[];if(!goog.isArray(oFormValues[aParents[i]["name"]]["selectedOption"])&&goog.isDef(oFormValues[aParents[i]["name"]]["selectedOption"]["value"]))aParentValues.push(oFormValues[aParents[i]["name"]]["selectedOption"]);else aParentValues=oFormValues[aParents[i]["name"]]["selectedOption"];var key=aParents[i]["filter_attr"];var equality=aParents[i]["filter_equality"];var parentFilter={"relation":"OR","operators":[]};
for(var ii=0;ii<aParentValues.length;ii++){var value=aParentValues[ii]["value"];if(!goog.isDefAndNotNull(value))continue;else if(goog.isString(value)&&!value.length>0)continue;parentFilter["operators"].push({"column":key,"compare_operator":equality,"value":value});delete value}if(parentFilter["operators"].length===0)continue;additionalFilter["operators"].push(parentFilter);delete aParentValues;delete key;delete equality;delete parentFilter}delete i;if(additionalFilter["operators"].length>0)originalFilter["operators"].push(additionalFilter);
...
"scope":scope,"headers":{"Accept":"application/x-vm-json"},"success":function(response){scope["userGroups"]=response.data["data"][0]["groups_label"].split(",");for(var group in scope["oFormDefinition"][sFormDefinitionName]["wab"][scope["wabState"]])if(scope["userGroups"].indexOf(group)>-1){scope["wabGroup"]=group;break}if(!goog.isDefAndNotNull(scope["wabGroup"]))console.warn("this user can't see this form")}});if(!goog.isDefAndNotNull(scope["wabGroup"]))console.warn("this user can't see this form")}});
...
formReader.appFormFieldSpecificParamsDrtv=function($timeout,$translate,propertiesSrvc,formReaderService,$log){return{link:function(scope,element,attrs){$log.log("formReader.appFormFieldSpecificParamsDrtv.link");if(typeof scope["field"]["attributes"]!=="undefined"){var i=0;var aKeys=Object.keys(scope["field"]["attributes"]);while(i<aKeys.length){element[0]["setAttribute"](aKeys[i],scope["field"]["attributes"][aKeys[i]]);i++}}if(scope["field"]["autofocus"]===true)element[0].focus();switch(scope["field"]["type"]){case "captcha":$timeout(function(){var test=
...
"input-file-"+scope["oFormDefinition"][scope["sFormDefinitionName"]]["input_size"];if(propertiesSrvc["language"]!="en")oOptions["language"]=propertiesSrvc["language"];$(element)["fileinput"](oOptions);break;case "file_wsdata":element[0].addEventListener("change",function(){var oFileList={"aFiles":this.files};scope["oFormValues"][scope["sFormDefinitionName"]][element[0].name]=oFileList},false);var aAvaliableFormats=[];if(goog.isDefAndNotNull(scope["field"]["formats"]))aAvaliableFormats=scope["field"]["formats"].split("|");
var oOptions={"showPreview":false,"showRemove":false,"showUpload":false,"allowedFileExtensions":aAvaliableFormats};if(typeof scope["field"]["options"]!=="undefined")goog.object.extend(oOptions,scope["field"]["options"]);oOptions["mainClass"]="input-file-"+scope["oFormDefinition"][scope["sFormDefinitionName"]]["input_size"];if(propertiesSrvc["language"]!="en")oOptions["language"]=propertiesSrvc["language"];$(element)["fileinput"](oOptions);break;case "image_wsdata":if(!goog.isDefAndNotNull(scope["field"]["display_width"]))scope["field"]["display_width"]=
"100%";element[0].addEventListener("change",function(){var oFileList={"aFiles":this.files};scope["oFormValues"][scope["sFormDefinitionName"]][element[0].name]=oFileList},false);var oOptions={"showPreview":false,"showRemove":false,"showUpload":false,"allowedFileTypes":["image"]};oOptions["mainClass"]="input-file-"+scope["oFormDefinition"][scope["sFormDefinitionName"]]["input_size"];if(propertiesSrvc["language"]!="en")oOptions["language"]=propertiesSrvc["language"];$(element)["fileinput"](oOptions);
break;case "email":scope["field"]["pattern"]="^(([a-zA-Z]|[0-9])|([-]|[_]|[.]))+[@](([a-zA-Z]|[0-9])|([-]|[_]|[.])){2,63}[.](([a-zA-Z0-9]){2,63})+$";var pattern=angular.copy(scope["field"]["pattern"]);scope.$watch("oFormValues."+scope["sFormDefinitionName"]+'["'+scope["field"]["name"]+'"]',function(){scope["field"]["pattern"]=angular.copy(pattern);$timeout(function(){if(element[0]["className"].indexOf("ng-invalid-pattern")!==-1){var advice;if(scope["oProperties"]["language"]==="fr")advice="ceci n'est pas une adresse mail";
else advice="this is not a mail adress";$(element).notify(advice,{"position":"top","className":"error","autoHide":false,"clickToHide":false})}else{var tabChild=element[0]["parentNode"]["children"];var elToDelete=-1;for(var i=0;i<tabChild.length;i++)if(tabChild[i]["className"]==="notifyjs-wrapper")elToDelete=i;if(elToDelete>-1)$(tabChild[elToDelete]).remove()}},1)});break;case "url":scope["field"]["pattern"]="^https?:\\/\\/.{1,}";var pattern=angular.copy(scope["field"]["pattern"]);scope.$watch("oFormValues."+
scope["sFormDefinitionName"]+'["'+scope["field"]["name"]+'"]',function(){scope["field"]["pattern"]=angular.copy(pattern);$timeout(function(){if(element[0]["className"].indexOf("ng-invalid-pattern")!==-1){var advice;if(scope["oProperties"]["language"]==="fr")advice="ce champ doit contenir une url qui commence\n par http:// ou https://";else advice="this field must contain an url that begin\n by http:// or https://";$(element).notify(advice,{"position":"top","className":"error","autoHide":false,"clickToHide":false})}else{var tabChild=
element[0]["parentNode"]["children"];var elToDelete=-1;for(var i=0;i<tabChild.length;i++)if(tabChild[i]["className"]==="notifyjs-wrapper")elToDelete=i;if(elToDelete>-1)$(tabChild[elToDelete]).remove()}},1)});break;case "integer":if(typeof scope["field"]["min"]!=="undefined")element[0].setAttribute("min",scope["field"]["min"]);if(typeof scope["field"]["max"]!=="undefined")element[0].setAttribute("max",scope["field"]["max"]);if(typeof scope["field"]["max_length"]!=="undefined")element[0].setAttribute("maxLength",
scope["field"]["max_length"]);scope["$parent"]["valid"]=true;scope["numberUpdate"]=function(){var val=scope["oFormValues"][scope["sFormDefinitionName"]][element[0].name];if(goog.isDef(val)&&(typeof val==="string"||typeof val==="number"))if(typeof val==="string"){var tmp=scope["ctrl"]["numberParser"](val,true);if(!isNaN(tmp))scope["oFormValues"][scope["sFormDefinitionName"]][element[0].name]=tmp;else scope["oFormValues"][scope["sFormDefinitionName"]][element[0].name]=""}else scope["oFormValues"][scope["sFormDefinitionName"]][element[0].name]=
val;else console.error("bad type argument")};$timeout(function(){scope["oFormValues"][scope["sFormDefinitionName"]][element[0].name]+=" ";scope["numberUpdate"]()},500);break;case "float":case "number":if(typeof scope["field"]["min"]!=="undefined")element[0].setAttribute("min",scope["field"]["min"]);if(typeof scope["field"]["max"]!=="undefined")element[0].setAttribute("max",scope["field"]["max"]);if(typeof scope["field"]["max_length"]!=="undefined")element[0].setAttribute("maxLength",scope["field"]["max_length"]);
scope["$parent"]["valid"]=true;scope["numberUpdate"]=function(){var val=scope["oFormValues"][scope["sFormDefinitionName"]][element[0].name];if(goog.isDef(val)&&(typeof val==="string"||typeof val==="number"))if(typeof val==="string"){var tmp=scope["ctrl"]["numberParser"](val,false);if(!isNaN(tmp)||typeof tmp==="string")scope["oFormValues"][scope["sFormDefinitionName"]][element[0].name]=tmp;else scope["oFormValues"][scope["sFormDefinitionName"]][element[0].name]=""}else scope["oFormValues"][scope["sFormDefinitionName"]][element[0].name]=
val;else console.error("bad type argument")};$timeout(function(){scope["oFormValues"][scope["sFormDefinitionName"]][element[0].name]+=" ";scope["numberUpdate"]()},500);break;case "image":case "imageurl":case "image_wsdata":if(goog.isDefAndNotNull(scope["field"])){if(goog.isDefAndNotNull(scope["field"]["width"]))if(scope["field"]["width"]==0||scope["field"]["width"]=="0px")scope["field"]["width"]=null;if(goog.isDefAndNotNull(scope["field"]["style"]))if(goog.isDefAndNotNull(scope["field"]["style"]["height"]))if(scope["field"]["style"]["height"]==
0||scope["field"]["style"]["height"]=="0px")scope["field"]["style"]["height"]=null}break;case "image":var options={"inline":false,"button":true,"navbar":false,"title":2,"toolbar":2,"tooltip":true,"fullscreen":false,"url":function(){return scope["oProperties"]["web_server_name"]+"/"+scope["oProperties"][scope["field"]["alias"]]+"/"+scope["field"]["folder"]+"/"+scope["oFormValues"][scope["sFormDefinitionName"]][scope["field"]["name"]]}};scope["viewer"]=new Viewer(element[0],options);scope["openViewer"]=
...
angular.element(".viewer-next").remove()};break;case "select":case "editable_select":var oField=scope["field"];var fieldValue=scope["oFormValues"][scope["sFormDefinitionName"]][scope["field"]["name"]];if(typeof fieldValue=="string"&&!isNaN(fieldValue))if(fieldValue.length==1||fieldValue.search(/^[1-9][0-9]+$/)!=-1)scope["oFormValues"][scope["sFormDefinitionName"]][scope["field"]["name"]]=Number(fieldValue);if(typeof oField["multiple"]!=="undefined"&&oField["multiple"]===true)element[0].setAttribute("multiple",
"");if(typeof oField["size"]!=="undefined"){element[0].setAttribute("size",oField["size"]);element[0].style.height="auto"}if(goog.isString(oField["options"])){var tmp=oField["options"].split(",");var tmp2;for(var i=0;i<tmp.length;i++){tmp2=tmp[i].split("|");tmp[i]={"label":tmp2[0],"value":tmp2[1]}}scope["oFormValues"][scope["sFormDefinitionName"]][oField["name"]]["options"]=tmp}if(typeof scope["field"]["child_select"]!="undefined")element[0].addEventListener("change",function(){scope["reloadSelectField"](scope["field"],
scope["sFormDefinitionName"])},false);if(goog.isDefAndNotNull(oField["onchange"]))element[0].addEventListener("change",function(){if(formReaderService["isFunctionCall"](oField["onchange"]))if(goog.isString(oField["onchange"]))formReaderService["callFunction"](oField["onchange"]);else if(goog.isFunction(oField["onchange"]))oField["onchange"](scope["oFormValues"])},false);break;case "map_bing":case "map_osm":case "map_vmap":var initMap=function(){if(goog.isDef(scope["oMap"])){scope["oMap"].MapObject.setTarget(null);
delete scope["oMap"]}scope["oMap"]=new nsVitisComponent.Map({"target":element[0],"options":scope["field"]["map_options"],"hiddenFieldId":scope["field"]["id"],"oFormValues":scope["oFormValues"][scope["sFormDefinitionName"]],"sFormName":scope["field"]["name"],"oProj":scope.$root["proj"]});if(typeof scope["field"]["style"]!="undefined")if(typeof scope["field"]["style"]["height"]!="undefined"&&scope["field"]["style"]["height"]=="ratio"){var ratio=window.innerWidth/window.innerHeight;scope["field"]["style"]["height"]=
parseInt(element[0].clientWidth/ratio)+"px"}scope.$root.$emit("OpenLayersMapCreated",scope["oMap"]);scope["oMap"].on("moveend",function(){scope["field"]["map_options"]["center"]=scope["oMap"]["getConfig"]();scope.$apply()},scope["oMap"])};setTimeout(function(){initMap();scope.$watch("field.nb_cols",function(value,old){$timeout(function(){scope["oMap"].MapObject.updateSize()})});scope.$watch("field.style",function(value,old){$timeout(function(){scope["oMap"].MapObject.updateSize()})},true);scope.$watch("oMap.bLayersTreeOpen",
function(value,old){$timeout(function(){scope["oMap"].MapObject.updateSize()})},true);scope.$on("studio resized",function(){scope["oMap"].MapObject.updateSize()});scope.$on("updateMap",function(event,data){switch(data){case "center":scope["oMap"]["setCenter"]([scope["field"]["map_options"]["center"]["coord"][0],scope["field"]["map_options"]["center"]["coord"][1]]);break;case "zoom":scope["oMap"]["setZoom"](scope["field"]["map_options"]["center"]["zoom"]);break;case "source":scope["oMap"]["setBingSource"](scope["field"]["map_options"]["source"]);
break;case "extent":scope["oMap"]["setExtent"](scope["field"]["map_options"]["center"]["extent"]);break;case "controls":scope["oMap"]["setControls"](scope["field"]["map_options"]["controls"]);break;case "interact":scope["oMap"]["setInteractions"](scope["field"]["map_options"]["interactions"]);break;case "proj":var extent=scope["oMap"]["getExtent"]();var center=scope["oMap"]["getCenter"]();var proj=scope["oMap"]["getProj"]();scope["field"]["map_options"]["center"]["extent"]=scope["oMap"]["transformExtent"](extent,
proj,scope["field"]["map_options"]["proj"]);scope["field"]["map_options"]["center"]["coord"]=scope["oMap"]["transformer"](center,proj,scope["field"]["map_options"]["proj"]);initMap();break;default:initMap();break}})});break;case "slider":scope["field"]["options"]["id"]=scope["field"]["id"]+"_slider";var mySlider=$(element)["slider"](scope["field"]["options"]);if(!goog.isDefAndNotNull(scope["oFormValues"][scope["sFormDefinitionName"]][scope["field"]["name"]]))scope["oFormValues"][scope["sFormDefinitionName"]][scope["field"]["name"]]=
...
case "text":if(typeof scope["field"]["max_length"]!=="undefined")element[0].setAttribute("maxLength",scope["field"]["max_length"]);break;case "treeview":var oTreeviewOptions=angular.copy(scope["field"]["options"]);formReaderService["callFunction"](oTreeviewOptions["data"]).then(function(aNodes){oTreeviewOptions["data"]=aNodes;if(typeof oTreeviewOptions["showCheckbox"]!="undefined"&&oTreeviewOptions["showCheckbox"]===true){this["setTreeviewNodesState"]=function(oNode){for(var i in oNode)if(typeof oNode[i]["nodes"]!=
"undefined")this["setTreeviewNodesState"](oNode[i]["nodes"]);else{if(typeof oNode[i]["state"]=="undefined")oNode[i]["state"]={};var aCheckedItem=[];var sValue=scope["oFormValues"][scope["sFormDefinitionName"]][scope["field"]["name"]];if(typeof sValue=="string"&&sValue!="")aCheckedItem=sValue.split("|");if(aCheckedItem.indexOf(String(oNode[i]["value"]))!=-1)oNode[i]["state"]["checked"]=true}};this["setTreeviewNodesState"](oTreeviewOptions["data"]);oTreeviewOptions["onNodeChecked"]=function(event,data){if(typeof data["nodes"]!=
"undefined")for(var i in data["nodes"])$(element)["treeview"]("checkNode",data["nodes"][i]["nodeId"]);else{var aCheckedItem=[];if(typeof data["value"]!="undefined"){var sValue=scope["oFormValues"][scope["sFormDefinitionName"]][scope["field"]["name"]];if(typeof sValue=="string"&&sValue!="")aCheckedItem=sValue.split("|");aCheckedItem.push(data["value"]);scope["oFormValues"][scope["sFormDefinitionName"]][scope["field"]["name"]]=aCheckedItem.join("|")}}};oTreeviewOptions["onNodeUnchecked"]=function(event,
data){if(typeof data["nodes"]!="undefined")for(var i in data["nodes"])$(element)["treeview"]("uncheckNode",data["nodes"][i]["nodeId"]);else{var aCheckedItem=[];if(typeof data["value"]!="undefined"){var sValue=scope["oFormValues"][scope["sFormDefinitionName"]][scope["field"]["name"]];if(typeof sValue=="string"&&sValue!="")aCheckedItem=sValue.split("|");aCheckedItem.splice(aCheckedItem.indexOf(data["value"]),1);scope["oFormValues"][scope["sFormDefinitionName"]][scope["field"]["name"]]=aCheckedItem.join("|")}}}}$(element)["treeview"](oTreeviewOptions)});
break}if(typeof scope["field"]["read_only"]!="undefined"&&scope["field"]["read_only"]==true)element[0].readOnly=true}}};formReader.module.directive("appFormFieldSpecificParams",formReader.appFormFieldSpecificParamsDrtv);
...
true&&oFormElementDefinition["required"]!=="true"){var isEmptyValuePresent=false;for(var i=0;i<oSelectedOptions["options"].length;i++)if(oSelectedOptions["options"][i]["value"]==="")isEmptyValuePresent=true;if(!isEmptyValuePresent)oSelectedOptions["options"].unshift({"label":"","value":""})}}oFormValues[sToSelectName]=oSelectedOptions;if(oFormElementDefinition["type"]!=="editable_select"){var selectedIsInOption=false;for(var i=0;i<oFormValues[sToSelectName]["options"].length;i++)if(oSelectedOptions["selectedOption"]["value"]===
...
0;i<aOptionsKeys.length;i++)if(isNaN(aOptionsKeys[i]))aOptionsKeys[i]=translations[aOptionsKeys[i]];oSelectOptions["options"]=getFormatedOptions(aOptionsKeys,aOptionsValues);if(oFormElementDefinition["type"]==="double_select"){sFromSelectName=oFormElementDefinition["name_from"];sToSelectName=oFormElementDefinition["name_to"];var aFromOptionsKeys=angular.copy(aOptionsKeys);var aFromOptionsValues=angular.copy(aOptionsValues);var aToOptionsKeys=[];var aToOptionsValues=[];for(var i=aFromOptionsValues.length-
1;i>=0;i--){aToOptionsValues.unshift(aFromOptionsValues[i]);aToOptionsKeys.unshift(aFromOptionsKeys[i]);aFromOptionsValues.splice(i,1);aFromOptionsKeys.splice(i,1)}var oFromSelectOptions={"options":getFormatedOptions(aFromOptionsKeys,aFromOptionsValues)};var oToSelectOptions={"options":getFormatedOptions(aToOptionsKeys,aToOptionsValues)};oFormValues[sFormDefinitionName][sFromSelectName]=oFromSelectOptions;oFormValues[sFormDefinitionName][sToSelectName]=oToSelectOptions}else{sSelectName=oFormElementDefinition["name"];
oFormValues[sFormDefinitionName][sSelectName]=oSelectOptions}if(typeof envSrvc["oFormDefaultValues"][sFormDefinitionName]==="undefined")envSrvc["oFormDefaultValues"][sFormDefinitionName]={};if(typeof envSrvc["oFormDefaultValues"][sFormDefinitionName][sSelectName]=="undefined")envSrvc["oFormDefaultValues"][sFormDefinitionName][sSelectName]=angular.copy(oSelectOptions)})},"selectOptionAfterLoad":function(oFormValues,oFormElementDefinition,sFormDefinitionName,sFieldName,value){if(oFormElementDefinition["type"]===
"double_select");else if(oFormElementDefinition["type"]==="select"){var opt=oFormValues[sFormDefinitionName][sFieldName].options;for(var i=0;i<opt.length;i++)if(opt[i].value===value){oFormValues[sFormDefinitionName][sFieldName].selectedOption=opt[i];break}}else if(oFormElementDefinition["type"]==="list");else console.error("this element is not a select")},"extractWebServiceData":function(sController,oWebServiceData){var sKey=sController;var aData=[];if(typeof oWebServiceData[sKey]!=="undefined")if(Array.isArray(oWebServiceData[sKey])){if(oWebServiceData[sKey].length>
0)aData=oWebServiceData[sKey]}else aData=oWebServiceData[sKey];return aData},"parseFilter":function(filter){if(goog.isObject(filter)){var isJSONFilter=false;if(goog.isDefAndNotNull(filter["operators"])&&goog.isDefAndNotNull(filter["relation"]))isJSONFilter=true;if(goog.isDefAndNotNull(filter["column"])&&goog.isDefAndNotNull(filter["compare_operator"])&&goog.isDefAndNotNull(filter["value"]))isJSONFilter=true;if(!isJSONFilter){var aFilters=[];var sFilterValue;for(var key in filter)if(key==="stringFilter")aFilters.push(filter[key]);
else{sFilterValue=filter[key];if(sFilterValue!==""){if(this["isFunctionCall"](sFilterValue))sFilterValue=this["callFunction"](sFilterValue);if(goog.isDef(sFilterValue)){var sCompareOperator="=";if(goog.isString(sFilterValue))if(sFilterValue.toLowerCase().trim()==="not null"){sCompareOperator="!=";sFilterValue="NULL"}aFilters.push({"column":key,"compare_operator":sCompareOperator,"value":sFilterValue})}}}if(aFilters.length>0){filter={"relation":"AND","operators":[]};for(var i=0;i<aFilters.length;i++)filter["operators"].push(aFilters[i])}else filter=
""}}return filter},"setWebServiceTags":function(oFormElementDefinition,sFormDefinitionName,oFormValues){var oWebService=angular.copy(oFormElementDefinition["web_service"]);var aRessourceId=oWebService["ressource_id"].split("/");var oFormValues=oFormValues[sFormDefinitionName],selectedOptionValue;var oFormSrvc=this;var oParams={};if(typeof oWebService["parameters"]!=="undefined"){oParams=oWebService["parameters"];if(typeof oWebService["parameters"]["filter"]!=="undefined"){oWebService["parameters"]["filter"]=
this["parseFilter"](oWebService["parameters"]["filter"]);if(oWebService["parameters"]["filter"]==="")delete oWebService["parameters"]["filter"]}}ajaxRequest({"method":"POST","url":this["oProperties"]["web_server_name"]+"/"+this["oProperties"]["services_alias"]+"/"+oWebService["ressource_id"],"headers":{"X-HTTP-Method-Override":"GET"},"data":oParams,"scope":$rootScope,"success":function(response){var data=response["data"];var j,oNotSelectedOptions,oSelectedOptions,sFromSelectName="",sToSelectName;
if(data["status"]===1){var aData=oFormSrvc["extractWebServiceData"](aRessourceId[1],data);var aTags=[];if(typeof oFormValues[oFormElementDefinition["name"]]==="string")aTags=oFormValues[oFormElementDefinition["name"]].split(",");var sLabelKey=oWebService["label_key"];var sValueKey=oWebService["id_key"];var i=0;var aAvailableTags=[];while(i<aData.length){if(aTags.indexOf(aData[i][sValueKey])===-1)aAvailableTags.push(aData[i][sValueKey]);i++}oFormValues[oFormElementDefinition[["name_available_tags"]]]=
...
sName=sName.trim();var oFormValues=this["oFormValues"][this["sFormDefinitionName"]];if(!goog.isDefAndNotNull(oFormValues))return null;var sValue=oFormValues[sName.trim()];if(goog.isDefAndNotNull(sValue["selectedOption"]))if(goog.isDefAndNotNull(sValue["selectedOption"]["value"]))sValue=sValue["selectedOption"]["value"];return sValue},"concat":function(){$log.info("concat");var sResult="";for(var i=0;i<arguments.length;i++)if(goog.isString(arguments[i]))sResult+=arguments[i];return sResult},"getBusinessObjectDef":function(sBusinessObject){var deferred=
$q.defer();showAjaxLoader();ajaxRequest({"method":"GET","url":this["oProperties"]["web_server_name"]+"/"+this["oProperties"]["services_alias"]+"/vmap/businessobjects/"+sBusinessObject,"scope":$rootScope,"headers":{"Accept":"application/x-vm-json"},"success":function(response){hideAjaxLoader();if(!goog.isDef(response["data"])){$translate("NO_VALUES_RETURNED").then(function(sTranslation){bootbox.alert(sTranslation)});return 0}if(!goog.isDef(response["data"]["data"]))return 0;if(!goog.isDef(response["data"]["data"][0]))return 0;
if(!goog.isDef(response["data"]["data"][0]["business_object_id"]))return 0;if(goog.isDef(response["data"]["errorMessage"])){bootbox.alert(response["data"]["errorMessage"]);return 0}deferred.resolve(response["data"]["data"][0])},"error":function(response){hideAjaxLoader();console.error(response);bootbox.alert(response)}});return deferred.promise},"showModalSubform":function(sModalId,oSubformScope,oSubformDefinition,oSubformDefinitionName,oSubformValues){var sToken=this["sToken"];var oProperties=this["oProperties"];
if(goog.isDefAndNotNull(oSubformScope["loadSubForm"])){oSubformScope["loadSubForm"]({"sFormDefinitionName":oSubformDefinitionName,"oFormDefinition":oSubformDefinition,"oFormValues":oSubformValues,"oProperties":oProperties,"sToken":sToken});$("#"+sModalId).modal("show")}},"updateBoElement":function(sBusinessObject,oObject,oSubformDefinition,sSubformDefinitionName,oFormValuesResulted,successCallback){var oProperties=this["oProperties"];var envSrvc=angular.element(vitisApp.appMainDrtv).injector().get(["envSrvc"]);
var formSrvc=angular.element(vitisApp.appMainDrtv).injector().get(["formSrvc"]);var aParamsToSave=[oObject["bo_id_field"]];envSrvc["oFormValues"]=oFormValuesResulted;envSrvc["oFormDefinition"]=oSubformDefinition;envSrvc["sFormDefinitionName"]=sSubformDefinitionName;var oValues=formSrvc["getFormData"](sSubformDefinitionName,true);for(var i=0;i<aParamsToSave.length;i++)if(goog.isDefAndNotNull(oFormValuesResulted[sSubformDefinitionName][aParamsToSave[i]]))oValues[aParamsToSave[i]]=oFormValuesResulted[sSubformDefinitionName][aParamsToSave[i]];
var oFormData=new FormData;for(var key in oValues)if(goog.isDefAndNotNull(oValues[key]["aFiles"])){oFormData.append(key+"_attached_content",oValues[key]["aFiles"][0]);oFormData.append(key,oValues[key]["aFiles"][0]["name"])}else oFormData.append(key,oValues[key]);ajaxRequest({"method":"PUT","url":this["oProperties"]["web_server_name"]+"/"+this["oProperties"]["services_alias"]+"/vmap/querys/"+sBusinessObject,"data":oFormData,"scope":$rootScope,"success":function(response){if(!goog.isDef(response["data"])){$translate("NO_VALUES_RETURNED").then(function(sTranslation){bootbox.alert(sTranslation)});
return 0}if(goog.isDef(response["data"]["errorMessage"])){bootbox.alert(response["data"]["errorMessage"]);return 0}if(goog.isDef(response["status"]))if(response["status"]===500){bootbox.alert(response["statusText"]);return 0}successCallback.call()},"error":function(response){bootbox.alert(response);console.error(response)}})},"insertBoElement":function(sBusinessObject,oBusinessObject,oSubformDefinition,sSubformDefinitionName,oFormValuesResulted,successCallback){var oProperties=this["oProperties"];
...
oFormValuesResulted[sSubformDefinitionName][aParamsToSave[i]];var oFormData=new FormData;for(var key in oValues)if(goog.isDefAndNotNull(oValues[key]["aFiles"])){oFormData.append(key+"_attached_content",oValues[key]["aFiles"][0]);oFormData.append(key,oValues[key]["aFiles"][0]["name"])}else oFormData.append(key,oValues[key]);ajaxRequest({"method":"POST","url":this["oProperties"]["web_server_name"]+"/"+this["oProperties"]["services_alias"]+"/vmap/querys/"+sBusinessObject,"data":oFormData,"scope":$rootScope,
"success":function(response){if(!goog.isDef(response["data"])){$translate("NO_VALUES_RETURNED").then(function(sTranslation){bootbox.alert(sTranslation)});return 0}if(goog.isDef(response["data"]["errorMessage"])){bootbox.alert(response["data"]["errorMessage"]);return 0}if(goog.isDef(response["status"]))if(response["status"]===500){bootbox.alert(response["statusText"]);return 0}successCallback.call()},"error":function(response){bootbox.alert(response);console.error(response)}})},"deleteBoElements":function(sBusinessObject,
aIds,successCallback){var oProperties=this["oProperties"];ajaxRequest({"method":"DELETE","url":this["oProperties"]["web_server_name"]+"/"+this["oProperties"]["services_alias"]+"/vmap/querys/"+sBusinessObject,"params":{"idList":aIds.join("|")},"scope":$rootScope,"success":function(response){if(!goog.isDef(response["data"])){$translate("NO_VALUES_RETURNED").then(function(sTranslation){bootbox.alert(sTranslation)});return 0}if(goog.isDef(response["data"]["errorMessage"])){bootbox.alert(response["data"]["errorMessage"]);
return 0}if(goog.isDef(response["status"]))if(response["status"]===500){bootbox.alert(response["statusText"]);return 0}successCallback.call()},"error":function(response){bootbox.alert(response);console.error(response)}})},"getSectionDef":function(sSection){var oProperties=this["oProperties"];var deferred=$q.defer();ajaxRequest({"method":"GET","url":this["oProperties"]["web_server_name"]+"/"+this["oProperties"]["services_alias"]+"/vitis/vitissections/"+sSection,"scope":$rootScope,"success":function(response){if(!goog.isDef(response["data"])){$translate("NO_VALUES_RETURNED").then(function(sTranslation){bootbox.alert(sTranslation)});
...
return 0}deferred.resolve(response["data"]["data"][0])},"error":function(response){bootbox.alert(response);console.error(response)}});return deferred.promise},"getSectionRessourceTab":function(sRessourceId){var oProperties=this["oProperties"];var deferred=$q.defer();ajaxRequest({"method":"GET","url":this["oProperties"]["web_server_name"]+"/"+this["oProperties"]["services_alias"]+"/vitis/ressources/"+sRessourceId,"headers":{"Accept":"application/json"},"scope":$rootScope,"success":function(response){if(!goog.isDef(response["data"])){$translate("NO_VALUES_RETURNED").then(function(sTranslation){bootbox.alert(sTranslation)});
