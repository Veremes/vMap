...
class Server extends Socket {
    ...
    public function __construct($host = 'localhost', $port = 8000, $ssl = false, $sVasHome = '') {
		$this->_vasHome = $sVasHome;
    ...
    public function registerApplication($key, $application) {
        ...
        if ($key === 'websocket') {
            $this->applications[$key]->setServerObject($this);
        }
    }
    ...
    public function log($message, $type = 'info') {
		$sFileName = $this->_vasHome . "/log/webSocket/websocket_" . date('Y-m-d') . ".log";
		if (!is_dir(dirname($sFileName)))
			mkdir(dirname($sFileName), 0777, true);                
		$handle = fopen($sFileName, 'a');
		fwrite($handle, date("d/n/Y H:i:s") . ' [' . ($type ? $type : 'error') . '] ' . '|' . $message . CR . LF);
		fclose($handle);
    ...
    private function _checkRequestLimit($clientId) {
        ...
        if ($this->_requestStorage[$clientId]['totalRequests'] > $this->_maxRequestsPerMinute) {
            $this->log('Max requests reached: '. $clientId);
            return false;
        }
